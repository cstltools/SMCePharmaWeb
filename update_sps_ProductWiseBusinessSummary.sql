...
isnull(tblRtn.ReturnAmountTP,0) +  isnull(tblOldRtn.ReturnAmountTP,0) + isnull(tbl2Rtn.TP,0) AS SumofNetReturnAmount 
,isnull(tblRtn.ReturnAmountVat,0)+isnull(tblOldRtn.ReturnAmountVat,0) + isnull(tbl2Rtn.vat,0)   AS DelReTpVat ,
isnull(tblRtn.ReturnGrossAmt,0)+isnull(tblOldRtn.ReturnGrossAmt,0)+ isnull(tbl2Rtn.gross,0) GrossRetuen
...
, (ISNULL(tblSaleBonusQty.DeliveryQuantity,0)) -  (ISNULL(tblRtnBonus.Retqty,0))  AS bouns,
 (isnull(tblSale.SalesAmtTP,0)+  isnull(tblOldRtn.ReturnAmountTP,0) )-(isnull(tblRtn.ReturnAmountTP,0) +  isnull(tblOldRtn.ReturnAmountTP,0)) - isnull(tbl2Rtn.TP,0) AS SumofNetSalesAmount,
(isnull(tblSale.SalesVat,0) +  isnull(tblOldRtn.ReturnAmountVat,0) )-(isnull(tblRtn.ReturnAmountVat,0) +  isnull(tblOldRtn.ReturnAmountVat,0)) - isnull(tbl2Rtn.vat,0) AS DelTpVat,
 ((isnull(tblSale.SalesGrossAmt,0))+isnull(tblOldRtn.ReturnGrossAmt,0))-(isnull(tblRtn.ReturnGrossAmt,0)+isnull(tblOldRtn.ReturnGrossAmt,0)) - isnull(tbl2Rtn.gross,0)  AS GrossSales,
...
where RejectionSts='Old' and ID.DeliveryStatus IN ('Reject','Partial') and I.UpdateDate BETWEEN @fromdate AND @todate  GROUP BY  ID.ProductCode ) tblOldRtn ON tblOldRtn.ProductCode=C.ProductCode
	 ...
	 ---return 2 (2nd / sndReturn payment) - mirrors tbl2Rtn in sp_RPT_MIS_BusinessSummary,
	 --- without it Gross Sales Amt here is overstated by the whole 2nd-return amount.
LEFT JOIN (
...
((sum(ISNULL(ivD.PaymentTotalPrice- r.sndReturnTotalPrice,0)) )-sum(ISNULL(ivD.PaymentDiscountAmount- r.sndReturnDiscountAmount,0)) ) + sum(ISNULL(ivD.PaymentTotalPriceVatAmount- r.sndReturnTotalPriceVatAmount,0)) gross
...
INNER JOIN dbo.tblInvoiceDetail ivD with(nolock) on iv.InvoiceId=ivD.InvoiceId
INNER JOIN dbo.tblInvoiceDetailReturn r with(nolock) on r.InvoiceDetailId=ivD.InvoiceDetailId
INNER JOIN dbo.tblOrder mas  with(nolock) on mas.OrderId=iv.OrderId

where  r.PreviousQuantity<>r.sndReturnQuantity
 and CONVERT(date,iv.SndReturnPaymentDate) between CONVERT(date,@fromdate) AND CONVERT(date,@todate) and  (mas.AreaId= COALESCE( NULLIF(@Area , 0) ,mas.AreaId)) and     (mas.RegionId= COALESCE( NULLIF(@ZonId , 0) ,mas.RegionId))
...
where  r.PreviousQuantity<>r.sndReturnQuantity
 and CONVERT(date,iv.SndReturnPaymentDate) between CONVERT(date,@fromdate) AND CONVERT(date,@todate) and (mas.TerritoryId= COALESCE( NULLIF(@Terr , 0) ,mas.TerritoryId))  and  (mas.AreaId= COALESCE( NULLIF(@Area , 0) ,mas.AreaId)) and     (mas.RegionId= COALESCE( NULLIF(@ZonId , 0) ,mas.RegionId))
