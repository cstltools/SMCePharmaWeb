...
{
    public class SubDepoAdjustBLL
    {
        ChalanInfo aChalanInfo = new ChalanInfo();
        SubDepoAdjustDAL aChalanDal = new SubDepoAdjustDAL();
        ClsPrimaryKeyFind aClsPrimaryKeyFind = new ClsPrimaryKeyFind();
        public bool SaveDataForSubDepoAdjust(SubDepoAdjustMentDAO aChalanInfo, out int ChalanId)
        {
            ChalanId = aClsPrimaryKeyFind.PrimaryKeyMax("SubDepoStockOutId", "tblSubDepoStockAdjustment");
            aChalanInfo.SubDepoStockOutId = ChalanId;
            aChalanInfo.SubDepoStockCode = ChalanNoGenerator(aChalanInfo.SubDepoStockOutId);
            return aChalanDal.SaveDataForSubDepoAdjust(aChalanInfo);
        }
        public bool SaveDataForSubDepoAdjustDetail(List<SubDepoAdjustDetailDAO> adjustDetailDao)
        {
            bool status = false;
            foreach (var subDepoAdjustDetailDao in adjustDetailDao)
            {
                
                status=aChalanDal.SaveDataForSubDepoAdjustDetail(subDepoAdjustDetailDao);
                if (status)
                {
                    aChalanDal.UpdateSubDepoStockQuantity(subDepoAdjustDetailDao.SubDCStoreId.ToString(),subDepoAdjustDetailDao.Quantity);
                }
            }
            return status;
        }
        public int DCStockIn2(DCStockNew aDcStockNew)
        {
            ...
            SubDCStoreId = aDcStockNew.SubDCStoreId = aClsPrimaryKeyFind.PrimaryKeyMax("SubDCStoreId", "tblSubDepotStore", "SSIDB");
        ...
        public bool SaveDataForChalanDetail(List<ChalanDetail> aIChalanList)
        {
            ...
            {
                chalanDetail.ChalanDetailId = aClsPrimaryKeyFind.PrimaryKeyMax("SChalanDetailsId", "tblSubDepotChalanDetail");
        ...
        {
            string ChalanNo = "";
            ChalanNo = ChalanNoGenerator(aClsPrimaryKeyFind.PrimaryKeyMax("SChalanId", "tblSubDepotChalanInfo"));
            return ChalanNo;
        }
