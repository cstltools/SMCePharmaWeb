...
namespace Library.BLL.SInventory_BLL
{
    public class InvoiceBLL
    {
        ...
        public bool SaveInvoice(Invoice aInvoice, out int invoiceId, out string invoiceNo)
        {
            ...
            batchn = HttpContext.Current.Session["RouteId"].ToString() + "-" + batchn;
            aInvoiceDal.SaveDataForInvoiceBatch( aInvoice.InvoiceId.ToString(), batchn);

            return aInvoiceDal.SaveDataForInvoice(aInvoice);
        }

        /// Same as <see cref="SaveInvoice(Invoice, out int, out string)"/> but writes participate
        /// in the caller's SqlTransaction instead of auto-committing independently.
        public bool SaveInvoice(Invoice aInvoice, out int invoiceId, out string invoiceNo, SqlTransaction transaction)
        {
            invoiceId = aClsPrimaryKeyFind.PrimaryKeyMax("invoiceId", "tblInvoice");
            aInvoice.InvoiceId = invoiceId;
            invoiceNo = InvoiceNoGenerator(aInvoice.ComUnitId, aInvoice.ComUnitCode);
            ...
            batchn = (orderInfoBLL.LoadInvoiceBatchId().Rows[0][0].ToString());
            batchn = HttpContext.Current.Session["RouteId"].ToString() + "-" + batchn;
            aInvoiceDal.SaveDataForInvoiceBatch(aInvoice.InvoiceId.ToString(), batchn, transaction);

            return aInvoiceDal.SaveDataForInvoice(aInvoice, transaction);
        }
        ...
        public bool SaveInvoiceDetails(List<InvoiceDetail> aInvoiceList, string comUnitId, SqlTransaction transaction)
        {
            foreach (var invoiceDetail in aInvoiceList)
            {
                UpdateDCStock(invoiceDetail, comUnitId, transaction: transaction);
            }
            return true;
        }
        ...
        {
            return aInvoiceDal.UpdateOrder(status, id, transaction);
        }
        private void UpdateDCStock(InvoiceDetail aInvoiceDetail, string comUnitId, SqlTransaction transaction = null)
        {
            ...
            for (int i = 0; i < aTableBatchQty.Rows.Count; i++)
            {
              ...
              ////////////////////////////////end//////////////////////
                ...
                if (restQty < 0 || restBonusQtyBool==true)
                {
                    ...
                    {
                        InvoiceDetailInsertBLL(aTableBatchQty, i, 0, restBonusQtyOverFlow, aInvoiceDetail, comUnitId,dCStoreId, transaction);
                    }
                    else
                    {
                        InvoiceDetailInsertBLL(aTableBatchQty, i, stockQty - bonusQty, bonusQty, aInvoiceDetail, comUnitId,dCStoreId, transaction);
                    }
                    if (transaction == null)
                        aInvoiceDal.UpdateDCStoreQuantity(dCStoreId, 0);
                    else
                        aInvoiceDal.UpdateDCStoreQuantity(dCStoreId, 0, transaction);
                    bonusQty = 0;
                ...
                else
                {
                    ...
                    InvoiceDetailInsertBLL(aTableBatchQty, i, qty, bonusQty, aInvoiceDetail, comUnitId,dCStoreId, transaction);
                    if (transaction == null)
                        aInvoiceDal.UpdateDCStoreQuantity(dCStoreId, restQty);
                    else
                        aInvoiceDal.UpdateDCStoreQuantity(dCStoreId, restQty, transaction);
                    break;
                }
            }
        }
        private void InvoiceDetailInsertBLL(DataTable aTableBatchQty, int index, decimal qty, decimal bonusQty, InvoiceDetail aInvoiceDetail, string comUnitId,string dcstoreId, SqlTransaction transaction = null)
        {
            ...
            if (transaction == null)
                ...
                aInvoiceDal.SaveDataForInvoiceDetails(aDetailForInsert, transaction);
        }
       ...
       public DataTable ProductInfoBatch(string comUnitId, List<string> productCodes)
       {
           return aInvoiceDal.ProductInfoBatchDAL(comUnitId, productCodes);
       }
       ...
       {
           return aInvoiceDal.LoadProductQtyBatch(orderid);
       }
        ...
        public void UpdateInvoice(Invoice aInvoice, out string invoiceno, SqlTransaction transaction)
        {
            invoiceno = aInvoice.InvoiceNo;
            aInvoiceDal.UpdateInvoice(aInvoice, transaction);
        }

        public void PaymentUpdateInvoice(Invoice aInvoice, out string invoiceno)
        {
            //invoiceno = DelInvoiceNoGenerator(aInvoice.ComUnitId, aInvoice.ComUnitCode);
            //aInvoice.DelivaryInvoiceNo = invoiceno;
            invoiceno = aInvoice.InvoiceNo;
        ...
        public void PaymentUpdateInvoice(Invoice aInvoice, out string invoiceno, SqlTransaction transaction)
        {
            invoiceno = aInvoice.InvoiceNo;
        ...
        ///////////////////////////////////////////////////////////////////////////////
         ...
         {
             return aInvoiceDal.SaveDataForReturnAmount(amountDao, transaction);
         }
