...
| `sp_FixTourPlanMissingSerial` *(added 2026-08-15)* | (none) | SU |  |  |  |  | tbl_TourPlanInfo | `spec/database/procs/sp_FixTourPlanMissingSerial.sql` |
...
| `sp_Get_MonthlyInventoryReportBatchWise_SAP` | @fromDate DATE = '2026-07-31', @toDate DATE = NULL, @CiD NVARCHAR, @ProductCode NVARCHAR = NULL | S |  |  |  |  | SAP_API_Data, Sap_Stock13thSepOpening, tblChalanDetail, tblChalanInfo, tblCompanyUnit, tblConvQty (+2 more) | `spec/database/procs/sp_Get_MonthlyInventoryReportBatchWise_SAP.sql` |
...
| `sp_RptBussinessSummary_DayWise` *(added 2026-08-15)* | @ComUnitId INT, @Year INT, @Month INT | S |  |  |  |  | SAP_API_Data, tblCompanyUnit, tblInvoice, tblInvoiceDetail, tblInvoiceDetailReturn, tblOrder | `spec/database/procs/sp_RptBussinessSummary_DayWise.sql` |
| `sp_RPT_DuplicateCustomerCode` *(added 2026-08-15)* | (none) | S |  |  |  |  | tblCustMaster | `spec/database/procs/sp_RPT_DuplicateCustomerCode.sql` |
| `sp_RPT_DuplicateInvoiceNo` *(added 2026-08-15)* | (none) | S |  |  |  |  | tblInvoice | `spec/database/procs/sp_RPT_DuplicateInvoiceNo.sql` |
| `sp_RPT_DuplicateOrderCode` *(added 2026-08-15)* | (none) | S |  |  |  |  | tblOrder | `spec/database/procs/sp_RPT_DuplicateOrderCode.sql` |
| `sp_RPT_DuplicateOrderNoInInvoice` *(added 2026-08-15)* | (none) | S |  |  |  |  | tblInvoice | `spec/database/procs/sp_RPT_DuplicateOrderNoInInvoice.sql` |
| `sp_RPT_InvoicePaymentVatTpMismatch` *(added 2026-08-15)* | (none) | S |  |  |  |  | tblCustPayDetail, tblInvoice, tblInvoiceDetail | `spec/database/procs/sp_RPT_InvoicePaymentVatTpMismatch.sql` |
...
### SInventory (185 procs, `SInventory_DAL`/`ChartDAL`/`SInventory_UI`)
...
- Security: 105 of 114 `SInventory_DAL` C# files build raw inline SQL via string literals executed through `ClsCommonInternalDAL.DataContainerDataTable`, bypassing stored procs entirely; 47 of 185 procs in this module build dynamic SQL from a `@param` fragment that 44 `SInventory_UI` pages construct via string concatenation of dropdown/hidden-field postback values.
- **`sp_RptBussinessSummary_DayWise` and its 8 siblings (added 2026-08-15)** — back the new `SInventory_UI/RptBussinessSummary_DayWise.aspx` page (see `spec/reports.md`). `sp_RptBussinessSummary_DayWise` reuses `sp_RPT_MIS_BusinessSummary`'s `@Type='SC'` Net Sales formula (`JustSalesAmtTP`/`JustSalesGrossAmt` collapse to `SalesAmtTP`/`SalesGrossAmt` minus 1st/2nd return, the "Old Return" term cancels out algebraically) re-aggregated per calendar day via a recursive `Days` CTE, plus a per-day `SAPsendAmount` (SAP delivery confirmations minus SAP returns minus SAP expiry returns, all three cross-database `SAP_API_Data..tbl_*`; the expiry-return branch joins `tblCompanyUnit` via `SAP_Code` rather than the `Customer_Code` the other two use). `sp_RPT_NegativeClosingStock`/`_DCWise` are a single-DC/all-DC-grouped pair over the same underlying "closing stock went negative" computation (the DCWise variant threads `ComUnitId` through every join instead of filtering by it, cross-verified row-for-row against the single-DC proc). The 4 `sp_RPT_Duplicate*` procs and `sp_RPT_InvoicePaymentVatTpMismatch` are straightforward data-integrity checks (duplicate OrderCode/OrderNo/InvoiceNo/CustomerCode, invoice payment VAT/TP posted incorrectly). `sp_RPT_TourPlanMissingSerial`/`sp_FixTourPlanMissingSerial` are a check/fix pair — the latter is the only one of the 9 that writes (`UPDATE tbl_TourPlanInfo SET SerialNo=1`), triggered by a confirm-gated button on the page.
- **`sp_ProductWiseBusinessSummaryMISReportByParam` (fixed 2026-08-20)** - backs `SInventory_UI/TotalSummaryNew.aspx` ("Product wise Sales Summary"), the per-product counterpart of `sp_RPT_MIS_BusinessSummary`'s per-`ComUnitId` `SInventory_UI/RptBussinessSummary_Loading.aspx`. None of its four `@Type` branches (`SC`/`Zone`/`Area`/`Territory`) joined the 2nd return (`tblInvoiceDetailReturn` keyed off `tblInvoice.SndReturnPaymentDate` - goods returned *after* the payment/collection return was posted), which `sp_RPT_MIS_BusinessSummary` does subtract via its `tbl2Rtn` join. Net effect: `GrossSales`/`SumofNetSalesAmount`/`DelTpVat` overstated and `GrossRetuen`/`SumofNetReturnAmount`/`DelReTpVat` understated by the whole 2nd-return amount - measured at exactly 1,026,327.99 (106 invoices) over 01-Jul-2025..30-Jun-2026 Distribution Center wise, with both procs' raw sales base identical, which is what ruled out a data cause. Each branch now carries a `ProductCode`-grouped `tbl2Rtn` join (the `Area`/`Territory` ones also join `tblOrder` for the usual `COALESCE(NULLIF(@Area,0), ...)` scope predicates) and all six shared columns now reconcile to the cent against `sp_RPT_MIS_BusinessSummary`. Note the proc's own `---return 2` comment labels the `RejectionSts='Old'` join (`tblOldRtn`), which is a *different* return and was never a substitute. Still open in this proc: `RetQty` uses `count(...)` where it means `sum(...)`, the `Zone` branch is byte-identical to `SC` and ignores `@ZonId`, and `where C.Productgroupid=1` silently drops any non-group-1 product. The same missing-`tbl2Rtn` defect remains in `sp_ProductWiseBusinessSummaryMISReport`, `sp_ProductWiseBranchwiseBusinessSummaryMISReport`, `sp_RPT_MIS_ProductWiseSalesReport` and `sp_RPT_MIS_RptMIOWiseReceiveableReport`. Full analysis: `docs/BusinessSummary_2ndReturn_Fix.md`; apply script: `update_sps_ProductWiseBusinessSummary.sql`.

### DoctorModule (512 procs combined: `DoctorModule_DAL` + `DoctorVisit_DAL`/`DoctorMaster_DAL`/`DoctorInfo_DAL`/`Doctor_Monitoring_DAL`)
