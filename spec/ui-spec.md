...
ASP.NET Web Forms (`.aspx`/`.aspx.cs` code-behind pairs, `.ascx` user controls), ~700 pages across ~25 feature folders — see [`modules.md`](modules.md) for the full inventory, including the live menu tree pulled directly from `tblMainMenuNew` (326 rows, 33 top-level categories) — that's the ground truth for what pages a logged-in user actually navigates to; this document covers the UI *mechanics*, not the navigation structure. `Solution.Web` is an AspNetCompiler Website project, not MVC/Razor.
...
**Confirmed absent, not just "largely absent."** A full sweep of all 700 `.aspx` files (see
[`validation-rules.md`](validation-rules.md) §1-2) found only 4 files using an ASP.NET validator
control at all, and the one global client-side library loaded on every page (jQuery Validate, via
`MasterPage.master`) targets a form ID (`#signupForm`) that doesn't exist anywhere in the
