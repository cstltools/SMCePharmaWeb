...
ASP.NET Web Forms (`.aspx`/`.aspx.cs` code-behind pairs, `.ascx` user controls), 701 pages across ~25 feature folders — see [`modules.md`](modules.md) for the full inventory, including the live menu tree pulled directly from `tblMainMenuNew` (327 rows, 34 top-level categories) — that's the ground truth for what pages a logged-in user actually navigates to; this document covers the UI *mechanics*, not the navigation structure. `Solution.Web` is an AspNetCompiler Website project, not MVC/Razor.
