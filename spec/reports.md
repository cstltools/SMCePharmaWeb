...
| MIO-wise Business Summary | Business summary rolled up per MIO (rep) | Date range, MIO, **Only Active** checkbox (added 2026-09-13 — filters to `emp.EmployeeStatus = 'Active'` via new `@OnlyActive bit = 0` param, default off/unfiltered) | Grid | `sp_RPT_MIOWiseBusinessSummary` |
...
| Day Wise Net Sales Report (`SInventory_UI/RptBussinessSummary_DayWise.aspx`, added 2026-08-15) | Grew from a single day-wise Net Sales grid into a combined report/data-audit page — 6 sections on one screen: **(1)** Day Wise Net Sales — daily `JustSalesAmtTP`/`JustSalesGrossAmt` (same Net Sales formula as `sp_RPT_MIS_BusinessSummary`'s `@Type='SC'` branch, re-aggregated per calendar day) plus a per-day SAP Send Amount (SAP delivery confirmations minus SAP returns minus SAP expiry returns, the last joined to `tblCompanyUnit` via `SAP_Code` rather than the `Customer_Code` the other two SAP joins use), with rows where Gross Amount ≠ SAP Send Amount highlighted red — the `InvoiceDate` column was dropped from this grid 2026-09-13; **(2)** Negative Closing Stock — per-DC list plus an always-on, all-DC-wise chart of products whose closing stock has gone negative; **(3)** Monthly Inventory Report (Batch Wise) — reuses the existing `MonthlyInventoryReportBatchWise.aspx` proc/BLL call, plus an async all-DC negative-closing chart (loaded via a `[WebMethod]` after page render and parallelized across DCs, since the underlying proc takes ~12s per DC sequentially); **(4)** Duplicate Order Number / Duplicate Order No in Invoice / Duplicate Invoice No / Duplicate Customer Code — four data-integrity checks, each with a chart + grid, normally-empty by design; **(5)** Invoice Payment VAT/TP Mismatch — invoices where posted payment TP/VAT don't reconcile against invoice detail, unioned from two checks (payment TP > invoice TP, or payment VAT > invoice VAT — `UNION ALL`, so an invoice failing both appears twice), with both a TP Difference and VAT Difference column; **(6)** Tour Plan Missing Serial (Today) — field staff with a tour plan today but no Serial No 1 row, with a confirm-gated "Fix Missing Serial" button that runs the write proc and re-renders the list — **sections (2), (4), (5) and (6) (`RenderNegativeClosingChart`, `RenderDuplicateOrderCode`, the four `RenderDuplicateChart` calls, `RenderVatTpMismatch`, `RenderTourPlanMissingSerial`) are commented out of `Page_Load` as of 2026-09-13**, so only sections (1) and (3) currently render; the markup/code for the disabled sections is left in place, not deleted | Distribution Center + Year + Month (section 1); Distribution Center + From Date, fixed default 31-Jul-2026 (section 2); Distribution Center + From/To Date, fixed minimum 31-Jul-2026 (section 3); none (sections 4-6 are whole-table/whole-day checks) | GridView + Highcharts + Excel export (Mechanism B); section 3's chart loads async via AJAX | `TotalSummaryBLL` → `sp_RptBussinessSummary_DayWise`, `sp_RPT_NegativeClosingStock`/`_DCWise`, `sp_Get_MonthlyInventoryReportBatchWise` (shared with `MonthlyInventoryReportBatchWise.aspx`), `sp_RPT_DuplicateOrderCode`/`DuplicateOrderNoInInvoice`/`DuplicateInvoiceNo`/`DuplicateCustomerCode`, `sp_RPT_InvoicePaymentVatTpMismatch`, `sp_RPT_TourPlanMissingSerial` + `sp_FixTourPlanMissingSerial` (the only write among these 11 procs — corrected this revision from a previous miscount of 9) — full proc source in `spec/database/procs/`, menu registration in `spec/database/menu/RptBussinessSummary_DayWise_menu.sql` |
| DA Expense Bill — Day Wise (`SInventory_RPTVIEW/DAExpenseDayWiseSummaryViewer.aspx`, added 2026-09-13) | Print-formatted expense bill (Delivery/Market Visit/Tour), one bill section per DA, driven entirely by query-string params (no server controls beyond a print button) — `daIds` (comma-separated, optional — omitted means all), `FromDate`/`ToDate` (`yyyy-MM-dd`, default: current calendar month), `fType=Print` to auto-fire `window.print()` on load. Grid rows are per work-day placeholders (date/place columns filled, time/type/transport left blank for manual fill-in); allowance total lands on the "Ex. HQ Allowance" line only, since the proc returns a single `AllowanceAmount` with no HQ/OS breakdown | `daIds`, `FromDate`, `ToDate` via query string only | Server-rendered HTML (`Literal`), not GridView/Crystal — styled for A4 print (`@page` CSS, one `page-break-after` per DA) | `sp_Get_DAExpenseDayWiseSummary` (`@Mode='DayWise'`), same proc used by the DA Expense Claim pages |
...
| Monthly Inventory Report (Batch Wise) (`SInventory_UI/MonthlyInventoryReportBatchWise.aspx`, added 2026-08-09, re-pointed at the SAP sources 2026-08-21) | Batch-wise opening/received/issued/closing stock movement per product, one row per (product, batch) anchored on `tblDCStore` for the selected DC and filtered to `ProductGroupId = 1`. Opening comes from the `Sap_Stock13thSepOpening` snapshot (divided by `tblConvQty.ConvertionQty`), sales and returns from `SAP_API_Data..tbl_DeliveryConfirmation_Sales` / `..tbl_Return`, CWH vs B2B receives from `tblDCStore` split on `ChalanDetailsId`, and B2B transfers out from `tblChalanDetail`. `Closing_Qty = (Opening + CWH Receive + B2B Receive + Return) - (Sales + B2B Transfer)`, surfaced on the grid behind an info button in the column header. **"Show All Zero Value" checkbox (added 2026-09-13):** unchecked by default, client-side-triggered postback (`AutoPostBack`) that filters out any row where all 7 quantity columns (Opening/Cwh Receive/B2B Rcv/Sales/Return/B2B Transfer/Closing) are zero — filtering happens in C# on the already-fetched `DataTable`, not in the proc | Sales Center (required), Product Code (optional, matches `ProductCode` **or** `SAP_Code`), From Date (**pinned** to 31-Jul-2026 - `Opening_Qty` comes from a dated snapshot with no date column, so a later From Date keeps the same opening but drops the movements before it), To Date (required, >= From Date, defaults to today), Show All Zero Value checkbox | GridView + Excel export (Mechanism B); sticky header via `position: sticky` on `#MainGradeDiv th` | `TotalSummaryBLL.LoadMonthlyInventoryReportBatchWiseSap` -> `sp_Get_MonthlyInventoryReportBatchWise_SAP` (`@fromDate`, `@toDate`, `@CiD`, `@ProductCode`). Kept separate from the older `sp_Get_MonthlyInventoryReportBatchWise`, which `RptBussinessSummary_DayWise.aspx` still uses for its section-3 grid and closing-stock chart. Every join key in these tables is `NVARCHAR(MAX)`, which blocks hash/merge joins - the proc casts them to `NVARCHAR(100)`, taking one DC from 161s to 0.7s. Full notes in `docs/MonthlyInventoryReportBatchWise_SAP_Rewrite.md` |
