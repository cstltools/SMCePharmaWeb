...
CREATE VIEW [dbo].[View_FieldForceSubTerritory]
AS
SELECT        STTR.SubTerritoryId, STTR.SubTerritoryName, STTR.TerritoryId, NSM.EmpMasterCode AS NSMCode, RSM.EmpMasterCode AS RSMCode, ASM.EmpMasterCode AS ASMCode, MIO.EmpMasterCode AS MIOCode
FROM            dbo.tbl_Group AS GP 
LEFT OUTER JOIN  dbo.tblRegion AS RGN ON GP.GroupId = RGN.GroupId 
LEFT OUTER JOIN dbo.tblArea AS ARA ON RGN.RegionId = ARA.RegionId
 LEFT OUTER JOIN dbo.tblTerritory AS TTR ON ARA.AreaId = TTR.AreaId
  LEFT OUTER JOIN dbo.tblSubTerritory AS STTR ON TTR.TerritoryId = STTR.TerritoryId 
 -- LEFT OUTER JOIN   dbo.tblMarket AS MKT ON STTR.SubTerritoryId = MKT.SubTerritoryId
   LEFT OUTER JOIN (SELECT        N.GroupId, EGI.EmpMasterCode, EGI.EmpName
...
WHERE        (GP.IsActive = 1) AND (RGN.IsActive = 1) AND (ARA.IsActive = 1) AND (TTR.IsActive = 1) AND (STTR.IsActive = 1)
 --and SubTerritoryCode='51520'
