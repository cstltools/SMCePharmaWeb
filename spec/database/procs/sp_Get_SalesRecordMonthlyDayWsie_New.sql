...
CREATE
 ...
 PROCEDURE [dbo].[sp_Get_SalesRecordMonthlyDayWsie_New]
	-- Add the parameters for the stored procedure here

	@Month INT,
	@Year INT,
	@param nvarchar(max),@CustomerTypeId INT
...
left join 
(SELECT rg.RegionCode Criteria , ISNULL(SUM(ID.TotalPrice-ID.DiscountAmount) - sum(ISNULL(ID.AdjustmentAmount,0))
...
,0) Amount FROM   dbo.tblRegion rg with (nolock)
  ...
  left JOIN dbo.tblOrder  with (nolock) ON rg.RegionId=dbo.tblOrder.RegionId
   ...
   WHERE  MONTH(InvoiceDate)='''+convert(nvarchar(max),@Month)+''' AND YEAR(InvoiceDate)='''+convert(nvarchar(max),@Year)+'''  '+@param+'   '+@NewParam+'  
