...
--------------------------------------------------
-- PROCEDURE: sp_GET_PaymentInvSP_DA
--------------------------------------------------
CREATE   PROCEDURE [dbo].sp_GET_PaymentInvSP_DA
	-- Add the parameters for the stored procedure here
	@param nvarchar(max)
AS
BEGIN
  SET NOCOUNT ON;
    DECLARE @Q NVARCHAR(MAX)='SELECT ISNULL(payCol.PayableAmount,0)  PayableAmount_DA, ord.TerritoryName_Ord,   (ISNULL(TotalDelivery,0)-isnull(tblinvDetls.PaymentTotalPriceVatAmount,0))  -isnull(TPAmount,0)  
 TP_Pay,
 -isnull(VATAmount,0)  
  Vat_Pay , ord.OrderCode OrderNo,CONVERT(VARCHAR,ord.SubmissionDate,103) as OrderDate, ord.CustomerCode,ord.CustomerName,  ord.MarketId,ord.DistributionRouteId,  ord.CustomerMasterId,  INV.InvoiceId,InvoiceNo,InvoiceDate,PaymentInvoiceNo RtnInvoiceNo,INV.PaymentDate RtnInvoiceDate,   TotalDelivery   TotalDelivery,ISNULL(PP,0) PaymentAmount,  (   ISNULL(TotalDelivery,0)   - ISNULL(PP,0))-ISNULL(payCol.PayableAmount,0) AS   Due,ISNULL(ReturnTotal,0) AdjustableAmount FROM tblInvoice AS INV WITH(NOLOCK)
inner join tblOrder ord with (nolock) on INV.OrderId=ord.OrderId
inner join tblPaymentCollection_appLog payCol with (nolock) on INV.InvoiceId=payCol.InvoiceId
