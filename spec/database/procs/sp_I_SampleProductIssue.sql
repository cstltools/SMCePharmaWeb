...
-- =============================================
CREATE PROCEDURE [dbo].[sp_I_SampleProductIssue] 
	(
		 @OrderId int OUT,	
...
BEGIN
	...
	SELECT @OrderNo ='SMP-'+(@yearText+@monthText+@dateText)+CONVERT(NVARCHAR(MAX),CONVERT(INT,ISNULL(MAX(SUBSTRING(OrderCode,11,6)),10000))+1)  FROM tblSampleIssue 
