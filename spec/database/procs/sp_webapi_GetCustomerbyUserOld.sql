...
create PROCEDURE [dbo].[sp_webapi_GetCustomerbyUserOld]
...
AS
	...
	SET @Q='SELECT distinct  isnull(D.CustomerBsPTag,'''') CustomerBsPTag,  isnull(D.CustomerBsPCode,'''') CustomerBsPCode, ISNULL(tblt.SerialNo,1000000) SerialNo,  D.MarketCode MarketCode, D.CustomerTypeId, D.ProgramTypeId, D.SMCTypeId, 0 as CustomerCheck,format(tbInvo.InvoiceDate,''dd MMMM, yyyy'') Note,  D.GroupId,D.RegionId, D.AreaId, D.TerritoryId, D.SubTerritoryId,D.MarketId,  D.CustomerMasterId ,
                D.CustomerName ,
                D.CustomerCode ,
                D.Address ,
                D.MarketName ,
...
SELECT   @Id=  STUFF( (SELECT CONCAT(',', mm.EmpAreaId , '')   FROM View_Webapi_EmployeeFieldForceInfo mm (NOLOCK)   WHERE EmpInfoId=@empId   FOR XML PATH ('') ),1,1,'')  
	...
	SET @Q=@Q+@Param
	+' ORDER BY ISNULL(tblt.SerialNo,1000000) ASC '
