-- =============================================
-- =============================================
CREATE PROCEDURE [dbo].[sp_AreawiseDailyOpeningClosingStockNational] 
...
SELECT P.ProductCode as ProductCode,P.ProductName as ProductName ,P.PackSize as BaseUnit,  ( ISNULL(vTblOB.Quantity,0)  ) OpeningStock   ,
...
ISNULL(vTblStockReceive.TotalStockReceiveQty,0)+ ( ISNULL(vTblOB.Quantity,0)  ) AS TotalReceived   ,
...
LEFT JOIN (SELECT ProductCode,SUM(StockQty)Quantity FROM dbo.tblDCStore_OpeningBalance  with(nolock) WHERE
...
LEFT JOIN (SELECT ProductCode,SUM(TotalQuantity)TotalStockReceiveQty FROM dbo.tblDCStore  with(nolock)     WHERE ChalanDetailsId IS NOT NULL AND StockRcvDate 
BETWEEN @fromDate and @toDate     GROUP BY ProductCode) vTblChallanReceive ON vTblChallanReceive.ProductCode = P.ProductCode     


LEFT JOIN (SELECT ProductCode,SUM(TotalQuantity)TotalStockReceiveQty FROM dbo.tblDCStore  with(nolock) WHERE StockRcvDate 
...
WHERE  ISGiftProduct=0 AND InvoiceDate between @fromDate and @toDate GROUP BY ProductCode) vTblsales ON vTblsales.ProductCode = P.ProductCode  
...
--ID.DeliveryStatus IN ('Reject','Partial') 
--AND I.UpdateDate BETWEEN @fromDate and @toDate
--AND I.TpGrandTotal>0  GROUP BY ID.ProductCode)tblreturnBonusSubdeport ON tblreturnBonusSubdeport.ProductCode = p.ProductCode 
...
WHERE  ChalanDate between @fromDate and @toDate GROUP BY ProductCode) vTblChallan ON vTblChallan.ProductCode = P.ProductCode    

--IsDeliver='True' and
LEFT JOIN (SELECT ProductCode,(SUM(StockQty)+SUM(DamageQty))Freeze FROM dbo.tblDCStoreFreeze  with(nolock)    WHERE   [StockCondition]='Restricted' and ReceiveDate 
BETWEEN @fromDate and @toDate GROUP BY ProductCode) vTblFreez ON vTblFreez.ProductCode = P.ProductCode   


LEFT JOIN (SELECT ProductCode,(SUM(StockQty)+SUM(DamageQty))Freeze FROM dbo.tblDCStoreFreeze  with(nolock)    WHERE   [StockCondition]='Blocked' and ReceiveDate 
BETWEEN @fromDate and @toDate GROUP BY ProductCode) vTblFreez2 ON vTblFreez2.ProductCode = P.ProductCode   


LEFT JOIN (SELECT sum(StockQty)Closingstock,ProductCode FROM dbo.tblDCStore  with(nolock)    group by ProductCode) currentStock ON currentStock.ProductCode = P.ProductCode 
...
LEFT JOIN (SELECT ProductCode,SUM(Quantity)qty FROM dbo.tblDepotToWHChalanInfo  with(nolock)
            ...
            WHERE   ChalanDate between @fromDate and @toDate GROUP BY ProductCode) vTblChallantoWH ON vTblChallantoWH.ProductCode = P.ProductCode
...
LEFT JOIN (SELECT ProductCode,SUM(Quantity)qty1 FROM dbo.tblSubDepotChalanInfo  with(nolock)
            ...
            WHERE  ChalanDate between @fromDate and @toDate GROUP BY ProductCode) vTblSubdeportTransfer ON vTblSubdeportTransfer.ProductCode = P.ProductCode  
--IsDeliver='True' AND
...
--SELECT P.ProductCode as ProductCode,P.ProductName as ProductName ,P.PackSize as BaseUnit,  ( ISNULL(vTblOB.Quantity,0)  ) OpeningStock   ,
--ISNULL(vTblStoReceive.TotalStockReceiveQty,0) AS ReceiveFromCentralWarehouse   ,ISNULL(vTblChallanReceive.TotalStockReceiveQty,0) AS ReceiveFromAreaOfficeInterTransfer    ,
--ISNULL(vTblStockReceive.TotalStockReceiveQty,0)+ ( ISNULL(vTblOB.Quantity,0)  )+(ISNULL( vTblSubdeportReturn.qty2,0)) AS TotalReceived   ,((ISNULL(vTblsales.Sales,0)-ISNULL(tblD.DelQty,0)) ) AS IssuedToSales   
--,((ISNULL(vTblProductBonus.Sales,0)- ISNULL(tblreturnBonus.DelQty,0))  )AS IssuedToProductBonus   ,
--ISNULL(vTblChallan.Challan,0) AS IssuedToAreaOfficeInterTransfer    ,ISNULL(vTblFreez.Freeze,0) AS IssuedToDamageAndOthers,    ISNULL(vTblFreez2.Freeze,0) AS Blocked, 
   
--((ISNULL(vTblOB.Quantity,0) + ISNULL(vTblStoReceive.TotalStockReceiveQty,0)+ISNULL(vTblChallanReceive.TotalStockReceiveQty,0)+(ISNULL( vTblSubdeportReturn.qty2,0))))
---(((((ISNULL(vTblsales.Sales,0)-ISNULL(tblD.DelQty,0)) ))+ISNULL(vTblChallan.Challan,0))+(ISNULL(vTblFreez.Freeze,0))+(ISNULL( vTblChallantoWH.qty,0))+
--(ISNULL(vTblFreez2.Freeze,0))+(((ISNULL(vTblProductBonus.Sales,0)- ISNULL(tblreturnBonus.DelQty,0))))+ISNULL(vTblSubdeportTransfer.qty1,0))
--as ClosingStock,
...
--LEFT JOIN (SELECT ProductCode,SUM(tblInvoiceDetail.Quantity)Sales 
--FROM dbo.tblInvoiceDetail  with(nolock) INNER JOIN dbo.tblInvoice ON tblInvoiceDetail.InvoiceId=tblInvoice.InvoiceId  
--WHERE  OrderDetailsId<>0 AND InvoiceDate between @fromDate and @toDate GROUP BY ProductCode) vTblsales ON vTblsales.ProductCode = P.ProductCode  
 
 
--LEFT JOIN (SELECT (SUM(ID.Quantity) - SUM(ID.DeliveryQuantity))DelQty,ID.ProductCode, ((SUM(ID.NetAmount)-sum(ID.TotalPriceVatAmount))- SUM(ID.DeliveryNetAmount - 
--ID.DeliveryTotalPriceVatAmount)) AS SumofNetReturnAmount,COUNT(DISTINCT I.DelivaryInvoiceNo) NumberofReturnInvoice,SUM(ID.TotalPriceVatAmount)-sum(DeliveryTotalPriceVatAmount) AS
--TotalPriceVatAmount FROM dbo.tblInvoice I WITH (NOLOCK) INNER JOIN dbo.tblInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId  WHERE
--OrderDetailsId<>0 and
--ID.DeliveryStatus IN ('Reject','Partial') 
...
--LEFT JOIN (SELECT ProductCode,SUM(TotalQuantity)Sales FROM dbo.tblInvoiceDetail  with(nolock) INNER JOIN dbo.tblInvoice ON tblInvoiceDetail.InvoiceId=tblInvoice.InvoiceId 
--WHERE   OrderDetailsId=0 and InvoiceDate between @fromDate and @toDate GROUP BY ProductCode)
--vTblProductBonus ON vTblProductBonus.ProductCode = P.ProductCode  
...
LEFT JOIN (SELECT ProductCode,SUM(StackOutQty)StockOutQty FROM dbo.tblDeStockOutMaster  with(nolock)
            ...
            WHERE  ApprovedDate between @fromDate and @toDate GROUP BY ProductCode) vTblDirectStockOut ON vTblDirectStockOut.ProductCode = P.ProductCode  
