...
create PROCEDURE [dbo].[sp_DeleteArchiveDBInvoiceData]
...
DELETE id
FROM ' + @DbName + N'.dbo.tblInvoiceDetail id
INNER JOIN ' + @DbName + N'.dbo.tblInvoice i
    ON id.InvoiceId = i.InvoiceId
WHERE i.InvoiceDate >= @FromDate
  AND i.InvoiceDate < DATEADD(DAY, 1, @ToDate);

DELETE i
FROM ' + @DbName + N'.dbo.tblInvoice i
WHERE i.InvoiceDate >= @FromDate
  AND i.InvoiceDate < DATEADD(DAY, 1, @ToDate);';
