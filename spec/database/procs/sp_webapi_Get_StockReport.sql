CREATE PROCEDURE [dbo].[sp_webapi_Get_StockReport]
...
AS
	...
	END
        ...
        FROM    tblDCStore A
                INNER JOIN dbo.tblCompanyUnit B ON B.ComUnitId = A.ComUnitId
				INNER JOIN
                      dbo.tblProduct ON A.ProductCode = dbo.tblProduct.ProductCode 
 
        
				inner join tblRouteInformationMaster dcMas   with (nolock) on A.ComUnitId=dcMas.DCId

				inner join tblRouteInformationMarketDetail dcDtl  with (nolock) on dcMas.RouteInformationMasterId=dcDtl.RouteInformationMasterId

INNER JOIN (SELECT DISTINCT
                     MarketId, MIOEmpInfoId, ASMEmpInfoId,RSMEmpInfoId,NSMEmpInfoId FROM dbo.View_CustomerMaster) AS tblt ON tblt.MarketId = dcDtl.MarketId
        WHERE   A.StockQty > 0   and  tblProduct.ProductGroupId=1 '+@param+' 
        GROUP BY A.ProductCode ,
                A.ProductName
        ORDER BY A.ProductName'
