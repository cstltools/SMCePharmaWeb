...
CREATE PROCEDURE [dbo].[sp_webapi_SaveTourPlanAppLog]  --[dbo].[sp_webapi_SaveOrderMaster] 'shaon20','20245','11/25/2020 5:04:16 PM'
    @TourPlanApprovalId INT ,
...
AS
	...
	SELECT @MapRoleTypeId=RoleTypeId FROM dbo.tbl_TourPlanMaster
	LEFT JOIN dbo.tblEmpGeneralInfo ON tblEmpGeneralInfo.EmpInfoId = tbl_TourPlanMaster.EmpInfoId
	...
	WHERE dbo.tbl_TourPlanMaster.TPMaster=@TableId

	DECLARE @CurrentRoleTypeId INT=@MapRoleTypeId
	DECLARE @CurrentStep INT=1
	SELECT TOP 1 @CurrentRoleTypeId=ToRoleTypeId FROM dbo.tblTourPlanApprovalLog WHERE TableId=@TableId ORDER BY TourPlanApprovalId DESC
	...
	IF(@Status='Accepted')
	BEGIN
	    UPDATE dbo.tbl_TourPlanMaster SET ApprovalStatus='2' WHERE TPMaster=@TableId
	END
	IF(@Status='Verified')
	BEGIN
	    UPDATE dbo.tbl_TourPlanMaster SET ApprovalStatus='1' WHERE TPMaster=@TableId
	END

	IF(@Status='Rejected')
	BEGIN
	 
	    UPDATE dbo.tbl_TourPlanMaster SET ApprovalStatus='3',ApprovalRemarks=@Comments WHERE TPMaster=@TableId
			    --UPDATE dbo.tbl_TourPlanMaster SET ApprovalStatus='3', IsFinalSubmit=1,ApprovalRemarks=@Comments  WHERE TPMaster=@TableId
	END
	...
	IF(@Status='Rejected')
	BEGIN
	    DELETE FROM dbo.tblTourPlanApprovalLog WHERE TableId=@TableId AND Step<>'1'
		INSERT INTO dbo.tblTourPlanApprovalVoidLog
    (
   ...
   SELECT 
          ...
          RoleTypeId,
          ToRoleTypeId
           FROM dbo.tblTourPlanApprovalLog WHERE TableId=@TableId AND Step<>'1'
	END
	ELSE
    BEGIN

    INSERT INTO dbo.tblTourPlanApprovalLog
    (
    ...
    (   
       ...
       @EntryTimeApp,
		...
		SELECT @TourPlanApprovalId=SCOPE_IDENTITY()
