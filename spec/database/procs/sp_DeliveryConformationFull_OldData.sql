...
-- =============================================
CREATE PROCEDURE [dbo].[sp_DeliveryConformationFull_OldData] ---- sp_DeliveryConformationFull 'INV-BD32-00001394','Test','29-Oct-2018'
...
BEGIN
	...
	IF EXISTS (SELECT * FROM SalesDisDB_SMC..tblInvoice WHERE InvoiceNo=@InvoiceNo AND DeliveryInvoiceStatus IS NULL)
	BEGIN
	   ...
	   DECLARE @TotalSpecialAmount decimal(18,2)
	    ...
	    SELECT @InvoiceId=InvoiceId ,
			   ...
			   @TotalSpecialAmount=TotalSpecialAmount
                FROM SalesDisDB_SMC..tblInvoice WHERE InvoiceNo=@InvoiceNo
       ...
       DECLARE MY_data CURSOR FOR
				...
				SELECT D.InvoiceDetailId ,
					   ...
					   D.DCStoreId ,
					   D.SpecialAmount 
						FROM SalesDisDB_SMC..tblInvoiceDetail D INNER JOIN
				SalesDisDB_SMC..tblInvoice I ON I.InvoiceId = D.InvoiceId WHERE D.InvoiceId=@InvoiceId
				...
				UPDATE SalesDisDB_SMC..tblInvoiceDetail SET DeliveryQuantity=@Quantity ,
			...
			UPDATE SalesDisDB_SMC..tblInvoice SET DeliveryTpTotal=@TpTotal,DeliveryTpDiscount=@TpDiscount,DeliveryTpVat=@TpVat,
