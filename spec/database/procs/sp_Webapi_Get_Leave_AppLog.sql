...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Webapi_Get_Leave_AppLog]
...
AS
	...
	BEGIN
	   SET @params=@params+' and Employee_LeaveApplications.ApprovalStatus in ('+@AppStatus+')'
  ...
  --      IF(@FromDt IS NOT NULL AND @ToDt IS NULL)
		--BEGIN
		--    SET @params=@params+ ' AND CONVERT(DATE,Employee_LeaveApplications.EntryDate)='''+CAST(CONVERT(DATE,@FromDt) AS NVARCHAR(max))+''''
		--END
		IF(@FromDt IS NOT NULL AND @ToDt IS NOT NULL)
		BEGIN
		    SET @params=@params+ ' AND CONVERT(DATE,Employee_LeaveApplications.EntryDate) between '''+CAST(CONVERT(DATE,@FromDt) AS NVARCHAR(max))+''' AND '''+ CAST(CONVERT(DATE,@ToDt) AS NVARCHAR(max))+''' '
