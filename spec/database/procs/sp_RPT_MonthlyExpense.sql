...
CREATE PROCEDURE [dbo].[sp_RPT_MonthlyExpense]
...
BEGIN
 ...
 Select distinct    ''''  TerritoryCode , PM.EmpName,  PM.EmpInfoId,PM.EmpMasterCode,   usRT.RoleType, usR.RoleName,  CONVERT(NVARCHAR(50),PM.DateOfBirth,106)AS EmpDateOfBirth,ISNULL(tblDA.DAAmount ,0) DAAmount,ISNULL(tblMilag.MilageExpense ,0) MilageExpense,ISNULL(tblExpense.ExpenseAmount ,0) ExpenseAmount, case when (case when  month(PM.JoiningDate)='''+@Month+''' and year(PM.JoiningDate)='''+@Year+''' then  (isnull(tblall.MonthlyAllowance,0)/DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JoiningDate),0))))*((DATEDIFF(d, CONVERT(Date,PM.JoiningDate),CONVERT(Date, EOMONTH(PM.JoiningDate))))+1)   when  month(PM.JobleftDate)='''+@Month+''' and year(PM.JobleftDate)='''+@Year+''' then  nullif(cast( isnull(tblall.MonthlyAllowance,0)/ DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JobleftDate),0)))  as decimal(18,2))*   CAST((DATEDIFF(DAY, (SELECT DATEADD(m, DATEDIFF(m, 0, PM.JobleftDate), 0)) , PM.JobleftDate))+1  as decimal(18,2)) ,0)   else isnull (tblall.MonthlyAllowance,0) end) >= isnull(tblall.MonthlyAllowance,0) then isnull(tblall.MonthlyAllowance,0) else 
 
   (case when  month(PM.JoiningDate)='''+@Month+''' and year(PM.JoiningDate)='''+@Year+''' then  (isnull(tblall.MonthlyAllowance,0)/DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JoiningDate),0))))*((DATEDIFF(d, CONVERT(Date,PM.JoiningDate),CONVERT(Date, EOMONTH(PM.JoiningDate))))+1)   when  month(PM.JobleftDate)='''+@Month+''' and year(PM.JobleftDate)='''+@Year+''' then  nullif(cast( isnull(tblall.MonthlyAllowance,0)/ DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JobleftDate),0)))  as decimal(18,2))*   CAST((DATEDIFF(DAY, (SELECT DATEADD(m, DATEDIFF(m, 0, PM.JobleftDate), 0)) , PM.JobleftDate))+1  as decimal(18,2)) ,0)   else isnull (tblall.MonthlyAllowance,0) end) end  MonthlyAllowance, ISNULL(tblDA.DAAmount ,0) +ISNULL(tblMilag.MilageExpense ,0) +ISNULL(tblExpense.ExpenseAmount ,0) + (case when (case when  month(PM.JoiningDate)='''+@Month+''' and year(PM.JoiningDate)='''+@Year+''' then  (isnull(tblall.MonthlyAllowance,0)/DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JoiningDate),0))))*((DATEDIFF(d, CONVERT(Date,PM.JoiningDate),CONVERT(Date, EOMONTH(PM.JoiningDate))))+1)   when  month(PM.JobleftDate)='''+@Month+''' and year(PM.JobleftDate)='''+@Year+''' then  nullif(cast( isnull(tblall.MonthlyAllowance,0)/ DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JobleftDate),0)))  as decimal(18,2))*   CAST((DATEDIFF(DAY, (SELECT DATEADD(m, DATEDIFF(m, 0, PM.JobleftDate), 0)) , PM.JobleftDate))+1  as decimal(18,2)) ,0)   else isnull (tblall.MonthlyAllowance,0) end) >= isnull(tblall.MonthlyAllowance,0) then isnull(tblall.MonthlyAllowance,0) else 
 
   (case when  month(PM.JoiningDate)='''+@Month+''' and year(PM.JoiningDate)='''+@Year+''' then  (isnull(tblall.MonthlyAllowance,0)/DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JoiningDate),0))))*((DATEDIFF(d, CONVERT(Date,PM.JoiningDate),CONVERT(Date, EOMONTH(PM.JoiningDate))))+1)   when  month(PM.JobleftDate)='''+@Month+''' and year(PM.JobleftDate)='''+@Year+''' then  nullif(cast( isnull(tblall.MonthlyAllowance,0)/ DAY(DATEADD(DD,-1,DATEADD(MM,DATEDIFF(MM,-1,PM.JobleftDate),0)))  as decimal(18,2))*   CAST((DATEDIFF(DAY, (SELECT DATEADD(m, DATEDIFF(m, 0, PM.JobleftDate), 0)) , PM.JobleftDate))+1  as decimal(18,2)) ,0)   else isnull (tblall.MonthlyAllowance,0) end) end ) TotalExp from tblEmpGeneralInfo PM with (nolock)
 ...
 where DAMas.TadaID is not null  and  DAMas.ApprovalStatus=''2'' GROUP BY  DAMas.EmpInfoId,  Month(DAMas.TadaDate) ,Year(DAMas.TadaDate)  )tblDA    ON  tblDA.EmpInfoId= PM.EmpInfoId   and TadaDateMonth='+@Month+' and TadaDateYear='+@Year+'
...
LEFT JOIN (SELECT  EMas.EmpInfoId, SUM(ISNULL(EMas.Amount,0)  )   ExpenseAmount, Month(EMas.ExpenseDate) ExpenseDateMont  ,Year(EMas.ExpenseDate)  ExpenseDateYear  FROM dbo.tbl_ExpenseClaim EMas  WITH (NOLOCK)
 ...
 where  EMas.ExpenseClaimID is not null    and  EMas.ApprovalStatus=''2''   GROUP BY  EMas.EmpInfoId,Month(EMas.ExpenseDate)    ,Year(EMas.ExpenseDate))tblExpense    ON  tblExpense.EmpInfoId= PM.EmpInfoId  and ExpenseDateMont='+@Month+' and ExpenseDateYear='+@Year+'
 ...
 where PM.EmpInfoId is not null
   ...
   ' +@Parm +' 	) tblll where tblll.TotalExp>0 and tblll.MonthlyAllowance<>tblll.TotalExp '
