...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Get_TTargetAChivementReport]
	-- Add the parameters for the stored procedure here 

	   @_Month nvarchar(max)=null,
                 @_Year nvarchar(max)=null,
                 @FromDate nvarchar(max)=null,
                 @ToDate nvarchar(max)=null ,
				 @_Zone nvarchar(max)=null ,
				 @_Area nvarchar(max)=null ,
				 @_Tr nvarchar(max)=null 
...
WHERE  tm.TerritoryId IS NOT NULL  and   ((rg.RegionId= COALESCE( NULLIF(@_Zone , 0) ,rg.RegionId)) and ar.AreaId= COALESCE( NULLIF(@_Area , 0) ,ar.AreaId) and tr.TerritoryId= COALESCE( NULLIF(@_Tr , 0) ,tr.TerritoryId))  
