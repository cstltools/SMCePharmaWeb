CREATE PROCEDURE [dbo].[sp_Webapi_FinalSubmitSend_TourPlan]
...
BEGIN
			...
			SELECT @MasterId=TPMaster FROM dbo.tbl_TourPlanMaster WHERE EmpInfoId = @empId AND MonthValue = @month AND YearValue = @year
...
EXECUTE dbo.sp_webapi_SaveTourPlanAppLog @TourPlanApprovalId = 0,                         -- int
                                 @Date = @EDate,           -- datetime
                                 @FromEmpId = @empId,                          -- int
                                 @ToEmpId = 0,                            -- int
                                 @TableId = @Id,                            -- int
