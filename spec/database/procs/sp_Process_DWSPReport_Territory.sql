...
CREATE PROCEDURE [dbo].[sp_Process_DWSPReport_Territory] --exec sp_GetHourlySweingOutput '20-sep-2020' 
...
FOR
  ...
  select distinct tr.TerritoryId,  Emp.EmpMasterCode, tr.TerritoryCode ,ISNULL(Emp.EmpName,'Vaccant') EmpName,  dgs.DesigName , ur.RoleName ,0 TargetSetof  
...
where tr.IsActive=1     and ((rg.RegionId= COALESCE( NULLIF(@RegionId , 0) ,rg.RegionId ))  and (ar.AreaId= COALESCE( NULLIF(@AreaId , 0) ,ar.AreaId )) and (tr.TerritoryId= COALESCE( NULLIF(@TrId , 0) ,tr.TerritoryId )) )

--union all


--  select distinct tr.TerritoryId,  EmpAm.EmpMasterCode,ar.AreaCode  TerritoryCode ,ISNULL(EmpAm.EmpName,'Vaccant') EmpName,  dgsAm.DesigName , urAm.RoleName ,0 TargetSetof  
--   from tblTerritory tr  with (nolock)
