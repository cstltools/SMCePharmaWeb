CREATE PROCEDURE [dbo].[sp_Get_TourPlanApp]
...
AS
	...
	DECLARE @params NVARCHAR(max)=' mas.TPMaster  is not null '
	IF(@AppStatus IS NOT NULL)
	BEGIN
	    SET @params=' tblTourPlanApprovalLog.Status IN ('''+@AppStatus+''')'
		...
		BEGIN
		    SET @params=@params+ ' AND tblTourPlanApprovalLog.EntryDateApp between '''+@FromDt+''' AND '''+@ToDt+''' '
		END
	END
	ELSE
    BEGIN
	...
	SET @Q='
		SELECT distinct mas.FinalSubmitRemarks, DateName( month , DateAdd( month , mas.MonthValue , -1 ))  MonthValue,  toRole.RoleType WaitingForRole, usr.RoleName, dgs.DesigName,  case when mas.ApprovalStatus=''0'' then ''Pending''  when mas.ApprovalStatus=''1'' then ''Verified'' when mas.ApprovalStatus=''2'' then ''Approved'' when mas.ApprovalStatus=''3'' then ''Disapproveed''  else mas.ApprovalStatus end ApprovalStatusWeb,   emp.EmpInfoId,mas.TPMaster,
       MonthValue ,
                  YearValue ,
...
0 GroupId,
    ...
    0 TerritoryId,
	   ...
	   FROM dbo.tbl_TourPlanMaster mas  with (nolock)
	     
LEFT JOIN dbo.tblTourPlanApprovalLog  with (nolock) ON dbo.tblTourPlanApprovalLog.TableId=mas.TPMaster
LEFT JOIN (SELECT TableId,MAX(tblTourPlanApprovalLog.TourPlanApprovalId)MaxStep FROM dbo.tblTourPlanApprovalLog   GROUP BY TableId) AS LogMax ON LogMax.TableId=dbo.tblTourPlanApprovalLog.TableId
...
left JOIN dbo.tblRoleType toRole  with (nolock) ON tblTourPlanApprovalLog.ToRoleTypeId = usr.RoleTypeId
...
WHERE '+@params+'  AND tblTourPlanApprovalLog.TourPlanApprovalId=LogMax.MaxStep '+@param
