CREATE PROCEDURE [dbo].[sp_Save_CustomerPropUpdateDetail]
...
SELECT @CustId=CustomerMasterId FROM dbo.tblCustMaster WHERE CustomerCode=@CustCode
SELECT @ProvId=ProgramTypeId FROM dbo.tblProgramType WHERE PrgmTypeCode=@ProviderType
SELECT @TypeId=CustomerTypeId FROM dbo.tblCustomerType WHERE CustTypeCode=@CustTypeCode
...
SELECT @SMCTypeId=SMCTypeId FROM dbo.tblSMCType WHERE SMCTypeCode=@PharmaPlatformCode
	...
	INSERT INTO dbo.tblCustomerPropUpdateDetail
	(
	    CustPropMasterId,
	    CustCode,
	    CustomerId,
