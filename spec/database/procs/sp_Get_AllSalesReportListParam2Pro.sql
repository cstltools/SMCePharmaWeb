create PROCEDURE [dbo].[sp_Get_AllSalesReportListParam2Pro]
...
BEGIN
   ...
   DECLARE @Q NVARCHAR(MAX)=' select sum(GrossValue) GrossValue from ( SELECT  mas.SMCType_Ord,   isnull(ID.AdjustmentAmount,0) AdjustmentAmount, ID.DeliveryTotalPrice GrossValue, ID.DeliveryTotalPriceVatAmount
...
CONVERT(VARCHAR,I.InvoiceDate,103)  InvoiceDate,I.DelivaryInvoiceNo, CONVERT(VARCHAR,I.UpdateDate,103) UpdateDate,ID.ProductCode,ID.ProductName,ID.PackSize,convert(nvarchar(max),dst.BatchNo) BatchNo ,  
 ...
 mas.SubTerritoryCode_Ord+'' : ''+  mas.SubTerritoryName_Ord SubTerritoryName, mas.MarketCode_Ord MarketCode,  mas.MarketName_Ord MarketName,rt.RouteName,ID.ExpDate,ID.DeliveryQuantity as soldQty
...
INNER JOIN SalesDisDB_SMC..tblProduct P ON ID.ProductCode = P.ProductCode 
INNER JOIN SalesDisDB_SMC..tblProductSQ SQ ON P.ProductBrandId = SQ.ProductBrandId 
where ID.DeliveryStatus IN (''Full'',''Partial'') '  +@Parm +')tbl'
