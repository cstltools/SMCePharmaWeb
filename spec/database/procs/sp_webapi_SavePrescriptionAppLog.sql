...
CREATE PROCEDURE [dbo].[sp_webapi_SavePrescriptionAppLog]  --[dbo].[sp_webapi_SaveOrderMaster] 'shaon20','20245','11/25/2020 5:04:16 PM'
    @PrescriptionApprovalId INT ,
...
AS
	...
	SELECT @Countdata=COUNT(*) FROM dbo.tblPrescriptionApprovalLog WHERE FromEmpId=@FromEmpId AND TableId=@TableId
	...
	SELECT @MapRoleTypeId=RoleTypeId FROM dbo.tbl_PrescriptionMaster
	
	LEFT JOIN dbo.tblUser ON tblUser.UserId = tbl_PrescriptionMaster.EntryBy
	LEFT JOIN dbo.tbl_UserRoleInfo ON tbl_UserRoleInfo.UserRoleID = tblUser.UserRoleID
	WHERE dbo.tbl_PrescriptionMaster.PrescriptionId=@TableId

	DECLARE @CurrentRoleTypeId INT=@MapRoleTypeId
	DECLARE @CurrentStep INT=1
	SELECT TOP 1 @CurrentRoleTypeId=ToRoleTypeId FROM dbo.tblPrescriptionApprovalLog WHERE TableId=@TableId ORDER BY PrescriptionApprovalId DESC
	...
	IF(@Status='Accepted')
	BEGIN
	    UPDATE dbo.tbl_PrescriptionMaster SET ApprovalStatus='2' WHERE PrescriptionId=@TableId
	END
	IF(@Status='Verified')
	BEGIN
	    UPDATE dbo.tbl_PrescriptionMaster SET ApprovalStatus='1' WHERE PrescriptionId=@TableId
	END


		IF(@Status='Rejected')
	BEGIN
	    UPDATE dbo.tbl_PrescriptionMaster SET ApprovalStatus='3' WHERE PrescriptionId=@TableId
	END

	--DECLARE @RoleTypeIds INT=0

	--SELECT TOP 1 @RoleTypeIds=ToRoleTypeId FROM dbo.tblPrescriptionApprovalLog_New WHERE TableId=@TableId ORDER BY Step DESC
    ...
    INSERT INTO dbo.tblPrescriptionApprovalLog
    (
    ...
    (   
       ...
       @EntryTimeApp,
		...
		SELECT @PrescriptionApprovalId=SCOPE_IDENTITY()
