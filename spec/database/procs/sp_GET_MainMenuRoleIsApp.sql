create PROCEDURE [dbo].[sp_GET_MainMenuRoleIsApp]
	-- Add the parameters for the stored procedure here
  @RoleId INT,@TypeId bit
...
AS
	...
	WHERE M.URL<>'#' and M.SL not in (select SL from tblMenuRole where RoleId=@RoleId) and M.IsApp=@TypeId
	...
	LEFT JOIN dbo.tblMainMenuNew P ON M.ParantId=P.SL
	WHERE RoleId=@RoleId and M.IsApp in(@TypeId)) AS tblt
	GROUP BY tblt.SL,
           tblt.ManuName,
           tblt.URL,
           tblt.ParantId,
