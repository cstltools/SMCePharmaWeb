CREATE PROCEDURE [dbo].[sp_Webapi_Get_TourPlanInfo_Vthree] -- sp_Webapi_Get_TourPlanInfo 2,2021,0
        ...
        details = STUFF(
            (
                SELECT ', ' + CAST(InnerA.SerialNo AS VARCHAR(10)) + ', ' +
                  ...
                  isnull(  case when InnerA.IsMarketVisit=1 then 'Market Visit' else 'Other Visit' end+ ' ('+case when InnerA.IsMorning=1 then 'Morning' else 'Evening' end+')' + ', ','') +
                  isnull( InnerE.StationTypeName + ', ','') +
               -- isnull( InnerE.StationTypeName + ', ','') +
                    InnerA.MarketCode_TP + ' : ' + InnerA.MarketName + ' [' + InnerE.StationTypeName + ']' + ' (Time:' + InnerA.Starttime + ')' + CHAR(13) + CHAR(10)
            ...
            --    LEFT JOIN dbo.tbl_TourPlanType AS InnerE  with(nolock) ON InnerE.TourTypeId = InnerA.TourTypeId
					LEFT JOIN dbo.tblStationType InnerE  with (nolock)  ON InnerE.StationTypeId = InnerA.TourTypeId
