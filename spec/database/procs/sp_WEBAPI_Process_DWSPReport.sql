...
CREATE PROCEDURE [dbo].[sp_WEBAPI_Process_DWSPReport] --exec sp_GetHourlySweingOutput '20-sep-2020' 
...
select   
  ...
  CAST(CONVERT (decimal(18,2),ISNULL( ( ISNULL([1Week],0)/[Total])* 100,0)) as nvarchar(max))+'%' as FirstWeek_Percent ,  
  
  CAST( ISNULL([2Week],0)  as nvarchar(max)) SecondWeek, 
    CAST(CONVERT (decimal(18,2), ISNULL( ( ISNULL([2Week],0)/[Total])* 100,0)) as nvarchar(max))+'%' as SecondWeek_Percent,
	
	  CAST(ISNULL([3Week],0) as nvarchar(max)) ThirdWeek,    CAST(CONVERT (decimal(18,2), ISNULL(( ISNULL([3Week],0)/[Total])* 100,0)) as nvarchar(max))+'%' as ThirdWeek_Percent, CAST(CONVERT (decimal(18,2),ISNULL([4Week],0)) as nvarchar(max)) RestDay,  CAST(CONVERT (decimal(18,2),ISNULL(( ISNULL([4Week],0)/[Total])* 100, 0)) as nvarchar(max))+'%' as RestDay_Percent,
