...
CREATE PROCEDURE [dbo].[sp_Get_SAP_DICStockReceivePendingData]
...
BEGIN
		...
		WHERE to_plant_code IS NOT NULL AND( (ISNULL(IsReceived,0) = 0 AND is_from_wharehouse = 1) OR (ISNULL(IsReceived,0) = 0 AND from_plant_code IS NOT NULL)) and M.is_from_wharehouse=1 and UNT.ComUnitId=@Parm
		end
		...
		FROM SAP_API_Data..tblSAP_StockMovementDetail AS D
		GROUP BY StockMovementMasterId) AS D ON M.StockMovementMasterId = D.StockMovementMasterId
		WHERE to_plant_code IS NOT NULL AND( (ISNULL(IsReceived,0) = 0 AND is_from_wharehouse = 1) OR (ISNULL(IsReceived,0) = 0 AND from_plant_code IS NOT NULL)) and M.is_from_wharehouse=1  
