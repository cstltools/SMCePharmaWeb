create PROCEDURE [dbo].[sp_Get_DashboardTopBarData_NSM] 
...
set	@Prvious7Days=getdate()-7
        ...
        WHERE tblOrder.ActionStatus<>'3' and      convert(Date,SubmissionDate) =convert(Date, @currentDate) and   GroupId in( @GroupId)
  ...
  --      WHERE   convert(Date,A.SubmissionDate) = convert(Date,@currentDate)
        ...
        WHERE    A.ActionStatus<>'3' and convert(Date,A.SubmissionDate) = convert(Date,@currentDate)    and   A.GroupId in( @GroupId)


		 SELECT  @totalInvoice =ISNULL(sum(A.TpTotal),0)
        FROM    dbo.tblInvoice A  with (nolock)
		 inner join  dbo.tblOrder ord on A.OrderId=ord.OrderId
        WHERE   convert(Date,A.InvoiceDate) = convert(Date,@currentDate)    and   ord.GroupId in( @GroupId)
...
and ((isnull(custDtl.PaymentAmount,0))) >0  and FinalPaymentNo is not null and  I.PaymentStatus  IN ('Full','Partial') and   mas.GroupId in( @GroupId)
...
WHERE   convert(Date,RejectionDate) =convert(Date,@currentDate)  and   mas.GroupId in( @GroupId)
