...
CREATE
 ...
 PROCEDURE [dbo].[sp_Get_BrandWiseOrderDashboard_new]
...
BEGIN 
    ...
    DECLARE @Q NVARCHAR(MAX)='SELECT CONVERT(Date, InvoiceDate) InvoiceDate, format( Convert(Date,InvoiceDate),''dd-MMM'') Criteria, floor(ISNULL(SUM(ID.TotalPrice-ID.DiscountAmount) - sum(ISNULL(ID.AdjustmentAmount,0))

,0)) Amount FROM dbo.tblOrder with (nolock)
   --INNER JOIN dbo.tblOrderDetail  with (nolock) ON tblOrderDetail.OrderId = tblOrder.OrderId
    INNER JOIN dbo.tblInvoice  with (nolock) ON dbo.tblInvoice.OrderId=dbo.tblOrder.OrderId
  INNER JOIN dbo.tblInvoiceDetail  ID with (nolock) ON dbo.tblInvoice.InvoiceId=ID.InvoiceId
   ...
   where tblOrder.OrderId is not null  '+@param+'   
      group by  CONVERT(Date, InvoiceDate) , format( Convert(Date,InvoiceDate),''dd-MMM'')    ORDER BY CONVERT(Date, InvoiceDate) asc'
