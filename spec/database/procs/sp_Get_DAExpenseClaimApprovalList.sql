...
--------------------------------------------------
-- PROCEDURE: sp_Get_DAExpenseClaimApprovalList
--------------------------------------------------

CREATE   PROCEDURE dbo.sp_Get_DAExpenseClaimApprovalList
    @RouteId INT,
    @daid INT = NULL,
    @FromDate DATE = NULL,
    @ToDate DATE = NULL
...
BEGIN
    ...
    SELECT
        ...
        ISNULL(NULLIF(EC.ApprovalStatus, ''), 'Pending') AS ApprovalStatus,
    ...
    WHERE EXISTS (
              SELECT 1
              FROM dbo.tblRouteInformationDADetail RouteDA WITH (NOLOCK)
              WHERE RouteDA.RouteInformationMasterId = @RouteId
                AND RouteDA.DAId = EC.EmpInfoId
          )
      AND (@daid IS NULL OR EC.EmpInfoId = @daid)
      AND (@FromDate IS NULL OR EC.ExpenseDate >= @FromDate)
      AND (@ToDate IS NULL OR EC.ExpenseDate < @ToDate)
      AND UPPER(LTRIM(RTRIM(ISNULL(EC.ApprovalStatus, '')))) NOT IN ('APPROVED', 'DISAPPROVED')
    ...
    EXEC sys.sp_executesql
        @detailSql,
        N'@RouteId INT, @daid INT, @FromDate DATE, @ToDate DATE',
        @RouteId = @RouteId,
        @daid = @daid,
        @FromDate = @FromDate,
        @ToDate = @ToDate;
