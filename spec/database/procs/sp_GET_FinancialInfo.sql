...
CREATE PROCEDURE [dbo].[sp_GET_FinancialInfo]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT
		 FiscalYearId,CASE  WHEN  Entryemp.EmpName Is Null  THEN  us.UserName 
		...
		DPT.FiscalYearDesc,
		CONVERT(NVARCHAR(50),YearFromDate,106) YearFromDate,
		CONVERT(NVARCHAR(50),YearTodate,106)   YearTodate,
		DPT.IsActive 
		FROM tblFiscalYearInfos AS DPT 
		...
		WHERE DPT.FiscalYearId IS NOT NULL ' + @Parameter
