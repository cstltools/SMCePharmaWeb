CREATE PROCEDURE [dbo].[sp_Get_EmployeeDZSMditID]
...
BEGIN
   ...
   WHERE NSM.IsActive = 1
  --  UNION ALL SELECT DISTINCT EmployeeId FROM tblRSMInfo AS RSM  WHERE RSM.IsActive = 1
     UNION ALL SELECT DISTINCT EmployeeId FROM tblASMInfo AS ASM
   WHERE ASM.IsActive = 1  UNION ALL SELECT DISTINCT EmployeeId FROM tblMIOInfo AS MIO
   WHERE MIO.IsActive = 1 )
   
    UNION ALL SELECT DISTINCT NSM.EmployeeId EmpInfoId,emp.EmpMasterCode + ' : ' + emp.EmpName AS EmployeeName FROM tblRSMInfo AS NSM
    INNER JOIN tblEmpGeneralInfo emp ON NSM.EmployeeId=emp.EmpInfoId

    WHERE NSM.RSMId=@Id) tbl
