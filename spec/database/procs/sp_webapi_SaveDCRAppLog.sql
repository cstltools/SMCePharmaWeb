...
CREATE PROCEDURE [dbo].[sp_webapi_SaveDCRAppLog]  --[dbo].[sp_webapi_SaveOrderMaster] 'shaon20','20245','11/25/2020 5:04:16 PM'
    @DCRApprovalId INT ,
...
AS
	...
	SELECT @Countdata=COUNT(*) FROM dbo.tblDCRApprovalLog WHERE FromEmpId=@FromEmpId AND TableId=@TableId
	...
	SELECT @MapRoleTypeId=RoleTypeId FROM tbl_DCRInfo
	
	LEFT JOIN dbo.tblUser ON tblUser.UserId = tbl_DCRInfo.EntryBy
	LEFT JOIN dbo.tbl_UserRoleInfo ON tbl_UserRoleInfo.UserRoleID = tblUser.UserRoleID
	WHERE dbo.tbl_DCRInfo.DCRId=@TableId

	DECLARE @CurrentRoleTypeId INT=@MapRoleTypeId
	DECLARE @CurrentStep INT=1
	SELECT TOP 1 @CurrentRoleTypeId=ToRoleTypeId FROM dbo.tblDCRApprovalLog WHERE TableId=@TableId ORDER BY DCRApprovalId DESC
	...
	IF(@Status='Accepted')
	BEGIN
	    UPDATE dbo.tbl_DCRInfo SET ApprovalStatus='2' WHERE DCRId=@TableId
	END
	IF(@Status='Verified')
	BEGIN
	    UPDATE dbo.tbl_DCRInfo SET ApprovalStatus='1' WHERE DCRId=@TableId
	END
	IF(@Status='Rejected')
	BEGIN
	
	DECLARE @GWPromoQtyId INT=0
	DECLARE @qty DECIMAL(18,2)
	--SELECT * FROM dbo.tbl_DcrDetails WHERE DcrId=@
	DECLARE @Main CURSOR
     SET @Main = CURSOR FAST_FORWARD
     FOR

	 SELECT GWPromoQtyId,ProductQty FROM tbl_DcrDetails WHERE DcrId=@TableId

	 OPEN @Main
     FETCH NEXT FROM @Main
     INTO @GWPromoQtyId,@qty
	 WHILE @@FETCH_STATUS=0
     BEGIN

	 UPDATE dbo.tblGroupWisePromoQty SET TransactionQTY=TransactionQTY+@qty WHERE GWPromoQtyId=@GWPromoQtyId
...
(GETDATE(),@GWPromoQtyId,@qty)
	...
	--	UPDATE dbo.tblGroupWisePromoQty SET TransactionQTY=TransactionQTY-@qty WHERE GWPromoQtyId=@TableId
	    UPDATE dbo.tbl_DCRInfo SET ApprovalStatus='3' WHERE DCRId=@TableId
	END

	--DECLARE @RoleTypeIds INT=0

	--SELECT TOP 1 @RoleTypeIds=ToRoleTypeId FROM dbo.tblDCRApprovalLog_New WHERE TableId=@TableId ORDER BY Step DESC
    ...
    INSERT INTO dbo.tblDCRApprovalLog
    (
    ...
    (   
       ...
       @EntryTimeApp,
		...
		SELECT @DCRApprovalId=SCOPE_IDENTITY()
