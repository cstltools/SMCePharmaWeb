CREATE PROCEDURE [dbo].[sp_SAP_SalesDtls]
...
case when  ivD.UnitPrice =0 and ProductGroupId=3   then 'C'     when  ivD.UnitPrice =0 and ProductGroupId in (1,2) then 'B'    end                                     as FOCType
 ...
 @FrmDate and @ToDate) and DeliveryStatus in ('Partial','Full')

Group by isnull(O.MIOCode,'Blank') ,InvoiceDate,ProductGroupId,ivD.BatchNo  	,  U.SAP_Code , ivD.ProductCode ,  tblStockUOM.StockUOMName  ,CAST(ivD.UnitPrice as decimal(18,2)) )tblx
