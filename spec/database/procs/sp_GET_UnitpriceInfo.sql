CREATE PROCEDURE [dbo].[sp_GET_UnitpriceInfo]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT MT.UnitPriceId,com.CompanyName,MT.ProductName, MT.ProductCode,
	MT.PackSize,MT.MRPPrice,MT.CostPrice,MT.UnitPrice,MT.VATPercentage,
	MT.VATAmountPerUnit,convert(varchar,MT.ActiveDate, 0) ActiveDate,
	    convert(varchar,MT.InActiveDate, 0) InActiveDate,MT.IsActive,
	CASE WHEN MT.IsActive = 0 THEN ''disabled''
	ElSE ''''
	END as Dissable,
	 CASE  WHEN  Entryemp.EmpName Is Null  THEN  us.UserName 
		...
		END as  EMPUpdateBy,	
	    CASE  WHEN ApproveBy.EmpName  Is Null  THEN  Approve.UserName 
		ELSE ApproveBy.EmpName  
		END as  EMPApproveBy,			
		convert(varchar,MT.EntryDate, 0) EntryDatee,			
	    convert(varchar,MT.UpdateDate, 0) UpdateDatee,
	    convert(varchar,MT.ApproveDate, 0) ApproveDatee,
		MT.ActionStatus
		FROM tblUnitPrice AS MT
		LEFT JOIN tblCompanyInfo com ON  com.CompanyId = MT.CompanyId
		LEFT JOIN tblUser us ON us.UserId = MT.EntryBy
		LEFT JOIN tblUser up ON up.UserId = MT.UpdateBy
		LEFT JOIN tblUser Approve ON Approve.UserId = MT.ApproveBy
		...
		LEFT JOIN tblEmpGeneralInfo ApproveBy  ON ApproveBy.EmpInfoId = Approve.EmpInfoId
