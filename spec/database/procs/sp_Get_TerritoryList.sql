...
CREATE PROCEDURE [dbo].[sp_Get_TerritoryList]
...
BEGIN
		...
		STUFF(( SELECT  ',' + ThanaName
                FROM    dbo.tbl_Thana
                WHERE  ThanaId IN (
				SELECT ThanaId FROM dbo.tbl_TerritoryThanaRelation WHERE TerritoryId = DPT.TerritoryId
				)
              FOR
                XML PATH('')
              ), 1, 1, '') AS ThanaName
		FROM tblTerritory AS DPT  WITH (NOLOCK)
		...
		WHERE DPT.TerritoryId is not null    AND (@RegionId = 0 OR R.RegionId = @RegionId)    AND (@areaId = 0 OR DPT.AreaId = @areaId);
