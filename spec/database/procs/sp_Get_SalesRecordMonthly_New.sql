...
CREATE
 ...
 PROCEDURE [dbo].[sp_Get_SalesRecordMonthly_New]
...
BEGIN
   ...
   WHERE tblOrder.CustTypeId=1 and  IsCampaignProduct=1 AND tbl_BonusCampaignNewMaster.CampgainMasterId not in (1,2,3)   AND MONTH(InvoiceDate)='+convert(nvarchar(max),@Month)+' AND YEAR(InvoiceDate)='+convert(nvarchar(max),@Year)+'  '+@param+'  
  ) AS tblt
   ...
   WHERE tblOrder.CustTypeId=1 and  IsCampaignProduct=0  AND MONTH(InvoiceDate)='+convert(nvarchar(max),@Month)+' AND YEAR(InvoiceDate)='+convert(nvarchar(max),@Year) +'   '+@param+' 
    ) AS tblt
   GROUP BY tblt.Criteria
   ...
   WHERE (IsCampaignProduct=0 OR IsCampaignProduct IS NULL) AND (tblOrder.custtypeid=2) AND MONTH(InvoiceDate)='+convert(nvarchar(max),@Month)+' AND YEAR(InvoiceDate)='+convert(nvarchar(max),@Year)+'  '+@param+' 
   ...
   WHERE  (IsCampaignProduct=0 OR IsCampaignProduct IS NULL) AND (tblOrder.custtypeid=3) AND MONTH(InvoiceDate)='+convert(nvarchar(max),@Month)+' AND YEAR(InvoiceDate)='+convert(nvarchar(max),@Year)+' '+@param+'
  ) AS tblt
	GROUP BY tblt.Criteria
   UNION ALL
