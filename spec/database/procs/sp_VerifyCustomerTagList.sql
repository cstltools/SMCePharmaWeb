...
-- =============================================
CREATE PROCEDURE [dbo].[sp_VerifyCustomerTagList] --- exec sp_StockInMIGOtoCentralStore 3
	@MasterID INT
AS
BEGIN
...
FOR
---------------
	SELECT BRANCH,CustomerCode,MIOCode,TerritoryCode,FECode,DZSMCode,DetailID,SHIPPINGPOINT FROM dbo.tblCustomerMasterTagChangeExcelFileDetail WHERE MasterID=@MasterID
----------
OPEN @MyCursor
...
IF(@BranchData>0 AND @CustData>0 AND @MIOData>0 AND @TerrData>0 AND @FeData>0 AND @DzsmData>0 AND @SHIPPINGPOINTData>0)
BEGIN
	UPDATE dbo.tblCustomerMasterTagChangeExcelFileDetail SET Verifyed='1' WHERE DetailID=@DetailID
END
ELSE
BEGIN
UPDATE dbo.tblCustomerMasterTagChangeExcelFileDetail SET Verifyed='0' WHERE DetailID=@DetailID
	SET @AllOkStatus='0'
END
...
IF(@AllOkStatus='1')
BEGIN
	UPDATE dbo.tblCustomerMasterTagChangeExcelFileMaster SET VerifyedAll='1' 
	WHERE CustomerTagChangeExcelFileMasterID=@MasterID 
END
END
