...
-- =============================================
CREATE PROCEDURE [dbo].[sp_OrderGenerationFromUploadOrder_SingleOrder] --- exec sp_OrderGenerationFromUploadOrder_SingleOrder 21549,10002208916,1
	@OrderMasterID_In INT,
	@OrderCode NVARCHAR(250),
	@IsApiData BIT=1
AS
BEGIN
...
SELECT DISTINCT M.ManufacId,R.OrderMasterID,REPLACE(R.OrderCode,' ','')OrderCode,R.SubmissionDate,REPLACE(R.SalesCentre,' ','')SalesCentre,
R.SalesCentreName SalesCentreName,REPLACE(R.MIOCode ,' ','')MIOCode,R.MIOName,R.TerritoryCode,REPLACE(R.CustomerID,' ','') CustomerID,
R.CustomerName ,
'' as  CampaignName,OrderSenderType,OrderSenderCode,OrderSenderName,
  REPLACE(R.FECode ,' ','')FECode,
R.DZSMCode,
R.Address,
R.CellNo,
R.ConPerson,
...
(SELECT * FROM dbo.tblOrder WITH (NOLOCK) WHERE OrderCode=@OrderCode)	
BEGIN
-------- new cust create
...
IF	EXISTS(SELECT * FROM dbo.tblCustMaster WITH (NOLOCK) WHERE CustomerCode=@CustomerID ) --AND ComUnitCode=@SalesCentre) --AND AreaCode=@TerritoryCode )
--AND MiaCode=@MIOCode)
 BEGIN----------cond1
 ------------------------------------new work
 ----------customer update
...
UPDATE SalesDisDB_SMC..tblCustMaster
					...
					District=  @District,
					Thana=   @Thana,
					Upazila=  @Upazila,CustomerType=@CustomerType
