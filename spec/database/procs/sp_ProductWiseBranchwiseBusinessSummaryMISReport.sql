-- =============================================
-- =============================================
CREATE PROCEDURE [dbo].[sp_ProductWiseBranchwiseBusinessSummaryMISReport]  -- exec sp_ProductWiseBranchwiseBusinessSummaryMISReport 10,'01-Jul-2019','30-Jun-2020'
...
isnull(tblSale.SalesAmtTP,0)+  isnull(tblOldRtn.ReturnAmountTP,0) AS SumofNetProformaAmount,
...
ISNULL(tblA.SumofNetProformaAmount,0)   -  (ISNULL(tblD.SumofNetReturnAmount,0) + ISNULL(tblDr.SumofNetReturnAmount,0)) AS SumofNetSalesAmount,
((ISNULL(tblA.ProTpVat,0) )-(ISNULL(tblD.TotalPriceVatAmount,0) + ISNULL(tblDr.TotalPriceVatAmount,0) )) AS DelTpVat,
((ISNULL(tblA.ProTpVat,0)  + ISNULL(tblA.SumofNetProformaAmount,0) ))-(ISNULL(tblD.SumofNetReturnAmount,0) + ISNULL(tblDr.SumofNetReturnAmount,0) + (ISNULL(tblD.TotalPriceVatAmount,0) + ISNULL(tblDr.TotalPriceVatAmount,0) ) ) AS GrossSales,
...
FROM dbo.tblInvoice  WITH (NOLOCK)  INNER JOIN dbo.tblInvoiceDetail D WITH (NOLOCK) ON tblInvoice.InvoiceId = D.InvoiceId WHERE ComUnitId=@Branch AND D.ISGiftProduct=1 and  
...
INNER JOIN dbo.tblSubInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId WHERE  ComUnitId=@Branch AND  ID.DeliveryStatus IN ('Reject','Partial') AND I.UpdateDate  BETWEEN @fromdate and @todate AND 
...
WHERE  ComUnitId=@Branch AND I.TpGrandTotal>0 AND I.InvoiceDate BETWEEN @fromdate and @todate  GROUP BY ID.ProductCode)tblAOS ON tblAOS.ProductCode=C.ProductCode 
...
INNER JOIN dbo.tblInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId  where ComUnitId=@Branch AND ID.DeliveryStatus IN ('Reject','Partial') AND I.UpdateDate BETWEEN @fromdate and @todate AND 
...
FROM dbo.tblSubInvoiceMaster I WITH (NOLOCK)  INNER JOIN dbo.tblSubInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId  WHERE ComUnitId=@Branch AND I.TpGrandTotal>0 AND I.InvoiceDate 
...
FROM dbo.tblInvoice I WITH (NOLOCK) INNER JOIN dbo.tblInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId WHERE ComUnitId=@Branch AND I.TpGrandTotal>0 AND (I.InvoiceDate BETWEEN @fromdate and @todate) 
 ...
 WITH (NOLOCK) ON tblInvoice.InvoiceId = D.InvoiceId WHERE ComUnitId=@Branch AND  D.OrderDetailsId<>0 and InvoiceDate BETWEEN @fromdate and @todate GROUP BY  ProductCode)tblc ON tblc.ProductCode = C.ProductCode  
 ...
 SUM(ID.TotalPriceVatAmount)-sum(DeliveryTotalPriceVatAmount) as TotalPriceVatAmount FROM dbo.tblInvoice I WITH (NOLOCK)
  INNER JOIN dbo.tblInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId  where ComUnitId=@Branch AND ID.DeliveryStatus IN ('Reject','Partial') 
  ...
  INNER JOIN dbo.tblSubInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId  where ComUnitId=@Branch AND ID.DeliveryStatus IN ('Reject','Partial') AND I.UpdateDate 
   ...
   -- GROUP BY  ProductCode)tblCollection ON tblCollection.ProductCode = C.ProductCode  
	...
	D WITH (NOLOCK) ON tblSubInvoiceMaster.InvoiceId = D.InvoiceId WHERE ComUnitId=@Branch AND PaymentAmount IS NOT NULL AND DeliveryInvoiceStatus IN  ('Partial','Full')  AND TpGrandTotal>0 AND UpdateDate 
...
INNER JOIN SalesDisDB_SMC..tblInvoiceDetail ID WITH (NOLOCK) ON ID.InvoiceId = I.InvoiceId  where ComUnitId=@Branch AND ID.DeliveryStatus IN ('Reject','Partial') AND I.UpdateDate BETWEEN @fromdate AND @todate AND I.TpGrandTotal>0  
...
where I.ComUnitId=@Branch AND ID.DeliveryStatus IN ('Full','Partial')   AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate group by  ID.ProductCode)tblCollection ON tblCollection.ProductCode  =C.ProductCode 
...
where I.ComUnitId=@Branch AND ID.DeliveryStatus IN ('Full','Partial')   AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate  group by ID.ProductCode )tblCollectionold ON tblCollectionold.ProductCode =C.ProductCode
...
INNER JOIN SalesDisDB_SMC..tblProduct P ON ID.ProductCode = P.ProductCode 
INNER JOIN SalesDisDB_SMC..tblProductSQ SQ ON P.ProductBrandId = SQ.ProductBrandId 
where I.ComUnitId=13 AND ID.DeliveryStatus IN ('Full','Partial')  AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate  group by ID.ProductCode )tblCollectionoldsub ON tblCollectionoldsub.ProductCode =C.ProductCode
...
WHERE       I.UpdateDate BETWEEN @fromdate AND @todate and DelivaryInvoiceNo is not null   GROUP BY D.ProductCode)tblSale ON C.ProductCode=tblSale.ProductCode

	
	LEFT JOIN   (
SELECT  ID.ProductCode,count(ID.Quantity-ID.DeliveryQuantity) NumberofReturn,
