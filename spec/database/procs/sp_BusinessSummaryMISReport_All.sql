-- =============================================
-- =============================================
CREATE PROCEDURE [dbo].[sp_BusinessSummaryMISReport_All] 
...
((ISNULL(tblA.ProTpVat,0) )-(ISNULL(tblD.TotalPriceVatAmount,0) ))) - (ISNULL(tblDr.SumofNetReturnAmount,0)+ISNULL(tblDr.TotalPriceVatAmount,0)) AS SalesTotal--9
...
,(ISNULL(tblCollection.TpTotal,0)+ISNULL(tblCollectionold.TpTotal,0) )
...
where ID.DeliveryStatus IN ('Full','Partial')   AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate group by   O.AreaId)tblCollection ON tblCollection.AreaId = C.AreaId 
...
where ID.DeliveryStatus IN ('Full','Partial')   AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate group by   O.TerritoryId)tblCollection ON tblCollection.TerritoryId = C.TerritoryId 
