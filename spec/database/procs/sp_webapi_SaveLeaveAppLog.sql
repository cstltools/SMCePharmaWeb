...
CREATE PROCEDURE [dbo].[sp_webapi_SaveLeaveAppLog]  --[dbo].[sp_webapi_SaveOrderMaster] 'shaon20','20245','11/25/2020 5:04:16 PM'
    @LeaveApprovalId INT ,
...
AS
	...
	IF(@Status='Accepted')
	BEGIN
	--DECLARE @CountApprove INT=0
	--select @CountApprove=ISNULL(count(*),0) from  Employee_LeaveApplications WHERE LeaveApplicationId=@TableId and ApprovalStatus <> '2'
	--if(@CountApprove=0)
	--BEGIN
	...
	DECLARE @CountReject INT=0
	select @CountReject=ISNULL(count(*),0) from  Employee_LeaveApplications WHERE LeaveApplicationId=@TableId and ApprovalStatus <> '3'
	if(@CountReject=0)
	BEGIN
