CREATE PROCEDURE [dbo].[sp_GET_DZSMwiseReportParam_ByProcess_vv] 
...
inner join   SalesDisDB_SMC_TrSalesRepor..tblTerritory A  WITH (NOLOCK) on O.TerritoryId = A.TerritoryId
--inner join tblRegion R on O.RegionId = R.RegionId
 left join tblCustomerType cstType WITH (NOLOCK)  on  O.CustTypeId=cstType.CustomerTypeId
WHERE cstType.CustomerCategoryId in (2) AND DeliveryInvoiceStatus IN  ('Partial','Full')  
AND  tblInvoice.UpdateDate BETWEEN @FromDate and @ToDate  
...
left JOIN   SalesDisDB_SMC_TrSalesRepor..[tbl_BonusCampaignNewDetail] camp  with(nolock) ON camp.CampaignDetailId = masdtl.CampaignType
INNER JOIN  SalesDisDB_SMC_TrSalesRepor..tbl_BonusCampaignNewMaster MB WITH (NOLOCK) ON camp.CampaignMasterId = MB.CampgainMasterId 
WHERE   (( SUBSTRING(MB.CampaignName, 1, 3) like '%Bon%' or SUBSTRING(MB.CampaignName, 1, 3) like '%Sal%')) AND DeliveryInvoiceStatus IN  ('Partial','Full')  AND  
...
left JOIN   SalesDisDB_SMC_TrSalesRepor..[tbl_BonusCampaignNewDetail] camp  with(nolock) ON camp.CampaignDetailId = masdtl.CampaignType
INNER JOIN  SalesDisDB_SMC_TrSalesRepor..tbl_BonusCampaignNewMaster MB WITH (NOLOCK) ON camp.CampaignMasterId = MB.CampgainMasterId 
WHERE   (( SUBSTRING(MB.CampaignName, 1, 3) like '%Bon%' or SUBSTRING(MB.CampaignName, 1, 3) like '%Sal%'))   
AND  
...
left JOIN   SalesDisDB_SMC_TrSalesRepor..[tbl_BonusCampaignNewDetail] camp  with(nolock) ON camp.CampaignDetailId = masdtl.CampaignType
INNER JOIN  SalesDisDB_SMC_TrSalesRepor..tbl_BonusCampaignNewMaster MB WITH (NOLOCK) ON camp.CampaignMasterId = MB.CampgainMasterId 
WHERE   (( SUBSTRING(MB.CampaignName, 1, 3) like '%Bon%' or SUBSTRING(MB.CampaignName, 1, 3) like '%Sal%'))   AND  
...
inner join  SalesDisDB_SMC_TrSalesRepor..tblTerritory A  WITH (NOLOCK) on O.TerritoryId = A.TerritoryId
left join tblCustomerType cstType WITH (NOLOCK)  on  O.CustTypeId=cstType.CustomerTypeId
WHERE cstType.CustomerCategoryId in (2)  AND 
...
-- order by C.TerritoryCode 
 ...
 where  (tblA.RegionId= COALESCE( NULLIF(@ZoneSelect , 0) ,tblA.RegionId )
 OR tblc.RegionId=  COALESCE( NULLIF(@ZoneSelect , 0) ,tblc.RegionId )
 or tblD.RegionId=  COALESCE( NULLIF(@ZoneSelect , 0) ,tblD.RegionId )) 
