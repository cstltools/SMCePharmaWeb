...
CREATE PROCEDURE [dbo].[sp_Webapi_Save_ExpenseClaimMaster]
...
-- @expDate আগে থেকেই NVARCHAR হিসেবে আসছে ধরে নিচ্ছি
SET @expDate = LTRIM(RTRIM(@expDate));

DECLARE @norm NVARCHAR(100) = @expDate;

-- হাইফেন/স্ল্যাশ নরমালাইজ
SET @norm = REPLACE(@norm, '-', ' ');
SET @norm = REPLACE(@norm, '/', ' ');

-- 'Sept' -> 'Sep' (case-insensitive)
SET @norm = REPLACE(@norm COLLATE SQL_Latin1_General_CP1_CI_AS, 'SEPT', 'Sep');

-- একাধিক স্পেস কমানো (ঐচ্ছিক)
WHILE CHARINDEX('  ', @norm) > 0
    SET @norm = REPLACE(@norm, '  ', ' ');

DECLARE @expDateConverted DATE =
    COALESCE(
        TRY_CONVERT(DATE, @norm, 106),                      -- dd mon yyyy  (e.g., 01 Sep 2025)
        TRY_CONVERT(DATE, REPLACE(@norm,' ','/'), 103),     -- dd/mm/yyyy
        TRY_CONVERT(DATE, @expDate, 120),                   -- yyyy-mm-dd
        TRY_CONVERT(DATE, @expDate, 112)                    -- yyyymmdd
    );

IF @expDateConverted IS NULL
BEGIN
    RAISERROR('Invalid date format: %s', 16, 1, @expDate);
    RETURN;
END;


IF (@expDateConverted > CAST(GETDATE() AS DATE))
BEGIN
    PRINT 'no -- future date not allowed'
END
ELSE IF (@expDateConverted < DATEADD(DAY, -7, CAST(GETDATE() AS DATE)))
BEGIN
    PRINT 'Yes -- older than 7 days, not allowed'
END
ELSE
BEGIN
    PRINT 'Valid (within last 7 days)'
...
BEGIN TRY
    ...
    SET @expDate = CONVERT(DATETIME, @expDate, 106);
