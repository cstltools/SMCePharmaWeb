...
CREATE PROCEDURE [dbo].[sp_GET_DZSMwiseNAtionalReport] 
...
Select  ISNULL(tblA.RegionCode,tblA.RegionCode)RegionCode ,ISNULL(tblA.AreaCode,C.AreaCode)AreaCode  ,ISNULL(tblA.AreaName,C.AreaName)AreaName,
...
ISNULL(tblD.NumberofReturnInvoice,0) + ISNULL(tblDD.NumberofReturnInvoice,0) AS NumberofReturnInvoice,ISNULL(tblD.SumofNetReturnAmount,0) + ISNULL(tblDD.SumofNetReturnAmount,0)
 AS SumofNetReturnAmount 
...
LEFT JOIN (SELECT tblInvoice.AreaCode,COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,SUM(D.DeliveryNetAmount - D.DeliveryTotalPriceVatAmount) SumofNetSalesAmount ,  
SUM(D.DeliveryTotalPriceVatAmount)DelTpVat FROM dbo.tblInvoice  WITH (NOLOCK) INNER JOIN dbo.tblInvoiceDetail D WITH (NOLOCK) ON tblInvoice.InvoiceId = D.InvoiceId   WHERE DeliveryInvoiceStatus IN  
('Partial','Full') AND TpGrandTotal>0 AND  UpdateDate BETWEEN @FromDate and @ToDate   GROUP BY  tblInvoice.AreaCode)tblc 
...
--LEFT JOIN (SELECT ((CustomerMasterId  ) ) CustomerCoverPer ,T2.RegionCode,T2.AreaCode from (SELECT COUNT(CustomerMasterId) CustomerMasterId1,RegionCode,AreaCode  FROM dbo.tblCustMaster 
--GROUP BY RegionCode,AreaCode) AS T1,((SELECT COUNT(CustomerMasterId)CustomerMasterId,RegionCode,AreaCode       FROM dbo.tblCustMaster WHERE  CustomerMasterId  IN  (SELECT CustomerMasterId 
--FROM dbo.tblInvoice WHERE DeliveryInvoiceStatus<>'Reject' and UpdateDate BETWEEN @FromDate and @ToDate) GROUP BY RegionCode,AreaCode) ) AS 
...
--LEFT JOIN (SELECT ((CustomerMasterId ) ) CustomerCoverPer ,T2.RegionCode,T2.AreaCode from (SELECT COUNT(CustomerMasterId) CustomerMasterId1,RegionCode,AreaCode  FROM dbo.tblCustMaster 
--GROUP BY RegionCode,AreaCode) AS T1, ((SELECT COUNT(CustomerMasterId)CustomerMasterId,RegionCode,AreaCode      FROM dbo.tblCustMaster WHERE  CustomerMasterId  IN  
--(SELECT CustomerMasterId FROM dbo.tblSubInvoiceMaster WHERE DeliveryInvoiceStatus<>'Reject' and  UpdateDate BETWEEN @FromDate and @ToDate) GROUP BY RegionCode,AreaCode) ) 
...
---Customer Coverage for BSP,GSP end
 ...
 where 
 --(ISNULL(tblA.RegionCode,tblA.RegionCode)>'0' or ISNULL(tblA.AreaCode,C.AreaCode) >'0' or ISNULL(tblA.AreaName,C.AreaName)>'0')

 ( (ISNULL(tblA.NumberofProformaInvoice,0) + ISNULL(tblAA.NumberofProformaInvoice,0) ) > 0  or (ISNULL(tblD.NumberofReturnInvoice,0) + ISNULL(tblDD.NumberofReturnInvoice,0)) >0

  or (ISNULL(tblC.NumberofInvoiceSold,0) + ISNULL(tblCC.NumberofInvoiceSold,0)) >0 )  
