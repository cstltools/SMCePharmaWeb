...
CREATE PROCEDURE [dbo].[sp_GET_DesignationInfo]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT DPT.DesignationId,
		...
		DPT.DesigCode,DPT.DesigName,
		IsActive,CASE WHEN ISNULL(C.NoOf,0) > 0 THEN ''disabled'' ELSE '''' END AS DeleteStatus
		FROM tblDesignation AS DPT 
		...
		LEFT JOIN tblEmpGeneralInfo empAcIn  ON  empAcIn.EmpInfoId = AcIN.EmpInfoId		
			LEFT JOIN (SELECT DISTINCT DesignationId, COUNT(DesignationId) NoOf FROM tblEmpGeneralInfo GROUP BY DesignationId) AS C ON DPT.DesignationId = C.DesignationId
		WHERE DPT.DesignationId IS NOT NULL ' + @Parameter
