...
CREATE PROCEDURE [dbo].[sp_GET_StationTypeInfo]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT
		 DPT.StationTypeId,CASE  WHEN  Entryemp.EmpName Is Null  THEN  us.UserName 
		...
		DPT.StationTypeName, DPT.StationCode,
		IsActive
		FROM tblStationType AS DPT 
		...
		WHERE DPT.StationTypeId IS NOT NULL' + @Parameter
