...
CREATE PROCEDURE [dbo].[sp_GET_TherapueticGroup]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT PC.TherapeuticGroupId,PC.TherapeuticGroupCode,PC.TherapeuticGroupName,CASE  WHEN  Entryemp.EmpName Is Null  THEN  us.UserName 
		...
		PC.IsActive
		FROM tblTherapeuticGroup AS PC
		...
		WHERE PC.TherapeuticGroupId IS NOT NULL ' + @Parameter
