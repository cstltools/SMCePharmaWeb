-- =============================================
-- =============================================
Create PROCEDURE [dbo].[sp_Get_MonthlyInventoryReport_Back] 
...
-- 0 TotalIssued, ISNULL(InvActualQty,0) InvoiceQty,  ISNULL(DelActualQty,0) SalesQty, 
 ISNULL(tblreturn.ReturnQty,0) ReturnQty, 

isnull((select  ComUnitName from tblCompanyUnit where ComUnitId =@CiD),'')ComUnitName,CONVERT(varchar,@fromDate,6)  as fromDate,CONVERT(varchar,@toDate,6)  as toDate, P.ProductCode as ProductCode,P.ProductName as ProductName ,P.PackSize as BaseUnit,  ( ISNULL(vTblOB.Quantity,0)  ) OpeningStock   ,
...
)+ ISNULL(tblreturn.ReturnQty,0)))-ISNULL(vTblFreez.Freeze,0)
