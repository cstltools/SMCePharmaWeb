...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Get_EmployeeInformationListRpt]
...
declare @newMonth nvarchar(max)

set @newMonth=MONTH(GETDATE())

if(@Month=@newMonth-1 or @Month=@newMonth)
begin 
...
inner JOIN
                 ...
                 FROM    dbo.tblMIOInfo AS M LEFT OUTER JOIN
                              dbo.tblTerritory AS Tr ON M.TerritoryId = Tr.TerritoryId   where Tr.isactive=1
							  -- and M.isactive=1
