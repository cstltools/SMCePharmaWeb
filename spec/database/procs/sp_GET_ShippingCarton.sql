...
CREATE PROCEDURE [dbo].[sp_GET_ShippingCarton]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT DPT.ShippingCartonSizeId,DPT.ShippingCartonSizeName,DPT.ShippingCartonCode,DPT.IsActive,
	    ...
	    convert(varchar,DPT.ActiveInactiveDate, 0) InactiveDatee,
		DPT.IsActive
		FROM  tblShippingCartonSize AS DPT 
		...
		WHERE DPT.ShippingCartonSizeId IS NOT NULL' + @Parameter
