...
create PROCEDURE [dbo].[sp_DeleteArchiveDBTourPlanData]
...
DELETE ti
FROM ' + @DbName + N'.dbo.tbl_TourPlanInfo ti
WHERE ti.TourPlanDate >= @FromDate
  AND ti.TourPlanDate < DATEADD(DAY, 1, @ToDate);

DELETE tm
FROM ' + @DbName + N'.dbo.tbl_TourPlanMaster tm
WHERE ((tm.YearValue * 100) + tm.MonthValue) BETWEEN ((YEAR(@FromDate) * 100) + MONTH(@FromDate))
                                                  AND ((YEAR(@ToDate) * 100) + MONTH(@ToDate));';
