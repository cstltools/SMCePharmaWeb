...
CREATE PROCEDURE [dbo].[sp_GET_ProgramtypeInfo]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT
		 DPT.ProgramTypeId,CASE  WHEN  Entryemp.EmpName Is Null  THEN  us.UserName 
		...
		DPT.ProgramTypeName, DPT.PrgmTypeCode,
		IsActive,*
		FROM tblProgramType AS DPT 
		...
		WHERE DPT.ProgramTypeId IS NOT NULL' + @Parameter
