...
CREATE PROCEDURE [dbo].[sp_WEBAPI_Process_DWSPReportAM] --exec sp_GetHourlySweingOutput '20-sep-2020' 
...
where tr.IsActive=1     and ar.AreaId in (@EmpId)
...
declare 	@1WeekPer decimal (18,2) 
	...
	---1
  select @1Week=  ISNULL(SUM(ISNULL(dtl.GeneralAmount,0)+ISNULL(dtl.FCBAmount,0)+ISNULL(dtl.CampaignAmount,0)) ,0)  from  tbl_DWSPMaster mas   with (nolock) 
...
where  DAY(dtl.DWSPDate) between   1 and 7 and Month(dtl.DWSPDate)=Month(@DateNew) and Year(dtl.DWSPDate)=year(@DateNew)  and mas.TerritoryId=@TerritoryId and   mas.ApprovalStatus= '2'
    ...
    select @2Week=  ISNULL(SUM(ISNULL(dtl.GeneralAmount,0)+ISNULL(dtl.FCBAmount,0)+ISNULL(dtl.CampaignAmount,0)) ,0)   from  tbl_DWSPMaster mas   with (nolock) 
left join tbl_DWSPDetail dtl   with (nolock) on mas.DWSPMasterId=dtl.DWSPMasterId
where  DAY(dtl.DWSPDate) between   8 and 15 and Month(dtl.DWSPDate)=Month(@DateNew) and Year(dtl.DWSPDate)=year(@DateNew)  and mas.TerritoryId=@TerritoryId and   mas.ApprovalStatus= '2'
 ...
 select @3Week=  ISNULL(SUM(ISNULL(dtl.GeneralAmount,0)+ISNULL(dtl.FCBAmount,0)+ISNULL(dtl.CampaignAmount,0)) ,0)  from  tbl_DWSPMaster mas   with (nolock) 
left join tbl_DWSPDetail dtl   with (nolock) on mas.DWSPMasterId=dtl.DWSPMasterId
where  DAY(dtl.DWSPDate) between   16 and 23 and Month(dtl.DWSPDate)=Month(@DateNew) and Year(dtl.DWSPDate)=year(@DateNew)  and mas.TerritoryId=@TerritoryId and   mas.ApprovalStatus= '2'
 ...
 select @4Week=  ISNULL(SUM(ISNULL(dtl.GeneralAmount,0)+ISNULL(dtl.FCBAmount,0)+ISNULL(dtl.CampaignAmount,0) ) ,0)  from  tbl_DWSPMaster mas   with (nolock) 
left join tbl_DWSPDetail dtl   with (nolock) on mas.DWSPMasterId=dtl.DWSPMasterId
