...
--------------------------------------------------
-- PROCEDURE: sp_Get_DAExpenseClaimList
--------------------------------------------------

CREATE   PROCEDURE dbo.sp_Get_DAExpenseClaimList
    @ComUnitId INT,
    @RouteId INT = NULL,
    @daid INT = NULL,
    @FromDate DATE = NULL,
...
BEGIN
    ...
    WHERE EXISTS (
            SELECT 1
            FROM dbo.tblRouteInformationMaster route WITH (NOLOCK)
            INNER JOIN dbo.tblRouteInformationDADetail routeDa WITH (NOLOCK)
                ON route.RouteInformationMasterId = routeDa.RouteInformationMasterId
            WHERE route.DCId = @ComUnitId
              AND routeDa.DAId = EC.EmpInfoId
              AND (@RouteId IS NULL OR route.RouteInformationMasterId = @RouteId)
        )
      AND (@daid IS NULL OR EC.EmpInfoId = @daid)
      AND (@FromDate IS NULL OR EC.ExpenseDate >= @FromDate)
      AND (@ToDate IS NULL OR EC.ExpenseDate < DATEADD(DAY, 1, @ToDate))
    ...
    EXEC sys.sp_executesql
        @detailSql,
        N'@ComUnitId INT, @RouteId INT, @daid INT, @FromDate DATE, @ToDate DATE',
        @ComUnitId = @ComUnitId,
        @RouteId = @RouteId,
        @daid = @daid,
        @FromDate = @FromDate,
