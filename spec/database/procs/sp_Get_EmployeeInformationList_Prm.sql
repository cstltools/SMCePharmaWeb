...
-- =============================================
create PROCEDURE [dbo].[sp_Get_EmployeeInformationList_Prm]
...
BEGIN
  ...
  SET @SqlStatement = N'Select distinct PM.EmployeeStatus, PM.EmpMasterCode, PM.EmpName, fs.RegionCode+ '' : '' +fs.RegionName RegionName,  fs.AreaCode+ '' : '' +fs.AreaName AreaName,  fs.TerritoryCode+ '' : '' +fs.TerritoryName TerritoryName, dgs.DesigName, PM.CellNumber PhoneNo,format( PM.JoiningDate,''dd MMMM,yyyy'')  JoiningDate ,usRT.RoleType, usR.RoleName, PM.EmpInfoId,CONVERT(NVARCHAR(50),PM.DateOfBirth,106)AS EmpDateOfBirth  from tblEmpGeneralInfo PM with (nolock)
   ...
   where PM.EmpInfoId is not null  '+@Parm+'
