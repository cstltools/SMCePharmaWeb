...
create PROCEDURE [dbo].[sp_DeleteArchiveDBExpenseData]
...
DELETE dtl
FROM ' + @DbName + N'.dbo.tbl_ExpenseClaimDetails dtl
INNER JOIN ' + @DbName + N'.dbo.tbl_ExpenseClaim mas
    ON dtl.ExpenseClaimID = mas.ExpenseClaimID
WHERE mas.EntryDate >= @FromDate
  AND mas.EntryDate < DATEADD(DAY, 1, @ToDate);

DELETE mas
FROM ' + @DbName + N'.dbo.tbl_ExpenseClaim mas
WHERE mas.EntryDate >= @FromDate
  AND mas.EntryDate < DATEADD(DAY, 1, @ToDate);';
