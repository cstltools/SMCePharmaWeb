...
CREATE PROCEDURE [dbo].[sp_Process_DZSMwiseReportParam] 
...
BEGIN
 ...
 set @FromDate=  format(DATEADD(mm, DATEDIFF(m,0,GETDATE()),0),'dd MMMM, yyyy')
  ...
  delete from tblDZSMwiseReportParam where MonthValue= MONTH(CONVERT(Date, @FromDate)) and YearValue= YEAR(CONVERT(Date, @FromDate))


  INSERT INTO [dbo].[tblDZSMwiseReportParam]
           ([RegionId]
      ,[AreaCode]
      ,[AreaName]
      ,[TerritoryCode]
      ...
      ,[DelReTpVat]
      ,[CustomerCoverPer]
      ,[SumofNetSalesAmountFixed]
      ,[SumofNetSalesAmountCamp]
      ,[FinalSales]
      ,[SumofNetSalesAmountFixed2]
      ,[SumofNetSalesAmountCamp2]
      ,[FinalSales2]
      ,[CustomerCoverPerProforma]
      ,[BlueNetSell]
      ,[GreenNetSell]
      ,[DelBlueNetSell]
      ,[DelGreenNetSell]
...
-- (ISNULL(tblDelGreenCov.CustomerCoverPer,0)+ISNULL(tblDelGreenCovSub.CustomerCoverPer,0)) DelgreenCov 
  ...
  (ISNULL(tblProgrmaGrD.CustomerCoverPer,0)+ISNULL(tblDelGreenCov.CustomerCoverPer,0)+ISNULL(tblDelGreenCovSub.CustomerCoverPer,0)) AS DelgreenCov, MONTH(CONVERT(Date, @FromDate)), Year(CONVERT(Date, @FromDate)), getdate()
...
LEFT JOIN (SELECT A.TerritoryName,O.RegionId,A.TerritoryCode,count(DISTINCT I.InvoiceId) NumberofProformaInvoice,SUM(ID.NetAmount)-sum(ID.TotalPriceVatAmount)  
...
inner join tblTerritory A with(NoLock) on O.TerritoryId = A.TerritoryId
...
LEFT JOIN (SELECT O.RegionId,A.TerritoryName,A.TerritoryCode,COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,SUM(D.DeliveryTotalPrice) SumofNetSalesAmount ,  
...
left join tblTerritory A  WITH (NOLOCK) on O.TerritoryId = A.TerritoryId
--inner join tblRegion R on O.RegionId = R.RegionId
...
LEFT JOIN (SELECT RegionCode,A.AreaName,tblInvoice.AreaCode,COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,SUM(D.DeliveryTotalPrice) SumofNetSalesAmount ,  
...
LEFT JOIN  (SELECT A.AreaName,tblSubInvoiceMaster.RegionCode,tblSubInvoiceMaster.AreaCode,COUNT(DISTINCT tblSubInvoiceMaster.InvoiceId)NumberofInvoiceSold, SUM(D.DeliveryTotalPrice) SumofNetSalesAmount , SUM(D.DeliveryTotalPriceVatAmount)DelTpVat FROM SalesDisDB_SMC..tblSubInvoiceMaster  WITH (NOLOCK) INNER JOIN SalesDisDB_SMC..tblSubInvoiceDetail D WITH 
...
LEFT JOIN (SELECT O.RegionId,A.TerritoryCode, ((SUM(ID.NetAmount)-sum(ID.TotalPriceVatAmount))- SUM(ID.DeliveryNetAmount - ID.DeliveryTotalPriceVatAmount)) AS SumofNetReturnAmount,
...
LEFT JOIN (SELECT O.RegionId,A.TerritoryCode,COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,SUM(D.DeliveryTotalPrice) SumofNetSalesAmount ,  
SUM(D.DeliveryTotalPriceVatAmount)DelTpVat 
...
LEFT JOIN (SELECT tblInvoice.RegionCode,tblInvoice.AreaCode,COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,SUM(D.DeliveryTotalPrice) SumofNetSalesAmount ,  
...
SUM(D.DeliveryTotalPrice) SumofNetSalesAmount , SUM(D.DeliveryTotalPriceVatAmount)DelTpVat FROM SalesDisDB_SMC..tblSubInvoiceMaster  WITH (NOLOCK) 
INNER JOIN SalesDisDB_SMC..tblSubInvoiceDetail D WITH (NOLOCK) ON tblSubInvoiceMaster.InvoiceId = D.InvoiceId  INNER JOIN dbo.tblArea A WITH (NOLOCK) ON tblSubInvoiceMaster.AreaCode = A.AreaCode 
...
inner join tblorder O  WITH (NOLOCK) on O.OrderId = tblInvoice.OrderId
inner join tblTerritory A  WITH (NOLOCK) on O.TerritoryId = A.TerritoryId
INNER JOIN dbo.tblOrderDetail masdtl  with(nolock) ON D.OrderDetailsId = masdtl.OrderDetailId
left JOIN dbo.[tbl_BonusCampaignNewDetail] camp  with(nolock) ON camp.CampaignDetailId = masdtl.CampaignType
INNER JOIN dbo.tbl_BonusCampaignNewMaster MB WITH (NOLOCK) ON camp.CampaignMasterId = MB.CampgainMasterId 
...
LEFT JOIN  (SELECT A.AreaName,tblSubInvoiceMaster.RegionCode,tblSubInvoiceMaster.AreaCode,COUNT(DISTINCT tblSubInvoiceMaster.InvoiceId)NumberofInvoiceSold, 
SUM(D.DeliveryNetAmount - D.DeliveryTotalPriceVatAmount) SumofNetSalesAmount , SUM(D.DeliveryTotalPriceVatAmount)DelTpVat FROM SalesDisDB_SMC..tblSubInvoiceMaster  WITH (NOLOCK) 
...
LEFT JOIN (SELECT O.RegionId,A.TerritoryCode,COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,SUM(D.NetAmount - D.TotalPriceVatAmount) SumofNetSalesAmount ,  
...
inner join tblorder O  WITH (NOLOCK) on O.OrderId = tblInvoice.OrderId
inner join tblTerritory A  WITH (NOLOCK) on O.TerritoryId = A.TerritoryId
INNER JOIN dbo.tbl_BonusCampaignNewDetail B WITH (NOLOCK) ON D.CampaignType = B.CampaignDetailId 
INNER JOIN dbo.tbl_BonusCampaignNewMaster MB WITH (NOLOCK) ON B.CampaignMasterId = MB.CampgainMasterId 
...
LEFT JOIN (SELECT A.TerritoryName,O.RegionId,A.TerritoryCode,SUM(ID.DeliveryNetAmount)-sum(ID.DeliveryTotalPriceVatAmount)  
...
FROM dbo.tblInvoice  WITH (NOLOCK) 
inner join tblorder O  WITH (NOLOCK) on O.OrderId = tblInvoice.OrderId
INNER JOIN dbo.tblTerritory  WITH (NOLOCK) ON tblTerritory.TerritoryId = o.TerritoryId
