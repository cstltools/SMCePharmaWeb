CREATE PROCEDURE [dbo].[sp_webapi_Get_SampleStockReport_new]
...
AS
	...
	END
		...
		select pro.ProductCode ,
                pro.ProductName ,
                SUM(mas.TransactionQTY) AS StockQty from tblGroupWisePromoQty mas with (nolock)
inner join tblProduct pro on pro.productId=mas.ProductId
where mas.EmpInfoId= '+CONVERT(NVARCHAR(MAX),@empId)+' 
