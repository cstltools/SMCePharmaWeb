CREATE PROCEDURE [dbo].[DynamicPivotDoctorWiseDCP]
...
BEGIN
 --STUFF( (SELECT CONCAT('','', mm.DegreeName , '''') FROM tblDoctorDegree mm   WITH (NOLOCK)  INNER JOIN dbo.tblDoctorDegreeDetail mgd   WITH (NOLOCK)  ON mgd.DegId=mm.DegreeId WHERE mgd.DoctorId=mas.DoctorId ORDER BY mgd.DoctorDegId FOR XML PATH ('''') ),1,1,'''') AS Degree,ISNULL(STUFF( (SELECT CONCAT('','', mm.SpecialityName , '''') FROM tblDoctorSpeciality mm   WITH (NOLOCK)  INNER JOIN dbo.tblDoctorSpecialityDetail mgd   WITH (NOLOCK)  ON mgd.SpecialityId=mm.SpecialityId WHERE mgd.DoctorId=mas.DoctorId ORDER BY mgd.DoctorSpId FOR XML PATH ('''') ),1,1,''''),'''')  as [Speciality], pt.ProgramTypeName [Provider Type],dt.DoctorTypeName [Doctor Type], gr.GroupCode [Group Code],  gr.GroupName [Group Name],rg.RegionCode [Zone Code],rg.RegionName  [Zone Name],Ar.AreaCode [Area Code],Ar.AreaName [Area Name],Tr.TerritoryCode [Territory Code],Tr.TerritoryName [Territory Name],subTr.SubTerritoryCode [Sub-Territory Code],subTr.SubTerritoryName [Sub-Territory Name],mr.MarketCode [Market Code],mr.MarketName [Market Name]
  ...
  SET @SqlStatement = N'
    SELECT * FROM (
 select  DoctorId,DcrDate ,[Doctor Code],  [Doctor Name], [Doctor Speciality],[Provider Type], [Pharma Platform], [Doctor Type],[Group],[Region],[Area],[Territory],[Market] from   View_DCP mas with (nolock)
  
  
 

  where  convert(date, mas.DcrDate)  between convert(date,'''+@Month+''') and convert(date,'''+@Year+''')  
          and mas.[Pharma Platform]= COALESCE( NULLIF('''+@PharmaPlatform+''' , '''') ,mas.[Pharma Platform] )   and mas.[Provider Type]= COALESCE( NULLIF('''+@ProviderType+''' , '''') ,mas.[Provider Type] )   and mas.RegionId= COALESCE( NULLIF('''+@ZoneSelect+''' , '''') ,mas.RegionId)   and mas.AreaId= COALESCE( NULLIF('''+@AreaSelect+''' , '''') ,mas.AreaId) and mas.TerritoryId= COALESCE( NULLIF('''+@TeritorySelect+''' , '''') ,mas.TerritoryId)  
    ) StudentResults
    PIVOT (
          count(DoctorId)
      FOR ['+@ColumnToPivot+']
  ...
  where   convert(date, mas.DcrDate)  between convert(date,'''+@Month+''') and convert(date,'''+@Year+''')  
      and  mas.ApprovalStatus='+@ApprovalStatus+'   and mas.[Pharma Platform]= COALESCE( NULLIF('''+@PharmaPlatform+''' , '''') ,mas.[Pharma Platform] )   and mas.[Provider Type]= COALESCE( NULLIF('''+@ProviderType+''' , '''') ,mas.[Provider Type] )  and mas.RegionId= COALESCE( NULLIF('''+@ZoneSelect+''' , '''') ,mas.RegionId)   and mas.AreaId= COALESCE( NULLIF('''+@AreaSelect+''' , '''') ,mas.AreaId) and mas.TerritoryId= COALESCE( NULLIF('''+@TeritorySelect+''' , '''') ,mas.TerritoryId)   ) StudentResults
    PIVOT (
          count(DoctorId)
      FOR ['+@ColumnToPivot+']
      IN (
