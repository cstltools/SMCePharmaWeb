CREATE PROCEDURE [dbo].[sp_GET_DZSMwiseReportParam_ByProcess_Terri] 
...
--Cust Collection
LEFT JOIN (SELECT  t.TerritoryName,O.RegionId,t.TerritoryCode,sum((isnull(custDtl.TPAmount,0))) + sum((isnull(custDtl.VATAmount,0))) CustCollectionGross, 
sum((isnull(custDtl.TPAmount,0))) CollectionAmtTP,sum((isnull(custDtl.VATAmount,0))) CollectionVat
...
--inner join tblRegion R on O.RegionId = R.RegionId
WHERE  convert(date,custDtl.custPaymentDate) BETWEEN  @FromDate and @ToDate and 
  ((isnull(custDtl.PaymentAmount,0))) >0  and I.FinalPaymentNo is not null   
   GROUP BY t.TerritoryName,O.RegionId,t.TerritoryCode)tblCustCollection ON tblCustCollection.TerritoryCode=C.TerritoryCode 
...
AS T2 WHERE T1.AreaCode=T2.AreaCode) tblDelGreenCovSub  ON tblDelGreenCovSub.AreaCode = A.AreaCode  
 ...
 where  C.TerritoryId=@TeritorySelect
