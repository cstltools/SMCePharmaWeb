-- =============================================
-- =============================================
CREATE PROCEDURE [dbo].[sp_BusinessSummaryMISReport_TT] 
...
LEFT JOIN   (SELECT ComUnitId,COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,sum(NetAmount) SumofRCVAmount , 
...
WHERE   tblInvoice.DelivaryInvoiceNo IS NULL AND TpGrandTotal>0 AND  InvoiceDate BETWEEN @fromdate AND @todate   GROUP BY  ComUnitId)tblRcv ON tblRcv.ComUnitId=C.ComUnitId  
...
SELECT    ISNULL(tblRcv.SumofRCVAmount,0)- ISNULL(tblRcv.SumofRCVVat,0)  AS SumofRCVTP, 
...
-- NetSalesAmt--12
 ...
 ,ISNULL(tblCollection.TpTotal,0)+ISNULL(tblCollectionold.TpTotal,0) AS SumofNetSalesAmount--10
,ISNULL(tblCollection.TotalVat,0)+ISNULL(tblCollectionold.TotalVat,0) AS DelTpVat --11

,(ISNULL(tblCollection.TotalNetPayable,0) )
+ ISNULL(tblCollectionold.TotalNetPayable,0)
 NetSalesAmt--12
...
,
 ISNULL(tblUnD.NumberofInvoiceun,0) AS NumberofUndelInvoice,
ISNULL(tblUnD.SumofNetUnAmount,0) AS SumofNetUnAmount,
ISNULL(tblUnD.UnTpVat,0) AS UnTpVat, C.RegionCode+' : '+
C.RegionName  ShortName, C.RegionCode  ,
ISNULL(tblA.NumberofProformaInvoice,0) AS NumberofProformaInvoice,
ISNULL(tblC.NumberofInvoiceSold,0)  AS NumberofInvoiceSold,
ISNULL(tblD.NumberofReturnInvoice,0)  AS NumberofReturnInvoice,
...
(((ISNULL(tblA.SumofNetProformaAmount,0) ) - (ISNULL(tblD.SumofNetReturnAmount,0) )) )-(ISNULL(tblCollection.TpTotal,0)+ISNULL(tblCollectionold.TpTotal,0))
AS Outstanding1	
,
((((ISNULL(tblA.ProTpVat,0) )-(ISNULL(tblD.TotalPriceVatAmount,0) ))) )-(ISNULL(tblCollection.TotalVat,0)+ISNULL(tblCollectionold.TotalVat,0))
AS Outstanding2

,
(((ISNULL(tblA.SumofNetProformaAmount,0)) - (ISNULL(tblD.SumofNetReturnAmount,0) ) +
((ISNULL(tblA.ProTpVat,0) )-(ISNULL(tblD.TotalPriceVatAmount,0) )))) -((ISNULL(tblCollection.TpTotal,0)+ISNULL(tblCollectionold.TpTotal,0) )
+ ISNULL(tblCollection.TotalVat,0)+ISNULL(tblCollectionold.TotalVat,0))
AS Outstanding3
...
WHERE DeliveryInvoiceStatus IN  ('Partial','Full') AND TpGrandTotal>0 AND  tblInvoice.UpdateDate BETWEEN @fromdate AND @todate   GROUP BY  RegionId
 ...
 LEFT JOIN   (SELECT  O.RegionId, COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,sum(NetAmount) SumofRCVAmount , 
Sum(TotalPriceVatAmount)SumofRCVVat FROM dbo.tblInvoice  WITH (NOLOCK)  INNER JOIN dbo.tblInvoiceDetail D WITH (NOLOCK) ON tblInvoice.InvoiceId = D.InvoiceId   
inner join tblOrder O WITH (NOLOCK)  ON O.OrderId = tblInvoice.OrderId  
WHERE   tblInvoice.DelivaryInvoiceNo IS NULL AND TpGrandTotal>0 AND  InvoiceDate BETWEEN @fromdate AND @todate   GROUP BY   O.RegionId)tblRcv ON tblRcv.RegionId=C.RegionId  
...
where ID.DeliveryStatus IN ('Full','Partial')   AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate group by   O.RegionId)tblCollection ON tblCollection.RegionId = C.RegionId 
		...
		LEFT JOIN   ( select  I.RegionCode, sum(ID.DeliveryTotalPrice-ID.DeliveryDiscountAmount) TpTotal, sum(ID.DeliveryTotalPriceVatAmount)
TotalVat,  sum(ID.DeliveryNetAmount )  TotalNetPayable
...
INNER JOIN SalesDisDB_SMC..tblProduct P ON ID.ProductCode = P.ProductCode 
INNER JOIN SalesDisDB_SMC..tblProductSQ SQ ON P.ProductBrandId = SQ.ProductBrandId 
where ID.DeliveryStatus IN ('Full','Partial')   AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate  group by I.RegionCode )tblCollectionold ON tblCollectionold.RegionCode = C.RegionCode 
...
--where ID.DeliveryStatus IN ('Full','Partial')  AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate  group by  O.RegionId )tblCollectionoldsub ON C.RegionId = 13
 ...
 where   (C.RegionId= COALESCE( NULLIF(@ZoneId , 0) ,C.RegionId))  
...
WHERE DeliveryInvoiceStatus IN  ('Partial','Full') AND TpGrandTotal>0 AND  tblInvoice.UpdateDate BETWEEN @fromdate AND @todate   GROUP BY  AreaId
  ...
  LEFT JOIN   (SELECT O.AreaId, COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,sum(NetAmount) SumofRCVAmount , 
Sum(TotalPriceVatAmount)SumofRCVVat FROM dbo.tblInvoice  WITH (NOLOCK)  INNER JOIN dbo.tblInvoiceDetail D WITH (NOLOCK) ON tblInvoice.InvoiceId = D.InvoiceId   
inner join tblOrder O WITH (NOLOCK)  ON O.OrderId = tblInvoice.OrderId  
WHERE   tblInvoice.DelivaryInvoiceNo IS NULL AND TpGrandTotal>0 AND  InvoiceDate BETWEEN @fromdate AND @todate   GROUP BY   O.AreaId)tblRcv ON tblRcv.AreaId=C.AreaId   
...
--where ID.DeliveryStatus IN ('Full','Partial')  AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate  group by  O.RegionId )tblCollectionoldsub ON C.RegionId = 13
 ...
 and   (C.AreaId= COALESCE( NULLIF(@Area , 0) ,C.AreaId))  
...
--where ID.DeliveryStatus IN ('Full','Partial')  AND CONVERT(date,I.UpdateDate)  BETWEEN @fromdate AND @todate  group by  O.RegionId )tblCollectionoldsub ON C.RegionId = 13
 ...
 and   (C.AreaId= COALESCE( NULLIF(@Area , 0) ,C.AreaId))  and  (C.RegionId= COALESCE( NULLIF(@ZoneId , 0) ,C.RegionId))
ORDER BY  C.AreaCode
...
and
  ...
  (C.AreaId= COALESCE( NULLIF(@Area , 0) ,C.AreaId))  
...
and
  ...
  (C.AreaId= COALESCE( NULLIF(@Area , 0) ,C.AreaId))   and (C.RegionId= COALESCE( NULLIF(@ZoneId , 0) ,C.RegionId))
...
WHERE I.TpGrandTotal>0 AND I.InvoiceDate BETWEEN @fromdate AND @todate  GROUP BY  O.TerritoryId)tblA ON tblA.TerritoryId=C.TerritoryId
  ...
  LEFT JOIN   (SELECT O.TerritoryId, COUNT(DISTINCT tblInvoice.InvoiceId)NumberofInvoiceSold,sum(NetAmount) SumofRCVAmount , 
Sum(TotalPriceVatAmount)SumofRCVVat FROM dbo.tblInvoice  WITH (NOLOCK)  INNER JOIN dbo.tblInvoiceDetail D WITH (NOLOCK) ON tblInvoice.InvoiceId = D.InvoiceId   
inner join tblOrder O WITH (NOLOCK)  ON O.OrderId = tblInvoice.OrderId  
WHERE   tblInvoice.DelivaryInvoiceNo IS NULL AND TpGrandTotal>0 AND  InvoiceDate BETWEEN @fromdate AND @todate   GROUP BY   O.TerritoryId)tblRcv ON tblRcv.TerritoryId=C.TerritoryId   
...
and
  ...
  (C.TerritoryId= COALESCE( NULLIF(@Terr , 0) ,C.TerritoryId))  
 
  and  r.RegionId = COALESCE( NULLIF(@ZoneId , 0) ,r.RegionId)  
