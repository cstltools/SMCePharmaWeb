...
CREATE PROCEDURE [dbo].[sp_Webapi_Get_EmployyeMonthlyExpense]
...
AS
BEGIN 
SELECT   DAY(MainDate_) date,  EmpMas.EmpInfoId, EmpMas.EmpMasterCode, EmpMas.EmpName, dgs.DesigName, case when NSM.GroupName is not null then NSM.GroupName when RSM.RegionName is not null then  RSM.RegionName when ASM.AreaName is not null then  ASM.AreaName  else MIO.TerritoryName end  BaseHQ , case when NSM.GroupCode is not null then NSM.GroupCode when RSM.RegionCode is not null then  RSM.RegionCode when ASM.AreaCode is not null then  ASM.AreaCode   else MIO.TerritoryCode end MarketCode,vv.RegionCode ZoneCode, TadaDateMonthYear MonthYear, CAST(tblDA.TadaDate as nvarchar(max))+ ', '+tblDA.DayNameVal DAdate,  '' MarketName,tblDA.TourType MarketTourType, floor( ISNULL(tblDA.DAAmount,0)) daAmount , ISNULL(tblMilag.AllowedMileageInKM,0) AllowedMileageInKM, ISNULL(tblMilag.MilageExpense,0) mileageAmount, floor( ISNULL(tblExpense.ExpenseAmount,0)) expenseAmount, ISNULL(tblDA.DAAmount,0)  + ISNULL(tblMilag.MilageExpense,0) +ISNULL(tblExpense.ExpenseAmount,0) totalAmount FROM dbo.tblEmpGeneralInfo EmpMas  WITH (NOLOCK)
...
SELECT   distinct DAY(ExpenseDate_) date, EmpMas.EmpInfoId, EmpMas.EmpMasterCode, EmpMas.EmpName, dgs.DesigName, case when NSM.GroupName is not null then NSM.GroupName when RSM.RegionName is not null then  RSM.RegionName when ASM.AreaName is not null then  ASM.AreaName  else MIO.TerritoryName end  BaseHQ , case when NSM.GroupCode is not null then NSM.GroupCode when RSM.RegionCode is not null then  RSM.RegionCode when ASM.AreaCode is not null then  ASM.AreaCode   else MIO.TerritoryCode end MarketCode,vv.RegionCode ZoneCode, FORMAT(ExpenseDate_,'MMMM, yyyy') MonthYear, CAST( tblExpense.TadaDate as nvarchar(max))+ ', '+tblExpense.ExpenseMonthVal DAdate,   '' MarketName,'' MarketTourType, 0 daAmount , 0 AllowedMileageInKM, 0 mileageAmount, floor( ISNULL(tblExpense.ExpenseAmount,0)) expenseAmount, ISNULL(0,0)  + 0 +ISNULL(tblExpense.ExpenseAmount,0) totalAmount FROM dbo.tblEmpGeneralInfo EmpMas  WITH (NOLOCK)
  left join tblDesignation dgs   WITH (NOLOCK) on EmpMas.DesignationId=dgs.DesignationId


  LEFT JOIN (SELECT convert(Date,EMas.ExpenseDate) ExpenseDate_, DATENAME(DW,EMas.ExpenseDate) ExpenseMonthVal,  DAY(EMas.ExpenseDate) TadaDate,EMas.EmpInfoId, SUM(ISNULL(EMas.Amount,0)  )   ExpenseAmount, Month(EMas.ExpenseDate) ExpenseDateMont  ,Year(EMas.ExpenseDate)  ExpenseDateYear  FROM dbo.tbl_ExpenseClaim EMas  WITH (NOLOCK)
 inner join View_Webapi_EmployeeFieldForceInfo   on EMas.EmpInfoId=View_Webapi_EmployeeFieldForceInfo.EmpInfoId
 where  EMas.ExpenseClaimID is not null    and  EMas.ApprovalStatus='2'   and CONVERT(Date,EMas.ExpenseDate) not in (select * from  (select  DATEADD(dd,a.n-1,datefromparts(@Year,@Month,1)) mainDate  
from    (
	   ...
	   (select distinct convert(date,TadaDate)   from tbl_TadaClaimMaster  where MONTH(TadaDate)=@Month  and EmpInfoId in( select * from fnSplit(@EmpId,','))  ))    GROUP BY convert(Date,EMas.ExpenseDate), DATENAME(DW,EMas.ExpenseDate), EMas.EmpInfoId,DAY(EMas.ExpenseDate),Month(EMas.ExpenseDate)    ,Year(EMas.ExpenseDate))tblExpense    ON  tblExpense.EmpInfoId= EmpMas.EmpInfoId  and ExpenseDateMont=@Month and ExpenseDateYear=@Year 
  --da date join 
...
from    (
	   ...
	   where   DATEPART(mm,DATEADD(dd,a.n-1,datefromparts(@Year,@Month,1)))=@Month   ) tblx where convert(date,tblx.mainDate)  in 
	   (select distinct convert(date,MileageDate)   from tbl_MileageClaim  where MONTH(MileageDate)=@Month  and EmpInfoId  in( select * from fnSplit(@EmpId,','))  ))   and  milMas.ApprovalStatus='2'   GROUP BY   convert(Date,milMas.MileageDate), DATENAME(DW,milMas.MileageDate),  milMas.EmpInfoId,DAY(milMas.MileageDate),Month(milMas.MileageDate),Year(milMas.MileageDate))tblMilag    ON  tblMilag.EmpInfoId= EmpMas.EmpInfoId   and MonthMileageDate=@Month and YearMileageDate=@Year  
