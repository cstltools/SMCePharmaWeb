...
-- =============================================
CREATE PROCEDURE [dbo].[sp_ProformaReturnPosting] 
...
BEGIN
	...
	SELECT INV.InvoiceId,INV.InvoiceDate,UNT.CompanyId,SUM(((CTRS.UnitPrice*INVD.TotalQuantity) + (CTRS.VATPerUnit*INVD.TotalQuantity) )) AS TotalPrice,
	INV.InvoiceNo FROM tblInvoice AS INV
	...
	(   
	    ...
	    'Proforma Invoice Return ( Invoice No:' + @InvoiceNo + ' , Invoice Date:' + CONVERT(NVARCHAR(50), @InvDate, 101) + ', Stock Value:' + CONVERT(NVARCHAR(50), @CostPrice) + ')',       -- Narration - nvarchar(max)
	...
	DECLARE	@DrGl INT 
	SELECT @DrGl = GlId FROM ZAS_ACCDB..tblSalesIntegrationMaping WHERE Particular IN ('ProformaInvoice') AND BalanceType IN ('Dr')
	...
	DECLARE	@CrGl INT 
	SELECT @CrGl = GlId FROM ZAS_ACCDB..tblSalesIntegrationMaping WHERE Particular IN ('ProformaInvoice') AND BalanceType IN ('Cr')
