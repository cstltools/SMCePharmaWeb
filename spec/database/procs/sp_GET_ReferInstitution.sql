...
Create PROCEDURE [dbo].[sp_GET_ReferInstitution]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT
		InstitutionId,InstitutionName,InstitutionCode,CASE  WHEN  Entryemp.EmpName Is Null  THEN  us.UserName 
		...
		IsActive
		FROM tblReferInstitution AS DPT 
		...
		WHERE InstitutionId IS NOT NULL ' + @Parameter
