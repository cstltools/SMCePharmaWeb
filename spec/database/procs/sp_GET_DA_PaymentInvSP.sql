...
--------------------------------------------------
-- PROCEDURE: sp_GET_DA_PaymentInvSP
--------------------------------------------------

CREATE   PROCEDURE [dbo].[sp_GET_DA_PaymentInvSP]
    @param NVARCHAR(MAX) = N''
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SET @sql = N'
  SELECT    (SELECT LTRIM(RTRIM(ImagePath+''/''+ImagePreName)) FROM dbo.tbl_ImagePath_Setting  with (nolock)  WHERE ImageType=''PaymentCollectionLink'')+CAST(PLOG.PaymentCollectionAppLogId as nvarchar(max))+''.jpg'' AS   ImageLinkshow, PLOG.PaymentCollectionAppLogId, ord.ComUnitId, INV.CustomerMasterId, PLOG.PayableAmount  PayableAmount_DA,
       CUST.MarketId,
       ...
       PLOG.DaId AS DAId,
       PLOG.BankId,
       PAYDA.Name AS PaymentCollectedBy,
       PLOG.CreatedOn AS CollectedDate,
...
WHERE ISNULL(INV.DeliveryTpGrandTotal, 0) > 0
  ...
  AND ISNULL(PLOG.PaymentCollectionAppLogId, 0) >0 ' + ISNULL(@param, N'') + N'
