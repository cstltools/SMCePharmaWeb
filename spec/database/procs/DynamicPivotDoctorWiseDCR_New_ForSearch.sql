CREATE PROCEDURE [dbo].[DynamicPivotDoctorWiseDCR_New_ForSearch]
...
BEGIN
 ...
 select DoctorId,DcrDate ,[Doctor Code],  [Doctor Name], [Doctor Speciality],[Provider Type], [Pharma Platform], [Doctor Type],[Group],[Region],[Area],[Territory],[Market] from   View_DCR mas with (nolock)
  ...
  where  convert(date, mas.DcrDate)  between convert(date,'''+@Month+''') and convert(date,'''+@Year+''')   and mas.[Pharma Platform]= COALESCE( NULLIF('''+@PharmaPlatform+''' , '''') ,mas.[Pharma Platform] )   and mas.[Provider Type]= COALESCE( NULLIF('''+@ProviderType+''' , '''') ,mas.[Provider Type] )    and mas.RegionId= COALESCE( NULLIF('''+@ZoneSelect+''' , '''') ,mas.RegionId)   and mas.AreaId= COALESCE( NULLIF('''+@AreaSelect+''' , '''') ,mas.AreaId) and mas.TerritoryId= COALESCE( NULLIF('''+@TeritorySelect+''' , '''') ,mas.TerritoryId)   
    ) StudentResults
    PIVOT (
          count(DoctorId)
      FOR ['+@ColumnToPivot+']
