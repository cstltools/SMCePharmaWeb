...
CREATE PROCEDURE [dbo].[sp_GET_ProductLineList]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = 'SELECT CASE  WHEN  Entryemp.EmpName Is Null  THEN  us.UserName 
		...
		WHERE PC.ProductLineID IS NOT NULL ' + @Parameter
