CREATE PROCEDURE [dbo].[DynamicPivotBrandWiseDCR]
...
BEGIN
  ...
  SET @SqlStatement = N'
    SELECT * FROM (
     select  proBrnd.ProductSQName  [Brand Name],     CONVERT(date,mas.DcrDate)  DcrDate,  (ProductQty) ProductQty   from tbl_DcrDetails dcrDtl    WITH (NOLOCK) 
