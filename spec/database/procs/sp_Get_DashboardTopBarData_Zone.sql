CREATE PROCEDURE [dbo].[sp_Get_DashboardTopBarData_Zone] 
...
set	@Prvious7Days=getdate()-7
        ...
        ----WHERE     convert(Date,AttendanceDate) = convert(Date,@currentDate) and tblMarketAttendance_Master_webapi.RegionId in( @ZoneId)
        ...
        --                         AND     @weekEndDate and  C.RegionId in( @ZoneId)
        ...
        --        AND DATEPART(MM, SubmissionDate) = @month  and  C.RegionId in( @ZoneId)



        SELECT  @orderCountToday = COUNT(*)
        FROM    dbo.tblOrder  with (nolock)
        WHERE tblOrder.ActionStatus<>'3' and      convert(Date,SubmissionDate) =convert(Date, @currentDate) and   RegionId in( @ZoneId)
  ...
  --      WHERE     DelivaryInvoiceNo IS NOT NULL
  --              AND DeliveryInvoiceStatus != 'Reject'
  --              AND convert(Date,SubmissionDate) = convert(Date,@currentDate)  and   C.RegionId in( @ZoneId)
  ...
  --      WHERE   A.ActionStatus<>'3' and convert(Date,SubmissionDate) = convert(Date,@currentDate) 
		...
		--		AND A.OrderType ='Regular'  and   A.RegionId in( @ZoneId)
  ...
  --      WHERE   convert(Date,A.SubmissionDate) = convert(Date,@currentDate)
		...
		-- where tbl_DCRInfo.ApprovalStatus<>'3' and  convert(Date,DcrDate) = convert(Date,@currentDate)    and   tbl_DCRInfo.RegionId in( @ZoneId)

		--	SELECT @totalDcrNonGMP = ISNULL(COUNT(*),0) FROM dbo.tbl_DCRInfo  with (nolock)
		--inner JOIN dbo.tblDoctorMaster  with (nolock) ON dbo.tblDoctorMaster.DoctorId=dbo.tbl_DCRInfo.DoctorId WHERE tblDoctorMaster.DoctorTypeId=1 and tbl_DCRInfo.ApprovalStatus<>'3' and  convert(Date,DcrDate) = convert(Date,@currentDate)  and   tbl_DCRInfo.RegionId in( @ZoneId)  

		 

		----SELECT @TotalRX = ISNULL(COUNT(*),0) FROM dbo.tbl_PrescriptionMaster  with (nolock)   WHERE  tbl_PrescriptionMaster.ApprovalStatus<>'3' and  convert(Date,PrescriptionDate) = convert(Date,@currentDate)    and   tbl_PrescriptionMaster.RegionId in( @ZoneId)

		
		--SELECT @TotalRXGMP = ISNULL(COUNT(*),0) FROM dbo.tbl_PrescriptionMaster  with (nolock) inner JOIN dbo.tblDoctorMaster  with (nolock) ON dbo.tblDoctorMaster.DoctorId=dbo.tbl_PrescriptionMaster.DoctorId WHERE tblDoctorMaster.DoctorTypeId=2 and tbl_PrescriptionMaster.ApprovalStatus<>'3' and  convert(Date,PrescriptionDate) = convert(Date,@currentDate)     and   tbl_PrescriptionMaster.RegionId in( @ZoneId)
		...
		--		SELECT @TotalRXNONGMP = ISNULL(COUNT(*),0) FROM dbo.tbl_PrescriptionMaster  with (nolock) inner JOIN dbo.tblDoctorMaster  with (nolock) ON dbo.tblDoctorMaster.DoctorId=dbo.tbl_PrescriptionMaster.DoctorId WHERE tblDoctorMaster.DoctorTypeId=1 and tblDoctorMaster.ApprovalStatus<>'3' and  convert(Date,PrescriptionDate) = convert(Date,@currentDate)     and   tbl_PrescriptionMaster.RegionId in( @ZoneId)
		 	...
		 	--SELECT @TotalAttandence = ISNULL(COUNT(*),0) FROM tblMarketAttendance_Master_webapi  with (nolock) WHERE  tblMarketAttendance_Master_webapi.AttType=1 and tblMarketAttendance_Master_webapi.ApprovalStatus<>'3' and convert(Date,AttendanceDate) = convert(Date,@currentDate)    and   tblMarketAttendance_Master_webapi.RegionId in( @ZoneId)
			
			 --	SELECT @TotalAttandenceOut = ISNULL(COUNT(*),0) FROM tblMarketAttendance_Master_webapi  with (nolock) WHERE  tblMarketAttendance_Master_webapi.AttType=2 and tblMarketAttendance_Master_webapi.ApprovalStatus<>'3' and convert(Date,AttendanceDate) = convert(Date,@currentDate)    and   tblMarketAttendance_Master_webapi.RegionId in( @ZoneId)
 

			
			   

			    	--SELECT   @totalCustomerCoverage = ISNULL(COUNT( distinct CustomerMasterId ),0) FROM tblOrder  with (nolock)   WHERE convert(Date,SubmissionDate) = convert(Date,@currentDate)    and   tblOrder.RegionId in( @ZoneId)
        ...
        WHERE    A.ActionStatus<>'3' and convert(Date,A.SubmissionDate) = convert(Date,@currentDate)    and   A.RegionId in( @ZoneId)


		 SELECT  @totalInvoice =ISNULL(sum(A.TpTotal),0)
        FROM    dbo.tblInvoice A  with (nolock)
		 inner join  dbo.tblOrder ord on A.OrderId=ord.OrderId
        WHERE   convert(Date,A.InvoiceDate) = convert(Date,@currentDate)    and   ord.RegionId in( @ZoneId)
...
INNER JOIN dbo.tblOrder mas  with(nolock) ON I.OrderId = mas.OrderId
 ...
 where      	  convert(date,custDtl.custPaymentDate)= convert(Date,getdate())
and ((isnull(custDtl.PaymentAmount,0))) >0  and FinalPaymentNo is not null and  I.PaymentStatus  IN ('Full','Partial') and   mas.RegionId in( @ZoneId)
 


	 

--			 SELECT  @totalPayment =ISNULL(sum(A.PaymentAmount),0)
--        FROM    dbo.tblInvoice A  with (nolock)
--		  	 inner join  dbo.tblOrder ord on A.OrderId=ord.OrderId
--        WHERE   convert(Date,A.UpdateDate) = convert(Date,@currentDate)     and   ord.RegionId in( @ZoneId)
...
--                        WHERE Status='Undelivered' AND I.InvoiceNo IS NOT NULL and   convert(Date,i.InvoiceDate) = convert(Date,@currentDate)     and   O.RegionId in( @ZoneId)
...
SELECT  @RejectGrossAmt= ISNULL(SUM(D.NetAmount - D.TotalPriceVatAmount) + 
SUM(D.TotalPriceVatAmount),0)   FROM dbo.tblRejectionInvoiceMaster rejMas  WITH (NOLOCK) 
...
WHERE   convert(Date,RejectionDate) =convert(Date,@currentDate)  and   mas.RegionId in( @ZoneId)



----SELECT  @PartialRejectGrossAmt=SUM(ID.NetAmount-ID.DeliveryNetAmount)-SUM(ID.TotalPriceVatAmount-ID.DeliveryTotalPriceVatAmount) 
...
----where RejectionSts is null  and convert(Date,I.UpdateDate) =convert(Date,@currentDate) and DelivaryInvoiceNo is not null and ID.DeliveryStatus IN ('Reject','Partial')   and   mas.RegionId in( @ZoneId)
...
--        WHERE   convert(Date,A.SubmissionDate) between  convert(Date, @Prvious7Days) and convert(Date,@PrviousDay)   and   A.RegionId in( @ZoneId)
...
--        FROM    dbo.tblInvoice A  with (nolock)
--		  inner join  dbo.tblOrder ord on A.OrderId=ord.OrderId
--        WHERE   convert(Date,A.InvoiceDate) between  convert(Date, @Prvious7Days) and convert(Date,@PrviousDay)     and   ord.RegionId in( @ZoneId)
...
--		set @TotalInvoicerPer= 0 --cast (((@totalInvoiceAVG - @totalInvoice)/((@totalInvoiceAVG + @totalInvoice)/2) )*100 as decimal(18,2))
        ...
        SELECT  ISNULL(@puninTime, 0) AS PunchInTime ,
				...
				ISNULL(@totalDelivery,0) AS  totalDelivery,
