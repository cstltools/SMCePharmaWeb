...
CREATE PROCEDURE [dbo].[sp_Webapi_Check_ProductActiveorNotEmpId]
...
WHERE C.ProductGroupId=1 AND C.IsActive=1 AND D.IsActive=1     AND MKT.IsActive=1  AND tr.IsActive=1  AND terry.IsActive=1  AND ar.IsActive=1    AND rg.IsActive=1    AND gp.IsActive=1  and   C.ProductId='+@ProductId
 --and DC.ComUnitId=12
...
SELECT   @Id=  STUFF( (SELECT CONCAT(',', mm.EmpAreaId , '')   FROM View_Webapi_EmployeeFieldForceInfo mm (NOLOCK)   WHERE EmpInfoId=@empId   FOR XML PATH ('') ),1,1,'')  
	...
	BEGIN
	    ...
	    SET @Param=@Param+' and  gp.GroupId='+@Id
	END
