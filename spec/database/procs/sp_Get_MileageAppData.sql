...
-- =============================================
CREATE PROCEDURE [dbo].[sp_Get_MileageAppData]
...
AS
	...
	BEGIN
	   SET @params=@params+'  '
		...
		BEGIN
		    SET @params=@params+ ' AND Convert(Date,tblMileageApprovalLog.EntryDateApp)='''+@FromDt+''''
		END
		IF(@FromDt IS NOT NULL AND @ToDt IS NOT NULL)
		BEGIN
		    SET @params=@params+ ' AND Convert(Date,tblMileageApprovalLog.EntryDateApp) between '''+@FromDt+''' AND '''+@ToDt+''' '
		END
	END
	ELSE
    BEGIN
	...
	SELECT  RT.RoleType WaitingForRole, case when tbl_MileageClaim.ApprovalStatus=''0'' then ''Pending''  when tbl_MileageClaim.ApprovalStatus=''1'' then ''Verified'' when tbl_MileageClaim.ApprovalStatus=''2'' then ''Approved'' when tbl_MileageClaim.ApprovalStatus=''3'' then ''Rejected''  else tbl_MileageClaim.ApprovalStatus end ApprovalStatusWeb,  tbl_MileageClaim.Remarks, tbl_MileageClaim.MeterReading, MileageClaimId, mr.MarketName,  t.TransportName, tbl_MileageClaim.AllowedMileageInKM AllowedMilagePerKM,tbl_MileageClaim.MileageInKM , tbl_MileageClaim.AllowedMileageInKM*tbl_MileageClaim.MileageInKM MileageAmount,
       ...
       tblMileageApprovalLog.RoleTypeId,ToRoleTypeId,
...
LEFT JOIN dbo.tblRoleType RT  with (nolock) ON RT.RoleTypeId = tblMileageApprovalLog.ToRoleTypeId
WHERE tbl_MileageClaim.MileageClaimId is not null '+@params+'  AND   tblRoleType.RoleType<>'''+@Role+''' AND Step=LogMax.MaxStep '+@param +' ) tbl'
