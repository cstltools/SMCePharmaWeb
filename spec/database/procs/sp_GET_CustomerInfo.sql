...
CREATE PROCEDURE [dbo].[sp_GET_CustomerInfo]
...
AS
	...
	--	WHERE DeptId IS NOT NULL' + @Parameter
		...
		SET @Query = '		SELECT  cc.CustomerCategory,  IsOrderApproval IsCampaign, DPT.CustomerTypeId,
		...
		DPT.CustomerType, DPT.CustTypeCode,
		IsActive,*
		FROM tblCustomerType AS DPT 
		LEFT JOIN tblUser us ON us.UserId = DPT.EntryBy
		LEFT JOIN tblCustomerCategory cc ON cc.CustomerCategoryId = DPT.CustomerCategoryId
		...
		WHERE DPT.CustomerTypeId IS NOT NULL ' + @Parameter
