...
CREATE FUNCTION [dbo].[MainMenu]
(
    @UserId INT

	 --CASE WHEN URL='#'  then ISNULL(URL,'') WHEN SystemLink='MVC' THEN  ISNULL('http://45.64.134.85:580'+URL,'') WHEN SystemLink='MVC' THEN  ISNULL('http://45.64.134.85:261'+URL,'') end AS URL
)
...
BEGIN
    ...
    FOR
	SELECT ROW,ManuName,URL,Class,Icon,ParantId,SL FROM (select (ROW_NUMBER() OVER (ORDER BY dbo.tblMainMenuNew.SL)) AS ROW,ISNULL(ManuName,'') AS ManuName,   ISNULL(URL,'')   AS URL,ISNULL(Class,'') AS Class,ISNULL(Icon,'') AS Icon,ISNULL(ParantId,'') AS ParantId,SL FROM dbo.tblMainMenuNew) AS TblTemp where ParantId is null or ParantId='' order by SL ASC
	END
	ELSE
	BEGIN
	...
	select '1',ManuName,URL,Class,Icon,ParantId,dbo.tblMainMenuNew.SL from tblMainMenuNew 
                                     ...
                                     WHERE UserId=@UserId AND (ParantId is null or ParantId='') order by tblMainMenuNew.SL asc
    ...
    BEGIN
		...
		'<li class="nav-item" >'+
			'<a class="nav-link collapsed" data-toggle="collapse" href="#a'+@SL+'" role="button" aria-expanded="false" aria-controls="#a'+@SL+'">
			'+
				'<i class="'+@Icon+'"></i>
				<span class="link-title">'+@MainMenuName+'</span><i class="link-arrow" data-feather="chevron-down"></i></a>
				<div class="collapse" id="a'+@SL+'" style="">'
				...
				INNER JOIN dbo.tblMenuDistribution ON dbo.tblMainMenuNew.SL = dbo.tblMenuDistribution.MenuSL 
				WHERE UserId=@UserId AND ParantId=@SL ) AS TblTempa 
				END
				ELSE
				BEGIN
				...
				IF(@SubCount>0)
				BEGIN
					SET @MenuHTML=@MenuHTML+'<ul class="nav sub-menu">'	
				...
				BEGIN
					...
					'<li class="nav-item">'+
						...
						'<a class="nav-link" href="'+@SubURL+'">
							<i class="'+@SubIcon+'"></i>'+@SubMenuName+'</a>'
					END
					ELSE
					BEGIN
						SET @MenuHTML=@MenuHTML+
				...
				IF(@UserId<>'1')
				BEGIN
				SELECT @SubSubCount=COUNT(*) from tblMainMenuNew 
                                    INNER JOIN dbo.tblMenuDistribution ON dbo.tblMainMenuNew.SL = dbo.tblMenuDistribution.MenuSL 
                                    WHERE UserId=@UserId AND ParantId=@SubSL 
				END
				ELSE
				BEGIN
				...
				select '1',ISNULL(ManuName,'') AS ManuName,ISNULL(URL,'') AS URL,ISNULL(Class,'') AS Class,ISNULL(Icon,'') AS Icon,ISNULL(ParantId,'') AS ParantId,dbo.tblMainMenuNew.SL from tblMainMenuNew 
                                    INNER JOIN dbo.tblMenuDistribution ON dbo.tblMainMenuNew.SL = dbo.tblMenuDistribution.MenuSL 
                                    WHERE UserId=@UserId AND ParantId=@SubSL order by tblMainMenuNew.SL asc
				...
				IF(@UserId<>'1')
				BEGIN
				SELECT @SubSubSubCount=COUNT(*) from tblMainMenuNew 
                                    INNER JOIN dbo.tblMenuDistribution ON dbo.tblMainMenuNew.SL = dbo.tblMenuDistribution.MenuSL 
				...
				select '1',ISNULL(ManuName,'') AS ManuName,ISNULL(URL,'') AS URL,ISNULL(Class,'') AS Class,ISNULL(Icon,'') AS Icon,ISNULL(ParantId,'') AS ParantId,dbo.tblMainMenuNew.SL from tblMainMenuNew 
                                    INNER JOIN dbo.tblMenuDistribution ON dbo.tblMainMenuNew.SL = dbo.tblMenuDistribution.MenuSL 
                                    WHERE UserId=@UserId AND ParantId=@SubSubSL order by tblMainMenuNew.SL asc
