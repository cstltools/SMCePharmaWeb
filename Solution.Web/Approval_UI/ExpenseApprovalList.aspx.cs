...
public partial class Approval_UI_ExpenseApprovalList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    using (DataTable dt = _dataLoad.GetExpenseType())
                    {
                        ddlExpesneType.DataSource = dt;
                        ddlExpesneType.DataValueField = "ExpenseTypeId";
                        ddlExpesneType.DataTextField = "ExpenseTypeName";
                        ddlExpesneType.DataBind();
                        ddlExpesneType.Items.Insert(0, new ListItem("Please Select From List", String.Empty));
                        ddlExpesneType.SelectedIndex = 0;
                    }
    ...
    private void LoadData()
    {
        ...
        if (EmpInfoId != "" || EmpInfoId != null)
        {
            ...
            switch (RoleTypeName)
            {
                case "AM":
                    ...
                    pram = " AND View_Webapi_EmployeeFieldForceInfo.EmpAreaId=" + FFID + "  and (DATEDIFF(DAY,CONVERT(DATE,mas.ExpenseDate),CONVERT(DATE,GETDATE())))<=7   ";
                    Role = "AM";

                    break;
                case "DZSM":
                    FFID = dtMarket.Rows[0]["EmpRegionId"].ToString();
                    pram = " AND  View_Webapi_EmployeeFieldForceInfo.EmpRegionId=" + FFID + "   and (DATEDIFF(DAY,CONVERT(DATE,mas.ExpenseDate),CONVERT(DATE,GETDATE())))<=7  ";
                    Role = "DZSM";
                    break;
                case "NSM":
                    FFID = dtMarket.Rows[0]["EmpGroupId"].ToString();
                    pram = " AND  View_Webapi_EmployeeFieldForceInfo.EmpGroupId=" + FFID + "   and (DATEDIFF(DAY,CONVERT(DATE,mas.ExpenseDate),CONVERT(DATE,GETDATE())))<=7  ";
                    Role = "NSM";
                    break;
        ...
        if (ddlExpesneType.SelectedValue != "")
        {

            pram = pram + " AND mas.ExpenseTypeId= " + ddlExpesneType.SelectedValue + " ";
