        ...
        if (EmpInfoId != "" || EmpInfoId != null)
        {
            ...
            switch (RoleTypeName)
            {
                case "AM":
                    ...
                    pram = " AND View_Webapi_EmployeeFieldForceInfo.EmpAreaId=" + FFID + "  and (DATEDIFF(DAY,CONVERT(DATE,tbl_MileageClaim.MileageDate),CONVERT(DATE,GETDATE())))<=7   ";
                    Role = "AM";

                    break;
                case "DZSM":
                    FFID = dtMarket.Rows[0]["EmpRegionId"].ToString();
                    pram = " AND  View_Webapi_EmployeeFieldForceInfo.EmpRegionId=" + FFID + "  and (DATEDIFF(DAY,CONVERT(DATE,tbl_MileageClaim.MileageDate),CONVERT(DATE,GETDATE())))<=7   ";
                    Role = "DZSM";
                    break;
                case "NSM":
                    FFID = dtMarket.Rows[0]["EmpGroupId"].ToString();
                    pram = " AND  View_Webapi_EmployeeFieldForceInfo.EmpGroupId=" + FFID + "  and (DATEDIFF(DAY,CONVERT(DATE,tbl_MileageClaim.MileageDate),CONVERT(DATE,GETDATE())))<=7   " ;
                    Role = "NSM";
