...
public partial class MasterSetup_UI_CampaignSetup : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                rbPaymentOptions.Items[2].Selected = true;
    ...
    private void GetOneRecord(string Id)
    {
        try
        {
            using (DataTable dt = _BonusCampaignNewDAL.GetCampaignSetupById(Id))
            {
               ...
               string CustomerTypeIdNew = dt.Rows[0]["CustomerTypeIdNew"].ToString();
                if (CustomerTypeIdNew == "")
                {
                ...
                else
                {
                    ...
                    string[] degree = CustomerTypeIdNew.ToString().Split(',');

                    foreach (System.Web.UI.WebControls.ListItem item in ddlChemistType.Items)
                    {
                ...
                bool _IsPTforCOD = false;
                try
                {
                    _IsPTforCOD = Convert.ToBoolean(dt.Rows[0]["IsPTforCOD"].ToString());
                }
                catch (Exception ex)
                {
                ...
                if (_IsPTforCOD)
                {
                    rbPaymentOptions.Items[0].Selected = true;
                }


                bool _IsPTforOther = false;
                try
                {
                    _IsPTforOther = Convert.ToBoolean(dt.Rows[0]["IsPTforOther"].ToString());
                }
                catch (Exception ex)
                {
                ...
                if (_IsPTforOther)
                {
                    rbPaymentOptions.Items[1].Selected = true;
                }

                if(_IsPTforOther && _IsPTforCOD)
                {
                    rbPaymentOptions.Items[2].Selected = true;
                }
    ...
    private void LoadInitialInfo()
    {
        ...
        try
        {
            using (DataTable dt = _seedRepo.GetProLineDataTableList())
            {
                ...
                ddlProLine.DataBind();
                ddlProLine.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select From List", String.Empty));
        ...
        try
        {
            using (DataTable dt = _seedRepo.GetCampaignTypeList())
            {
                ...
                ddlCampaignType.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select From List", String.Empty));
        ...
        try
        {
            using (DataTable dt = _seedRepo.GetCampaignCategory())
            {
                ...
                ddlCampaignCategory.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select From List", String.Empty));
        ...
        try
        {
            using (DataTable dt = _BonusCampaignNewDAL.GetProductForDDL())
            {
                ...
                ddlProduct.DataBind();
                ddlProduct.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select From List", String.Empty));
        ...
        try
        {
            using (DataTable dt = _Dal.GetProductListActiveForALl())
            {
                ...
                ddlProductList.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select From List", String.Empty));
        ...
        if (ddlCampaignType.SelectedIndex > 0)
        {
            try
            {
                using (DataTable dt = _seedRepo.GetOfferTypeInfo(Convert.ToInt32(ddlCampaignType.SelectedValue)))
                {
                    ...
                    ddlOfferType.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select From List", String.Empty));
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (Validation())
            {
                ...
                Session["CampgainMasterMapId"] = null;


                try
                {
                    using (DataTable dt = _BonusCampaignNewDAL.GetCampaignSetupMapById("4"))
                    {
                        Session["CampgainMasterMapId"] = dt.Rows[0]["BonusCampaignDetailsCustTypeId"].ToString();
                ...
                foreach (System.Web.UI.WebControls.ListItem item in ddlChemistType.Items)
                {
                    if (item.Selected)
                    {
                        ...
                        aMaster.CustomerTypeId = Convert.ToInt32(item.Value);
                        ...
                        aMaster.IsPTforCOD = false;
                        aMaster.IsPTforOther = false;

                        if (rbPaymentOptions.Items[0].Selected == true)
                        {
                            aMaster.IsPTforCOD = true;
                        }

                        if (rbPaymentOptions.Items[1].Selected == true)
                        {
                            aMaster.IsPTforOther = true;

                        }

                        if (rbPaymentOptions.Items[2].Selected == true)
                        {
                        ...
                        Res = _BonusCampaignNewDAL.SaveBonusCampaign(aMaster, DtlList, MarketList, DtlCustList, Session["UserId"].ToString());
                ...
                if (Res.isSuccess == true)
                {
                    ...
                    try
                    {
                        masIID = Convert.ToInt32(id_mastetID.Value);
                    }
                    catch
                    {
                    ...
                    if (masIID > 0)
                    {
                        mail.Body = "Salauddin Bhai Update <br> Campaign Name:" + txtCampaignName.Text + "<br> Campaign Type:" + ddlCampaignType.SelectedItem.Text + "<br>Date Range: " + frmDate.Text + " to " + toDate.Text;
                    }
                    else
                    {
                        mail.Body = "Salauddin Bhai Save <br> Campaign Name:" + txtCampaignName.Text + "<br> Campaign Type:" + ddlCampaignType.SelectedItem.Text + "<br>Date Range: " + frmDate.Text + " to " + toDate.Text;
                    }
        ...
        catch (Exception ex)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup",
                "faildalert('" + ex.Message + "','Faild');", true);
        }
