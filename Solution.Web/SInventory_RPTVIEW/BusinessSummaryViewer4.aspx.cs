...
public partial class SInventory_RPTVIEW_BusinessSummaryViewer4 : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        if (Branch != "")
        {
            ...
            DataTable mainDataTable = aTotalSummaryDAL.MarketBranchwiseTotalProformaBLL(FromDate, ToDate, Branch).Copy();

            DataSet Ds = new DataSet();

            mainDataTable.TableName = "MarketwiseSummaryDS";
            Ds.Tables.Add(mainDataTable);
            rptdoc.Load(ReportPath("rptMarketwiseBusinessSummary.rpt"));
            rptdoc.SetDataSource(Ds);
            rptdoc.ExportToHttpResponse(ExportFormatType.ExcelRecord, Response, true,
               "MarketwiseBusinessSummary_Report");
        }
        else
        {
            DateTime FromDate = Convert.ToDateTime(fromDate);
            ...
            DataTable mainDataTable = aTotalSummaryDAL.MarketBranchwiseTotalProformaBLL(FromDate, ToDate).Copy();
