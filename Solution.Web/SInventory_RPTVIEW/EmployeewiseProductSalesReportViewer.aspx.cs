...
public partial class SInventory_RPTVIEW_EmployeewiseProductSalesReportViewer : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        DataTable monitoringInfo = aStockMonitoringBll.EmployeewiseProductSales().Copy();

        if (monitoringInfo.Rows.Count > 0)
        {
            var ds = new DataSet();

            monitoringInfo.TableName = "EmployeewiseProductSalesDataTable";
            ds.Tables.Add(monitoringInfo);

            if (reportType == "excel")
            {
                rptdoc.Load(ReportPath("crpEmployeewiseProductSales.rpt"));
