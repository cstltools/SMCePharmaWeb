...
public partial class SInventory_RPTVIEW_DayWiseBusinessReportViewer : System.Web.UI.Page
{
   
    ReportDocument rptdoc = new ReportDocument();
   DayWiseBusinessReportBll aBusinessReportBll = new DayWiseBusinessReportBll();
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        try
        {
            DataTable salesTrendInfo = aBusinessReportBll.LoadDayWiseBusinessInfo(fromDate, toDate).Copy();

            if (salesTrendInfo.Rows.Count > 0)
            {
                var ds = new DataSet();

                salesTrendInfo.TableName = "DayWiseBusinessDataTable";
                ...
                if (reportType == "CRP")
                {
                    rptdoc.Load(ReportPath("crpDayWiseBusinessReport.rpt"));
                    rptdoc.SetDataSource(ds);
