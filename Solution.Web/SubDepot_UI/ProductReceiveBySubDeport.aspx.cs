...
public partial class SubDepot_UI_ProductReceiveBySubDeport : System.Web.UI.Page
{
    ...
    private void LoadInfo(string reqId)
    {
        ...
        hdComUnitId.Value = aTable.Rows[0]["SubDepotId"].ToString();
    }
    ...
    private void Save()
    {
        ...
        for (int i = 0; i < rcvGridView.Rows.Count; i++)
        {
            ...
            DCStockNew aDcStockNew = new DCStockNew()
            {
                ...
                mfgdate = Convert.ToDateTime(rcvGridView.Rows[i].Cells[10].Text),
                ...
                DCStoreId = Convert.ToInt32(rcvGridView.DataKeys[i][2].ToString()),
            };
            int SubDCStoreId = aChalanBll.DCStockIn2(aDcStockNew);
            if (Convert.ToDecimal(damageTextBox.Text.Trim()) > 0)
            {
                DCStoreFreezeDAO aDCStoreFreezeDAO = new DCStoreFreezeDAO();
                aDCStoreFreezeDAO.DCStoreId = SubDCStoreId;
    ...
    {
        string url = "../SInventory_RPTVIEW/SubDeportTransferStockReceiveReportViewer.aspx?ChalanId=" + reqId;
    ...
    {
        Response.Redirect("StockReceiveBySubDepot.aspx");
    }
}