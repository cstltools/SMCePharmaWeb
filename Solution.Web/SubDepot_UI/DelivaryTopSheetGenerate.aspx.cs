...
public partial class SubDepot_UI_DelivaryTopSheetGenerate : System.Web.UI.Page
{
    ...
    protected void SearchButton_Click(object sender, EventArgs e)
    {
        if (dcDropDownList1.SelectedValue != "" && manufacturerDropDownList.SelectedValue != "" && MarketDropDownList1.SelectedValue != "" && InvoiceDateTextBox.Text.Trim() != "")
        {
            ...
            dt = aOrderInfoBll.LoadInvoiceSUbdeport((dcDropDownList1.SelectedValue), (manufacturerDropDownList.SelectedValue), (MarketDropDownList1.SelectedValue), Convert.ToDateTime(dateAndTime2));
    ...
    protected void gotoinvoiceButton_Click(object sender, EventArgs e)
    {
        ...
        string url = "../SInventory_RPTVIEW/SubDeportDelivaryInvoiceReturnViewer.aspx?InvNo=" + Server.UrlEncode(loadGridView.DataKeys[rowindex]["DelivaryInvoiceNo"].ToString());
    ...
    protected void opsheetButton_Click(object sender, EventArgs e)
    {
        ...
        string url = "../SInventory_RPTVIEW/SubDeportDelivaryTopSheetInvoiceReturnViewer.aspx?InvNo=";
        // string fullURL = "window.open('" + url + "', '_blank', 'height=600,width=900,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=no,titlebar=no' );";
        string fullURL = "var Mleft = (screen.width/2)-(950/2);var Mtop = (screen.height/2)-(700/2);window.open( '" + url + "', null, 'height=700,width=950,status=yes,toolbar=no,addressbar=no, scrollbars=yes,menubar=no,location=no,top=\'+Mtop+\', left=\'+Mleft+\'' );";
        ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_WINDOW", fullURL, true);
    }
