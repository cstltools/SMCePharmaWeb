...
public partial class DoctorModule_UI_ExpenseType : System.Web.UI.Page
{
    ...
    private void GetOneRecord(string Id)
    {
        try
        {
            using (DataTable dt = _setupDAL.GetEditDataForId(Convert.ToInt32(Id)))
            {
                ...
                try
                {
                    chkFixed.Checked = Convert.ToBoolean(dt.Rows[0]["isFixed"].ToString());
                    chkFixed_CheckedChanged(null, null);
                ...
                try
                {
                    ddlRoleType.SelectedValue =  (dt.Rows[0]["RoleType_xp"].ToString());
                }
                catch (Exception ex)
                {
            ...
            {
                ViewState["CurrentTable"] = dtDetail;
    ...
    public bool Validation()
    {
        ...
        ddlRoleType.CssClass = "form-select form-select-sm mb-3 multiple-select";
        EmployeeIdSelect.CssClass = "form-select form-select-sm mb-3 multiple-select";
        ...
        foreach (ListItem item in ddlRoleType.Items)
        {
            if (item.Selected)
            {
                ...
                RoleTypeArray = RoleTypeArray + item.Value + ",";
            }
        }

        RoleTypeArray = RoleTypeArray.TrimEnd(',');
        ...
        foreach (ListItem item in EmployeeIdSelect.Items)
        {
            if (item.Selected)
            {
                ...
                EmployeeArray = EmployeeArray + item.Value + ",";
            }
        }

        EmployeeArray = EmployeeArray.TrimEnd(',');

        if(RoleTypeArray == "" && EmployeeArray == "")
        {
            ddlRoleType.ToolTip = "please fill out this field";
            ddlRoleType.CssClass = "form-select form-select-sm mb-3 multiple-select is-invalid";
            ddlRoleType.Focus();


            EmployeeIdSelect.ToolTip = "please fill out this field";
            EmployeeIdSelect.CssClass = "form-select form-select-sm mb-3 multiple-select is-invalid";
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Validation())
        {
            ...
            aMaster.ExpenseAmount = string.IsNullOrEmpty(txtAmount.Text) ? (decimal?)null : decimal.Parse(txtAmount.Text);
            aMaster.RoleType_xp = 0;
            ...
            aMaster.RoleTypeMult = RoleTypeArray;
            aMaster.EmpNameMult = EmployeeArray;
            ...
            aMaster.isFixed = chkFixed.Checked;
