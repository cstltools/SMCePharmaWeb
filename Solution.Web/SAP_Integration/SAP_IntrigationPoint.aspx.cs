...
public partial class SAP_Integration_SAP_IntrigationPoint : System.Web.UI.Page
{
    ...
    private void LoadInfoNewMethod()
    {
        ...
        {
            dtTable = _DAL.GetSAP_StockReceivePendingData("");
    ...
    protected void previewButton_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("SAP_StockReceive.aspx");
    }
    ...
    protected void lbApprove_Click(object sender, EventArgs e)
    {
        ...
        // Debugging: Log the row index and total rows count
        //System.Diagnostics.Debug.WriteLine($"Row Index: {rowindex}");
       // System.Diagnostics.Debug.WriteLine($"Total Rows: {gv_ProductInfo.Rows.Count}");

        // Ensure the row index is within valid range
        if (rowindex >= 0 && rowindex < gv_ProductInfo.Rows.Count)
        {
            HiddenField hfstatus = (HiddenField)gv_ProductInfo.Rows[rowindex].Cells[0].FindControl("hfstatus");
            HiddenField hfproduct_code = (HiddenField)gv_ProductInfo.Rows[rowindex].Cells[0].FindControl("hfproduct_code");
            HiddenField hfproduct_id = (HiddenField)gv_ProductInfo.Rows[rowindex].Cells[0].FindControl("hfproduct_id");

            // Uncomment and implement your condition check if necessary
            // if (AllConditionCheck(hfproduct_id.Value, hfproduct_code.Value, hfstatus.Value))
            {
                ResultInfo Res = _DAL.SaveProductInfoDAL(hfproduct_id.Value, hfproduct_code.Value, hfstatus.Value);
                if (Res.isSuccess == true)
        ...
        {
            // Log an error or show a message if the row index is out of range
            System.Diagnostics.Debug.WriteLine("Row index is out of range.");
        }
