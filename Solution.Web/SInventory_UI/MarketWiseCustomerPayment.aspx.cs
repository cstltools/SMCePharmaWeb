    ...
    protected void saveButton_Click(object sender, EventArgs e)
    {
        if (Validation())
        {
            ...
            for (int i = 0; i < orderGridView.Rows.Count; i++)
            {
                ...
                if (orderGridView.Rows[i].Cells[8].Text != "&nbsp;")
                {
                     prevamount = Convert.ToDecimal(orderGridView.Rows[i].Cells[8].Text);
                }
                if (orderGridView.Rows[i].Cells[10].Text != "&nbsp;")
                {
                    adjustAmount = Convert.ToDecimal(orderGridView.Rows[i].Cells[10].Text);
                }
                ...
                if (ChkBoxRows.Checked)
                {
                    ...
                    aCustPayment.MarketId = Convert.ToInt32(orderGridView.DataKeys[i]["MarketId"].ToString());
                        //Convert.ToInt32(marketDropDownList.SelectedValue);
                  ...
                  bool sts =  aCustPaymentBll.SaveCustPayment(aCustPayment, aCustPaymentDetail);

                  if (sts==true)
                    {
                        if ((Convert.ToDecimal(payAmountTextBox.Text) + prevamount + adjustAmount) ==
