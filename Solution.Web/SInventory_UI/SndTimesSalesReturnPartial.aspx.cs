...
public partial class SInventory_UI_SndTimesSalesReturnPartial : System.Web.UI.Page
{
    private RequisitionBLL aRequisitionBll = new RequisitionBLL();
    private SalesReturnBLL aInvoiceBll = new SalesReturnBLL();
    ...
    private bool Validation()
    {
        ...
        if (ViewState["IsCalculated"] == null || !(bool)ViewState["IsCalculated"])
        {
        ...
        { 
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "faildalert('" + "Please Input Order Number!" + "','Faild');", true);
            return false;
        }
        ...
        for (int i = 0; i < gridLineItemGridView.Rows.Count; i++)
        {
                ...
                ((Label)gridLineItemGridView.Rows[i].Cells[15].FindControl("statusLabel")).Text == "Parial")
            {
                        ...
                        .SelectedIndex == 0)
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "faildalert('" + "Please Select Reason!" + "','Faild');", true);
                    return false;
                }
            }
        }
    ...
    {
        Response.Redirect("SalesReturnList_new.aspx");
    }
    public void LoadInvoice(string invoiceId)
    {
        OrderInfoBLLSalesReturn aOrderInfoBll = new OrderInfoBLLSalesReturn();
    ...
    /// writes that follow it run one at a time across ALL 2nd-time sales-return submits, closing
    ...
    private static void AcquireSndTimesSalesReturnSubmitLock(SqlConnection connection, SqlTransaction transaction, string invoiceId)
    {
        using (SqlCommand cmd = new SqlCommand("sp_getapplock", connection, transaction))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@Resource", "SndTimesSalesReturnSubmit_Global");
            ...
            if (result < 0)
            {
                throw new InvalidOperationException(
                    "Could not acquire the 2nd-time sales return submit lock for InvoiceId=" + invoiceId +
                    " (sp_getapplock result=" + result + "). Another submit for this invoice may be in progress.");
            }
        }
    }

    /// Releases the session-owned lock taken by AcquireSndTimesSalesReturnSubmitLock once the
    /// submit finishes (success or failure) so other submits are not blocked behind it.
    private static void ReleaseSndTimesSalesReturnSubmitLock(SqlConnection connection, SqlTransaction transaction)
    {
    ...
    protected void saveButton_Click(object sender, EventArgs e)
    {
        ...
        if (Session["IsSndTimesSalesReturnSubmitting"] != null &&
            (bool)Session["IsSndTimesSalesReturnSubmitting"] == true)
        {
            return;
        }

        Session["IsSndTimesSalesReturnSubmitting"] = true;
        try
        {
            if (Validation() == true)
            {
                try
                {
                    ...
                    BonusCampaignNewDAL _BonusCampaignNewDAL = new BonusCampaignNewDAL();
                    //DelivaryInvoiceNo
                    using (DataTable dt = _BonusCampaignNewDAL.GetDelivaryInvoiceNoCheckById(invoiceHiddenField.Value, "SndReturnInvoiceNo"))
                    {
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            // deduction) runs on a single connection/transaction so it commits or
                            // rolls back as one atomic unit: either every table is updated, or none is.
                            using (SqlConnection connection = new SqlConnection(connectionString))
                            {
                                ...
                                try
                                {
                                    AcquireSndTimesSalesReturnSubmitLock(connection, transaction, invoiceHiddenField.Value);

                                    // Re-check inside the lock+transaction: the pre-flight check above
                                    // can be beaten by a concurrent submit; this recheck cannot.
                                    DataTable existingReturn = _BonusCampaignNewDAL.GetDelivaryInvoiceNoCheckById(
                                        invoiceHiddenField.Value, "SndReturnInvoiceNo", transaction);
                                    if (existingReturn.Rows.Count > 0)
                                    {
                                    ...
                                    ReleaseSndTimesSalesReturnSubmitLock(connection, transaction);

                                    transaction.Commit();
                                    committed = true;

                                    ViewState["IsCalculated"] = null;
                                    Clear();

                                    ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "ShowSuccesalert('" + "Generate Invoice Created Successsfully!" + "','Success');", true);
                                }
                                catch (Exception ex)
                                {
                                    if (!committed)
                                    ...
                                    System.Diagnostics.Trace.TraceError(
                                        "2nd-Time Sales Return Submit failed for InvoiceId={0}: {1}", invoiceHiddenField.Value, ex);
                                    throw;
                                }
                            }
                        }
        ...
        {
            Session["IsSndTimesSalesReturnSubmitting"] = false;
        }
    ...
    private bool SaveInvoiceDetail(int invoiceId, SqlTransaction transaction)
    {
        ...
        if (gridLineItemGridView.Rows.Count > 0)
        {
            for (int i = 0; i < gridLineItemGridView.Rows.Count; i++)
            {
                ...
                aInvoiceDetail.InvoiceId =Convert.ToInt32(invoiceHiddenField.Value);
                aInvoiceDetail.Quantity =
                ...
                string value = ((HiddenField)gridLineItemGridView.Rows[i].Cells[13].FindControl("hfDelTotalQty")).Value;

                if (decimal.TryParse(value, out previousQty))
                {
                    aInvoiceDetail.PreviousQuantity = previousQty;
                }
                else
                {
                    aInvoiceDetail.PreviousQuantity = 0; // অথবা আপনি চাইলে error throw করতে পারেন
                }


                aInvoiceBll.PaymentUpdateInvoiceDetail(aInvoiceDetail, Convert.ToInt32(invoiceHiddenField.Value), transaction);
            ...
            if (delqty <= mainqty)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            // ধরলাম campaign match হওয়া item কয়টা => mainQty


                            // আপনার delivered qty
                            delqty = Convert.ToDecimal(delqty); // delQty আপনার ভ্যারিয়েবল

                            // ধরলাম dtEze থেকে DiscountPercentage আসবে
    ...
    protected void dQtyTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "faildalert('" + "Cannot be greater then Payment Quantity!" + "','Faild');", true);
    ...
    private void CalculateQty()
    {
        ...
        {
            int rowindex = i;
            HiddenField hfDelTotalQty = (HiddenField)gridLineItemGridView.Rows[rowindex].Cells[14].FindControl("hfDelTotalQty");
            TextBox dQty = (TextBox)gridLineItemGridView.Rows[rowindex].Cells[14].FindControl("dQtyTextBox");
