...
public partial class SInventory_UI_CustomerLedgerReportApps : System.Web.UI.Page
{
        ...
        if (empName != "")
        {
            try
            {
                if (empName.Contains(':'))
                {
                    ...
                    string[] parts = cCodeTextBox.Text.Split(':');
                    if (parts.Length > 0)
                    {
                        string firstPart = parts[0].Trim();  // This will give you the part before the colon
                        hfCustomerId.Value = firstPart;      // Set the hfCustomerId.Value to the first part
                    }
    ...
    private void LoadData()
    {
        ...
        if (hfCustomerId.Value.Trim() != "" && InvoiceDateTextBox.Text != "" && todateTextBox.Text != "")
        {

            DateTime toDate = Convert.ToDateTime(InvoiceDateTextBox.Text);
            DateTime
            fromDate = Convert.ToDateTime(todateTextBox.Text);
            string CustomerID = hfCustomerId.Value.Trim();

            aDataTable = aCustomerMasterBll.CustomerLedgerBll(CustomerID, InvoiceDateTextBox.Text, todateTextBox.Text);
        ...
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "faildalert('" + "Please input mandatory field!" + "','Faild');", true);
        }
