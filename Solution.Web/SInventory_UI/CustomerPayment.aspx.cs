...
public partial class SInventory_UI_CustomerPayment : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["SalesCenter"] != null)
            {

                aCustPaymentBll.LoadSC(salesCenterDropDownList, Session["UserId"].ToString());


                salesCenterDropDownList.SelectedValue = Session["SalesCenter"].ToString();
                ...
                {
                    rootDropDownList.SelectedValue = Session["Route"].ToString();
                    rootDropDownList_SelectedIndexChanged(null, null);

                }

                // Territory
                if (Session["Territory"] != null)
                {
                    ddlTerritoryName.SelectedValue = Session["Territory"].ToString();
    ...
    public bool Validation()
    {
        ...
        if (orderGridView.Rows.Count > 0)
        {
            for (int i = 0; i < orderGridView.Rows.Count; i++)
            {
                ...
                if (((CheckBox)orderGridView.Rows[i].Cells[1].FindControl("chkSelect")).Checked)
                {
                    if (((DropDownList)orderGridView.Rows[i].FindControl("ddlCollectionBy")).SelectedValue == "")
                    {
                        showMessageBox("Please Select Collection By !!");
                        return false;
                    }
                    countPay++;
                }
    ...
    private string GenerateParameter()
    {
        ...
        for (int i = 0; i < orderGridView.Rows.Count; i++)
        {
            var chkSelect = (CheckBox)orderGridView.Rows[i].Cells[0].FindControl("chkSelect");
            var hfInvoiceId = (HiddenField)orderGridView.Rows[i].Cells[0].FindControl("hfInvoiceId");

            if (chkSelect != null && hfInvoiceId != null && chkSelect.Checked)
            {
                if (salesCenterDropDownList.SelectedValue != "")
                {
                    if (found) param.Append(","); // যদি একাধিক আইডি হয়
        ...
        // যদি কোনো চেক নাই, তাহলে return empty string
    ...
    private void LoadGridView()
    {
        ...
        Session["SalesCenter"] = salesCenterDropDownList.SelectedValue;

        // Route value Session-এ store
        Session["Route"] = rootDropDownList.SelectedValue;

        // Territory value Session-এ store
        Session["Territory"] = ddlTerritoryName.SelectedValue;
        lblCount.Text = "Total Pay Amount: 0";
        DataTable aTable = aDal.LoadPaymentInvSP(GenerateParam());
    ...
    private void CollectionByChange()
    {
        for (int i = 0; i < orderGridView.Rows.Count; i++)
        {
            ...
            else if (ddlDAName.SelectedItem.Text == "X. MIO")
            {
                ddlCollectionBy.SelectedValue = "X. MIO";
            }


            else if (ddlDAName.SelectedItem.Text == "Please Select From List")
            {
                ddlCollectionBy.SelectedIndex = 0;// "X. MIO";
            }
            else
            {
