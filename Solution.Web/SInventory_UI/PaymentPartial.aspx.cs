...
public partial class SInventory_UI_PaymentPartial : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("DelivaryInvoiceCreationAfterSalesConfirm.aspx");
    }
    ...
    /// writes that follow it run one at a time across ALL partial-payment submits, closing the
    /// same TOCTOU race handled for the other Invoice/Delivery submit flows: the pre-flight
    /// GetDelivaryInvoiceNoCheckById above can be beaten by a concurrent submit for the same
    /// invoice; this recheck cannot, because the app lock serializes it.
    private static void AcquirePaymentPartialSubmitLock(SqlConnection connection, SqlTransaction transaction, string invoiceId)
    {
        using (SqlCommand cmd = new SqlCommand("sp_getapplock", connection, transaction))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@Resource", "PaymentPartialSubmit_Global");
            ...
            if (result < 0)
            {
                throw new InvalidOperationException(
                    "Could not acquire the payment partial submit lock for InvoiceId=" + invoiceId +
                    " (sp_getapplock result=" + result + "). Another submit for this invoice may be in progress.");
            }
        }
    }

    /// Releases the session-owned lock taken by AcquirePaymentPartialSubmitLock once the submit
    /// finishes (success or failure) so other submits are not blocked behind it.
    private static void ReleasePaymentPartialSubmitLock(SqlConnection connection, SqlTransaction transaction)
    {
    ...
    protected void saveButton_Click(object sender, EventArgs e)
    {
        ...
        if (Session["IsPaymentPartialSubmitting"] != null &&
            (bool)Session["IsPaymentPartialSubmitting"] == true)
        {
            return;
        }

        Session["IsPaymentPartialSubmitting"] = true;
        try
        {
            if (Validation() == true)
            {
                try
                {
                    ...
                    //DelivaryInvoiceNo
                    using (DataTable dt = _BonusCampaignNewDAL.GetDelivaryInvoiceNoCheckById(invoiceHiddenField.Value, "PaymentInvoiceNo"))
                    {
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            using (SqlConnection connection = new SqlConnection(connectionString))
                            {
                                ...
                                try
                                {
                                    AcquirePaymentPartialSubmitLock(connection, transaction, invoiceHiddenField.Value);
                                    ...
                                    DataTable existingPayment = _BonusCampaignNewDAL.GetDelivaryInvoiceNoCheckById(
                                        invoiceHiddenField.Value, "PaymentInvoiceNo", transaction);
                                    ...
                                    ReleasePaymentPartialSubmitLock(connection, transaction);
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    System.Diagnostics.Trace.TraceError(
                                        "Payment Partial Submit failed for InvoiceId={0}: {1}", invoiceHiddenField.Value, ex);
                                    throw;
                                }
                            }
                        }
        ...
        {
            Session["IsPaymentPartialSubmitting"] = false;
        }
    ...
    public void DqtyCalculation()
    {
        for (int i = 0; i < gridLineItemGridView.Rows.Count; i++)
        {
            ...
            if (delqty <= mainqty)
            {
                ...
                {
                    bool oldTradePolicy = false;

                    if (dtOffer.Rows.Count > 0 && dtOffer.Rows[0]["OldTradePolicy"] != DBNull.Value)
                    {
                        string value = dtOffer.Rows[0]["OldTradePolicy"].ToString().Trim();
