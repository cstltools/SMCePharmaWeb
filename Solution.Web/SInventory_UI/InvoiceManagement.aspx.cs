...
public partial class SInventory_UI_InvoiceManagement : System.Web.UI.Page
{
    ...
    protected void saveButton_Click(object sender, EventArgs e)
    {
        if (Validation() == true)
        {
            ...
            {
                grandTotal = Convert.ToDecimal(grandTotalTextBox.Text.Trim());
            }

            if (grandTotal > 50000 && !chbOverrideLimit.Checked)
            {
                showMessageBox("Invoice value exceeds BDT 50,000. Authorized override required.");
                return;
            }
    ...
    protected void custNameTextBox_TextChanged(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(custName))
        {
            if (custName.Contains(':'))
            {
                ...
                custCodeTextBox.Text = custInfo[0].Trim();
                string custCode = custCodeTextBox.Text.Trim();
                GetCustInfo(custCode);
            }
        }       
    ...
    protected void searchListButton_Click(object sender, EventArgs e)
    {
        LoadInvoiceList();
    }
    ...
    {
        if (searchDateTextBox.Text != "")
        {
            string comUnitId = Session["ComUnitId"].ToString();
            DataTable aDataTable = aInvoiceBll.AllInvoiceForPrintingBLL(comUnitId, searchDateTextBox.Text.Trim());
            if (aDataTable != null && aDataTable.Rows.Count > 0)
            {
                gvInvoiceList.DataSource = aDataTable;
            ...
            else
            {
                ...
                gvInvoiceList.DataBind();
                showMessageBox("No invoices found for the selected date.");
            }
        }
        else
        {
            showMessageBox("Please select a date to search.");
        }
    }
    ...
    protected void gvInvoiceList_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            DataTable mainData = aInvoiceBll.LoadInvoice(invoiceNo);
            if (mainData != null && mainData.Rows.Count > 0)
            {
                DataRow dr = mainData.Rows[0];
                hdInvoiceId.Value = dr["InvoiceId"].ToString();
                hdInvoiceNo.Value = invoiceNo;

                invDateTextBox.Text = Convert.ToDateTime(dr["InvoiceDate"]).ToString("dd-MMM-yyyy");
                orderNoTextBox.Text = dr["OrderNo"].ToString();
                orderDateTextBox.Text = Convert.ToDateTime(dr["OrderDate"]).ToString("dd-MMM-yyyy");
                
                GetCustInfo(dr["CustomerCode"].ToString());
                
                DataTable detailData = aInvoiceBll.LoadInvoiceWithDetail(hdInvoiceId.Value);
                if (detailData != null && detailData.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dRow in detailData.Rows)
                    {
                        ...
                        newRow["UnitVAT"] = dRow["UnitVatAmount"];
                        newRow["Quantity"] = dRow["Quantity"];
                        newRow["TotalPrice"] = Convert.ToDecimal(dRow["UnitPrice"]) * Convert.ToDecimal(dRow["Quantity"]);
                        newRow["VAT"] = Convert.ToDecimal(dRow["UnitVatAmount"]) * Convert.ToDecimal(dRow["Quantity"]);
                        newRow["DiscountPercentage"] = dRow["DiscountPercentage"];
                        newRow["DiscountAmount"] = "0"; 
                        newRow["NetPrice"] = Convert.ToDecimal(dRow["UnitPrice"]) * Convert.ToDecimal(dRow["Quantity"]);
                        newRow["BonusQty"] = dRow["BonusQuantity"];
                        newRow["TotalQty"] = dRow["Quantity"];
        ...
        {
            aInvoiceBll.DeleteInvoice(invoiceNo);
            showMessageBox("Invoice deleted successfully.");
            LoadInvoiceList();
        }
    }
    ...
    protected void updateButton_Click(object sender, EventArgs e)
    {
        if (Validation() == true)
        {
            ...
            aInvoiceBll.UpdateInvoice(aInvoice, out outInvoiceNo);
            
            using (System.Data.SqlClient.SqlConnection con = new System.Data.SqlClient.SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SolutionConnectionStringSSIDB"].ToString())) {
                con.Open();
                new System.Data.SqlClient.SqlCommand("DELETE FROM tblInvoiceDetail WHERE InvoiceId=" + hdInvoiceId.Value, con).ExecuteNonQuery();
            }

            SaveInvoiceDetail(Convert.ToInt32(hdInvoiceId.Value));
            Clear();
            saveButton.Visible = true;
            updateButton.Visible = false;
            hdInvoiceId.Value = "";
            hdInvoiceNo.Value = "";
            showMessageBox("Invoice Updated Successfully");
            LoadInvoiceList();
        }
    }
}