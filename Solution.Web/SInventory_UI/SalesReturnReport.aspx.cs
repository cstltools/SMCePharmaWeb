...
public partial class SInventory_UI_SalesReturnReport : System.Web.UI.Page
{
    ...
    private void LoadData()
    {
        ...
        comUnitDetailDataTable = _DAL.GetSalesReturnReportListDAL(Parm(), Parm2());
        ...
        if (comUnitDetailDataTable.Rows.Count > 0)
        {
           ...
           decimal total2 = comUnitDetailDataTable.AsEnumerable().Sum(row => row.Field<decimal?>("ReturnGrossAmt") == null ? 0 : row.Field<decimal>("ReturnGrossAmt"));

            lblCount.Text = "Return Amount (TP+VAT) : " + total2.ToString("N2");
        }
        else
        {
            loadGridView.DataSource = null;
            loadGridView.DataBind();
            lblCount.Text = "Return Amount (TP+VAT) : " + 0.ToString("N2");
        }
    }
    ...
    private string Parm()
    {
        ...
        {
            param = param + " AND CONVERT(date,I.PaymentDate)  BETWEEN '" + InvoiceDateTextBox.Text + "' AND '" + todateTextBox.Text + "' ";
        }
        if (InvoiceDateTextBox.Text != "" && todateTextBox.Text == "")
        {
            param = param + " AND CONVERT(date,I.PaymentDate)  BETWEEN '" + InvoiceDateTextBox.Text + "' AND '" + DateTime.Now + "' ";
        }
    ...
    private string Parm2()
    {
        ...
        string param = "";
            ...
            if (dcDropDownList1.SelectedValue != "")
            {
                param = param + " AND  iv.ComUnitId ='" + dcDropDownList1.SelectedValue + "' ";
            }
        ...
        if (InvoiceDateTextBox.Text != "" && todateTextBox.Text != "")
        {
            param = param + " AND CONVERT(date, iv.SndReturnPaymentDate)  BETWEEN '" + InvoiceDateTextBox.Text + "' AND '" + todateTextBox.Text + "' ";
        }
        if (InvoiceDateTextBox.Text != "" && todateTextBox.Text == "")
        {
            param = param + " AND CONVERT(date, iv.SndReturnPaymentDate)  BETWEEN '" + InvoiceDateTextBox.Text + "' AND '" + DateTime.Now + "' ";
        }
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        if (loadGridView.Rows.Count > 0)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=Sales_Return_Report_" + DateTime.Now.ToString("dd_MMM_yyyy_hh_mm_tt") + ".csv");
            Response.Charset = "";
