...
public partial class SInventory_UI_DelivaryInvoiceCreationForCustomerByOrderAuto_Loading : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("LoadingSummary.aspx");
    }
    ...
    /// writes that follow it run one at a time across ALL delivery-invoice submits, closing the
    /// same TOCTOU race described in InvoiceCreationForCustomerByOrder.aspx.cs: the pre-flight
    ...
    private static void AcquireDeliveryInvoiceSubmitLock(SqlConnection connection, SqlTransaction transaction, string invoiceId)
    {
        using (SqlCommand cmd = new SqlCommand("sp_getapplock", connection, transaction))
        {
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@Resource", "DelivaryInvoiceSubmit_Global");
            ...
            if (result < 0)
            {
                throw new InvalidOperationException(
                    "Could not acquire the delivery invoice submit lock for InvoiceId=" + invoiceId +
                    " (sp_getapplock result=" + result + "). Another submit for this invoice may be in progress.");
            }
        }
    }

    /// Releases the session-owned lock taken by AcquireDeliveryInvoiceSubmitLock once the submit
    /// finishes (success or failure) so other submits are not blocked behind it.
    private static void ReleaseDeliveryInvoiceSubmitLock(SqlConnection connection, SqlTransaction transaction)
    {
    ...
    protected void saveButton_Click(object sender, EventArgs e)
    {
        ...
        if (Session["IsDelivaryInvoiceSubmitting"] != null &&
            (bool)Session["IsDelivaryInvoiceSubmitting"] == true)
        {
            return;
        }

        Session["IsDelivaryInvoiceSubmitting"] = true;
        try
        {
            if (Validation() == true)
            {
                    ...
                    using (DataTable dt = _BonusCampaignNewDAL.GetDelivaryInvoiceNoCheckById(invoiceHiddenField.Value, "DelivaryInvoiceNo"))
                    {
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            using (SqlConnection connection = new SqlConnection(connectionString))
                            {
                                ...
                                try
                                {
                                    AcquireDeliveryInvoiceSubmitLock(connection, transaction, invoiceHiddenField.Value);
                                    ...
                                    ReleaseDeliveryInvoiceSubmitLock(connection, transaction);
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    System.Diagnostics.Trace.TraceError(
                                        "Delivery Invoice Submit failed for InvoiceId={0}: {1}", invoiceHiddenField.Value, ex);
                                    throw;
                                }
                            }
                        }
        ...
        {
            Session["IsDelivaryInvoiceSubmitting"] = false;
        }
    }
