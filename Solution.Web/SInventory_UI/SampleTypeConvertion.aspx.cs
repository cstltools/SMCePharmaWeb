...
public partial class SInventory_UI_SampleTypeConvertion : System.Web.UI.Page
{
    ...
    protected void ListImageButton_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("~/SInventory_UI/SampleTypeConventionView.aspx");
    ...
    protected void addButton_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i <   DerectStoctGridView .Rows.Count; i++)
        {
            ...
            if (ChkBoxRows.Checked && ((TextBox)DerectStoctGridView.Rows[i].Cells[6].FindControl("ConventionTextBox")).Text.Trim() != "")
            {
                if (HasSubDCStoreId(Convert.ToInt32(DerectStoctGridView.DataKeys[i][0].ToString())))
                {
                    ...
                    dataRow["DCStoreId"] = DerectStoctGridView.DataKeys[i][0].ToString();
                    ...
                    dataRow["StockQty"] = DerectStoctGridView.Rows[i].Cells[3].Text;
        ...
        for (int i = 0; i < ProductGridView.Rows.Count; i++)
        {
            ...
            dataRow["StockQty"] = ProductGridView.Rows[i].Cells[5].Text;
    ...
    {
        DataTable dtdata = aConventionBll.GetProductDcStore(productDropDownList.SelectedValue, DistributioncenterDropDownList1.SelectedValue);
    ...
    private bool Validation()
    {
        ...
        {
            showMessageBox("Please Add product and Stock  Qty");
            return false;
        }
        return true;
    }
    protected void submitButton_Click1(object sender, EventArgs e)
    {
        if (Validation())
        {
            SampleStockForDcMaster aDcStockOutMasterDao = new SampleStockForDcMaster()
            {
            ...
            if (status == true)
            {
                for (int i = 0; i < ProductGridView.Rows.Count; i++)
                {
                    ...
                    aStockOutDetailsDao.SampleStockForDcMasterId = SubDcStockOutMasterId;
                    aStockOutDetailsDao.DCStoreId = Convert.ToInt32(ProductGridView.DataKeys[i][0].ToString());
                ...
                {
                    if (aConventionBll.StockOut(aStockOutDetailsDaoList))
                    {
