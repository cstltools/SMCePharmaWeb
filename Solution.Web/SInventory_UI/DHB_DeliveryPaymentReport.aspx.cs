...
public partial class SInventory_UI_DHB_DeliveryPaymentReport : System.Web.UI.Page
{
    ...
    GroupSelect = (DropDownList)IVMarketStructure.FindControl("GroupSelect") as DropDownList;
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                {
                    dcDropDownList1.DataSource = dt;
                    dcDropDownList1.DataValueField = "ComUnitId";
                    dcDropDownList1.DataTextField = "ComUnitName";
                    dcDropDownList1.DataBind();
                    dcDropDownList1.Items.Insert(0, new ListItem("Please Select From List", String.Empty));
                    dcDropDownList1.SelectedIndex = 0;

                    if (Session["RoleTypeName"].ToString() == "DIC")
                    {
                        dcDropDownList1.SelectedValue = Session["DICCompanyUnitId"].ToString();
    ...
    private void LoadData()
    {
        ...
        comUnitDetailDataTable = _DAL.GetDBH_DeliveryPaymentDAL(Parm(), Parm_2());
    ...
    private string Parm()
    {
        ...
        if (InvoiceDateTextBox.Text != "" && todateTextBox.Text == "")
        {
            param = param + " AND CONVERT(date,custDtl.custPaymentDate)  BETWEEN '" + InvoiceDateTextBox.Text + "' AND '" + DateTime.Now + "' ";
        }
