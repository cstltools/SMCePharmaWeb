...
public partial class Thana_UI_ThanaEntry : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static ResultInfo Save_DistictInfo(ThanaDao thanaDao)
    {


        return (_Thana.Save_DistictInfo(thanaDao, HttpContext.Current.Session["UserId"].ToString()));
    ...
    [WebMethod]
    public static string Get_District_By_Id(string id)
    {
        DataTable dt = _Thana.Get_District_By_Id(id);
        string JSONresult;
        JSONresult = JsonConvert.SerializeObject(dt);
        return JSONresult;
    }
