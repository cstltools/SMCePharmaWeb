...
namespace Library.DAL.SubDepot_DAL
{
    public class SubDepoAdjustDAL
    {
        ...
        public bool SaveDataForSubDepoAdjust(SubDepoAdjustMentDAO adjustMentDao)
        {
            string insertQuery = @"insert into tblSubDepoStockAdjustment (SubDepoStockOutId,SubDepoStockCode,EntryBy,EntryDate) 
            values (" + adjustMentDao.SubDepoStockOutId + ",'" + adjustMentDao.SubDepoStockCode + "','" + adjustMentDao.EntryBy + "','" + adjustMentDao.EntryDate + "')";
            return aCommonInternalDal.SaveDataByInsertCommand(insertQuery, "SSIDB");
        }
        public bool SaveDataForSubDepoAdjustDetail(SubDepoAdjustDetailDAO adjustMentDao)
        {
            string insertQuery = @"INSERT INTO dbo.tblSubDepoStockAdjustDetail
(
    SubDCStoreId,
    SubDepoStockOutId,Quantity
)
VALUES

             (" + adjustMentDao.SubDCStoreId + ",'" + adjustMentDao.SubDepoStockOutId + "','" + adjustMentDao.Quantity + "')";
            return aCommonInternalDal.SaveDataByInsertCommand(insertQuery, "SSIDB");
        }
...
FROM dbo.tblSubDepotChalanInfo
        ...
        public void UpdateSubDepoStockQuantity(string stockId, decimal Quantity)
        {
            string updateQuery = @"UPDATE tblSubDepotStore SET StockQty=StockQty-" + Quantity + " WHERE SubDCStoreId='" + stockId.Trim() + "'   ";
            aCommonInternalDal.UpdateDataByUpdateCommand(updateQuery, "SSIDB");
        }
...
WHERE tblChalanInfo.ChalanNo='" + id + "'";
        ...
        {
            DataTable aDataTableEmpInfo = new DataTable();
            string query = @"SELECT ISNULL(VATAmountPerUnit,0)VATAmountPerUnit,ISNULL(VATPercentage,0)VATPercentage,UnitPrice,SubDCStoreId AS DCStoreId,*,
            tblProduct.ProductCode AS PCode , tblProduct.ProductName AS PName 
            FROM dbo.tblSubDepotStore
            LEFT JOIN dbo.tblProduct ON dbo.tblSubDepotStore.ProductCode = dbo.tblProduct.ProductCode 
            LEFT JOIN dbo.tblSubDepot ON tblSubDepot.SubDepotId = tblSubDepotStore.SubDepotId
            LEFT JOIN dbo.tblUnitPrice ON dbo.tblSubDepotStore.ProductCode=dbo.tblUnitPrice.ProductCode
            WHERE  tblSubDepotStore.SubDepotId='" + comunitId + "' AND tblProduct.ProductId='" + productCode + "'  AND tblUnitPrice.IsActive='True' AND StockQty>0 order by tblProduct.ProductCode";
            //tblDCStore.StockInTransfarId is not null AND
            aDataTableEmpInfo = aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
            return aDataTableEmpInfo;
        }
