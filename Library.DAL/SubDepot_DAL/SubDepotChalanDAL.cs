...
public DataTable GetProductDcStoreSubdeport(string productCode, string comunitId)
{
    ...
    string query = @"SELECT format(ExpDate,'dd MMM, yyyy') as ExpDate, format(ReceiveDate,'dd MMM, yyyy') as ReceiveDate, ISNULL(VATAmountPerUnit,0)VATAmountPerUnit,ISNULL(VATPercentage,0)VATPercentage,UnitPrice,*,
            ...
            LEFT JOIN dbo.tblCompanyUnit ON dbo.tblDCStore.ComUnitId = dbo.tblCompanyUnit.ComUnitId 
            LEFT JOIN dbo.tblUnitPrice ON dbo.tblDCStore.ProductCode=dbo.tblUnitPrice.ProductCode
            WHERE  ComUnitCode='" + comunitId + "' AND tblProduct.ProductId='"+productCode+"'  AND tblUnitPrice.IsActive='True'  order by tblProduct.ProductCode";
            //tblDCStore.StockInTransfarId is not null AND AND StockQty>0
            aDataTableEmpInfo = aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
    return aDataTableEmpInfo;
}
    }
