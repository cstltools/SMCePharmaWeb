...
- **`DataAccessManager.GetDataTable` loads results via `DataTable.Load(reader)`, which marks any SQL-computed/expression column `ReadOnly = true` on the resulting `DataTable`.** Page code that post-processes the result and writes back into a column with the same name as a computed column in the source query (common for building a display-friendly "Code : Name" string server-side) will throw `System.Data.ReadOnlyException` on the first row — see `docs/NewReceiveableReport_Bugfix.md` for a real instance of this crashing a report page silently (caught by `customErrors`, so it looked like "no data" rather than an error). If you need to write into a column that might come back from the query itself, clear `column.ReadOnly` first rather than assuming a freshly-added column.
